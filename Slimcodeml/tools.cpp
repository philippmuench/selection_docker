...
int
PopEmptyLines (FILE * fseq, int lline, char line[])
{
  /* pop out empty lines in the sequence data file.
     returns -1 if EOF.
   */
  const char *eqdel = ".-?";	//CMV added const
  char *p;			//CMV continutation of previous line
  int i;
  ...
  for (i = 0;; i++)
      ...
      if (p == NULL)
	{
      ...
      while (*p)
	if (*p == eqdel[0] || *p == eqdel[1] || *p == eqdel[2]
	    || isalpha (*p))
	  /*
...
int
difcodonNG (char codon1[], char codon2[], double *SynSite,
	    ...
	    int transfed, int icode)
{
  ...
  int ndiff, npath, nstop, sdpath, ndpath, dmark[3], step[3], b[2][3],
    bt1[3], bt2[3];
  int by[3] = { 16, 4, 1 };
  char str[4] = "";
  ...
  for (i = 0, *SynSite = 0, nstop = 0; i < 2; i++)
      ...
      for (j = 0; j < 3; j++)
	{
	  if (transfed)
	    {
	      b[i][j] = (i ? codon1[j] : codon2[j]);
	    }
	  else
	    b[i][j] = (int) CodeChara ((char) (i ? codon1[j] : codon2[j]), 0);
	  ...
	  if (b[i][j] < 0 || b[i][j] > 3)
	    {
	      if (noisy >= 9)
		printf ("\nwarning ambiguity in difcodonNG: %s %s",
			codon1, codon2);

	      *SynSite = 0.5;
	      *AsynSite = 2.5;
	      *SynDif = (codon1[2] != codon2[2]) ? 0.5 : 0;	//CMV put proper use of the bool test
	      *AsynDif =
		*SynDif + (codon1[0] != codon2[0]) + (codon1[1] != codon2[1]);
  ...
  *SynDif = *AsynDif = 0;
      ...
      for (k = 0; k < npath; k++)
	{
	  ...
	  for (i1 = 0; i1 < ndiff; i1++)	/* mutation steps for each path */
	      ...
	      for (i2 = 0, ic[0] = ic[1] = 0; i2 < 3; i2++)
		{
	      ...
	      for (i2 = 0; i2 < 2; i2++)
		{
		  iaa[i2] = GeneticCode[icode][ic[i2]];
		}
  ...
  if (npath == nstop)
    ...
    {
      *SynDif /= (double) (npath - nstop);
      *AsynDif /= (double) (npath - nstop);
    }
...
void
PMatUVRoot2 (double P[], double t, int n, double U[], double V[],
	     double Root[])
//CHHS Replacement for PMatUVRoot utilizing BLAS functions
{

  double TMP[61 * 61];		//CHHS Seems that U should not be overwritten, therefore we save it
  memcpy (TMP, U, sizeof (double) * n * n);
  /* P(t) = U * exp{Root*t} * V
   */
  int i, j, k;
  double expt, uexpt, *pP;
  double smallp = 0;
  double dummy;			//CHHS For BLAS
...
//CHHS Step 3: X1 = R*exp(LAMBDA*t/2)
//Eigenvectors (here called R) are in U after eigenRealSym()
//Eigenvalues LAMBDA in Root
  double da;
  for (i = 0; i < n; i++)
    {
      da = exp (Root[i] * t / 2);
      dscal_ (&n, &da, &TMP[i], &n);	//dscal_(N,DA,DX,INCX)
    }
//CHHS Step 4: X2 = X1*X1^T, X1 taken from U which has been computed above
//CHHS C := alpha*A**T*A + beta*C
  double alpha = 1, beta = 0;
  int lda = n, ldc = n;
  dsyrk_ ("U", "T", &n, &n, &alpha, TMP, &lda, &beta, P, &ldc);	// DSYRK(UPLO,TRANS,N,K,ALPHA,A,LDA,BETA,C,LDC); result symmetric, hence only a triangle filled by dsyrk
//CHHS Generate full matrix from triangle
  for (i = 0; i < n; i++)
    for (j = 0; j < i; j++)
      if (i > j)
	P[j * n + i] = P[i * n + j];
//CHHS Step 5, computing P = PI^(-1/2) * X2 * PI^(1/2) using BLAS routines; this may also be done in a single operation (see Step 1)
//CHHS First computing X2 = PI^(-1/2) * X2
  int ONE = 1;
//CHHS We only take com.pi for the PI values, for AA models this does NOT work!
  for (i = 0; i < n; i++)
    {
      da = 1 / sqrt (com.pi[i]);	//CHHS Frequencies cannot become 0 here
      dscal_ (&n, &da, &P[i * n], &ONE);	//dscal_(N,DA,DX,INCX)
    }
//CHHS Now computing X2 * PI^(1/2)
  for (i = 0; i < n; i++)
    {
      da = sqrt (com.pi[i]);	//CHHS Frequencies cannot become 0 here
      dscal_ (&n, &da, &P[i], &n);	//dscal_(N,DA,DX,INCX)
    }
}

int
PMatQRev (double Q[], double pi[], double t, int n, double space[])
{
  /* This calculates P(t) = exp(Q*t), where Q is the rate matrix for a
     ...
     space[n*n*2+n*2]
   */
  double *U = space, *V = U + n * n, *Root = V + n * n, *spacesqrtpi =
    Root + n;

  eigenQREV (Q, pi, n, Root, U, V, spacesqrtpi);
  PMatUVRoot2 (Q, t, n, U, V, Root);
  return (0);
}
...
int
PMatK80 (double P[], double t, double kappa)
{
  ...
  if (fabs (kappa - 1) < 1e-5)
  ...
  else
    {
      ...
      P[0 * 4 + 1] = P[1 * 4 + 0] = P[2 * 4 + 3] = P[3 * 4 + 2] =
	(1 + e1 - 2 * e2) / 4;
      P[0 * 4 + 2] = P[0 * 4 + 3] = P[2 * 4 + 0] = P[3 * 4 + 0] =
	P[1 * 4 + 2] = P[1 * 4 + 3] = P[2 * 4 + 1] = P[3 * 4 + 1] =
	(1 - e1) / 4;
    }
...
char *
getcodon (char codon[], int icodon)
{
  /* id : (0,63) */
...
char *
getAAstr (char *AAstr, int iaa)
{
  /* iaa (0,20) with 20 meaning termination */
  if (iaa < 0 || iaa > 20)
    {
      error2 ("getAAstr: iaa err. \n");
    }
...
int
NucListall (char b, int *nb, int ib[4])
{
...
int
DNA2protein (char dna[], char protein[], int lc, int icode)
{
  ...
  for (h = 0; h < lc; h++)
    {
      ...
      if (k == -1)
	{
	  printf (" stop codon at %d out of %d\n", h + 1, lc);
	}
...
int
printcu (FILE * fout, double fcodon[], int icode)
{
  ...
  const char *word = "|-";	//CMV added const
  char aa3[4] = "   ";		//CMV no changes, but taken from previous line
  char codon[4] = "   ";	//CMV no changes, but taken from previous line
  char ss3[4][4], *noodle;	//CMV no changes, but taken from previous line
      ...
      for (j = 0; j < 4; j++)
	{
	  for (k = 0; k < 4; k++)
	      ...
	      else
		{
	      ...
	      if (fcodon)
		{
		  fprintf (fout, "%*.*f", wc, wd, fcodon[it]);
		}
...
int
printcums (FILE * fout, int ns, double fcodons[], int icode)
{
  int neach0 = 6, neach = neach0, wc = 3, wd = 0;	/* wc: for codon, wd: decimal  */
  int iaa, it, i, j, k, i1, ngroup, igroup;
  const char *word = "|-";	//CMV const added
  char aa3[4] = "   ", codon[4] = "   ", ss3[4][4], *noodle;	//CMV no changes, but taken from previous line
	  ...
	  for (j = 0; j < 4; j++)
	      ...
	      for (k = 0; k < 4; k++)
		{
		  ...
		  for (i1 = 0; i1 < neach; i1++)
		    fprintf (fout, " %*.*f", wc - 1, wd,
			     fcodons[(igroup * neach0 + i1) * 64 + it]);
...
int
QtoPi (double Q[], double pi[], int n, double space[])
{
...
int
PtoPi (double P[], double pi[], int n, double space[])
{
...
int
ScanFastaFile (FILE * f, int *ns, int *ls, int *aligned)
{
...
int
printsma (FILE * fout, char *spname[], char *z[], int ns, int l, int lline, int gap, int seqtype,	//CHHS third parameter "unsigned" removed to unify pointer data types
	  int transformed, int simple, int pose[])
{
   ...
   */
  int igroup, ngroup, lt, h, hp, i, b, b0 = -1, igap, lspname =
    41, lseqlen = 7;
  char indel = '-', ambi = '?', equal = '.';
  char *pch = (seqtype <= 1 ? BASEs : (seqtype == 2 ? AAs : BINs));
  //char codon[4] = "   "; //CMV set but never used
  ...
  //codon[0] = -1;            /* to avoid warning */ //CMV never used
  ...
  for (igroup = 0, FPN (fout); igroup < ngroup; igroup++, FPN (fout))
    {
      ...
      if (igroup + 1 == ngroup)
	igap =
	  (l - igroup * lline) + (l - igroup * lline) / gap +
	  lspname + 1 - lseqlen - 1;
      ...
      for (i = 0; i < ns; i++)
	{
	  if (spname)
	    {
	      fprintf (fout, "%-*s  ", lspname, spname[i]);
	    }

	  for (h = igroup * lline, lt = 0, igap = 0; lt < lline && h < l;
	       h++, lt++)
	    {
	      hp = (pose ? pose[h] : h);
		...
		{
		  fprintf (fout, " %s", CODONs[(int) z[i][hp]]);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
		  continue;
		}
...
void
strcase (char *str, int direction)
{
...
FILE *
gfopen (const char *filename, const char *mode)	//CMV added const two times
{
  FILE *fp;
  char tmp_filename[1024];	//CMV New variable
  ...
  if (fp == NULL)
      ...
      if (!strchr (mode, 'r'))
	{
      ...
      scanf ("%s", tmp_filename);	//CMV New variable for fopen

      if ((fp = (FILE *) fopen (tmp_filename, mode)) != NULL)	//CMV tmp_filename introduced
	{
	  return (fp);
	}
...
int
appendfile (FILE * fout, const char *filename)	//CMV filename made const
{
...
void
error2 (const char *message)	//CMV const for message
{
  printf ("\nError: %s.\n", message);
  exit (-1);
}
...
//CMV inlined
int
indexing (double x[], int n, int index[], int descending, int space[])
{
...
void
f_and_x_fromx (double x[], double f[], int n, int LastItem)
{
...
SetSeed (unsigned int seed)
{
  ...
  z_rndu = seed;
  w_rndu = abs ((int) seed);	//CMV removed ambiguity in which abs() to call
}
...
rndu (void)
{
  ...
  if (z_rndu == 0 || z_rndu == 4294967295UL)	//CHHS Added "UL" to signify unsigned long constant value
    {
      z_rndu = 13;
    }
...
QuantileNormal (double prob)
{
   ...
   */
  double a0 = -.322232431088, a1 = -1, a2 = -.342242088547, a3 =
    -.0204231210245;
  ...
  if (p1 < 1e-20)
    ...
    {
      y = sqrt (log (1 / (p1 * p1)));
      z = y + ((((y * a4 + a3) * y + a2) * y + a1) * y +
	       a0) / ((((y * b4 + b3) * y + b2) * y + b1) * y + b0);
    }
	 ...
	 (x + 3.98064794e-4 + 1.98615381364 /
	  (x - 0.151679116635 + 5.29330324926 /
	   (x + 4.8385912808 - 15.1508972451 /
	    (x + 0.742380924027 + 30.789933034 / (x + 3.99019417011))))));
    }
...
double
IncompleteGamma (double x, double alpha, double ln_gamma_alpha)
{
  ...
  double factor, gin = 0, rn = 0, a = 0, b = 0, an = 0, dif = 0, term =
    0, pn[6];
...
static double
QuantileChi2 (double prob, double v)
{
  ...
  double xx, c, ch, a = 0, q = 0, p1 = 0, p2 = 0, t = 0, x = 0, b =
    0, s1, s2, s3, s4, s5, s6;
...
int
DiscreteGamma (double freqK[], double rK[], double alpha, double beta,
...
int
AutodGamma (double M[], double freqK[], double rK[], double *rho1,
...
static int GaussLegendreRule (double **x, double **w, int npoints);

static double
LBinormal (double h, double k, double r)
{
  ...
  double shk, h0 = h, k0 = k, sk, L = 0, t[2], hk2, y, a =
    0, b, c, d, bs, as, rs, smallr = 1e-10;
  ...
  if (fabs (r) < 0.925)		/* equation 3 */
      ...
      if (fabs (r) < 1)
	{
	  ...
	  if (shk > -500)
	    {
	      L -= exp (-shk / 2) * sqrt (2 * Pi) * CDFNormal (-b / a) * b *
		(1 - c * bs * (1 - d * bs / 5) / 3);
	    }
  ...
  if (L < -1e-12)
    {
      printf ("L = %.9g very negative.  Let me know please.\n", L);
    }
...
/// CDF for bivariate normal
///
static double
CDFBinormal (double h, double k, double r)
{
  return LBinormal (-h, -k, r);
}

double
CDFBeta (double x, double pin, double qin, double lnbeta)
{
...
double
QuantileBeta (double prob, double p, double q, double lnbeta)
{
  ...
  if (prob < 0 || prob > 1 || p < 0 || q < 0)
    {
      error2 ("out of range in QuantileBeta");
    }
...
#if PAML_RELEASE == 0
  //if (!PAML_RELEASE) //CMV Used preprocessor instead of if
...
double
Quantile (double (*cdf) (double x, double par[]),
...
static int
GaussLegendreRule (double **x, double **w, int npoints)
{
  ...
  static double x32[] = { 0.048307665687738316234812570441,
  ...
  static double w32[] = { 0.0965400885147278005667648300636,
    0.0956387200792748594190820022041,
    0.0938443990808045656391802376681,
    0.0911738786957638847128685771116,
    0.0876520930044038111427714627518,
    0.0833119242269467552221990746043,
    0.0781938957870703064717409188283,
    0.0723457941088485062253993564785,
    0.0658222227763618468376500637069,
    0.0586840934785355471452836373002,
    0.0509980592623761761961632446895,
    0.0428358980222266806568786466061,
    0.0342738629130214331026877322524,
    0.0253920653092620594557525897892,
    0.0162743947309056706051705622064,
    0.0070186100094700966004070637389
  };

  static double x64[] = { 0.024350292663424432508955842854,
    0.072993121787799039449542941940,
  ...
  static double w64[] = { 0.0486909570091397203833653907347,
    0.0485754674415034269347990667840,
    0.0483447622348029571697695271580,
    0.0479993885964583077281261798713,
    0.0475401657148303086622822069442,
    0.0469681828162100173253262857546,
    0.0462847965813144172959532492323,
    0.0454916279274181444797709969713,
    0.0445905581637565630601347100309,
    0.0435837245293234533768278609737,
    0.0424735151236535890073397679088,
    0.0412625632426235286101562974736,
    0.0399537411327203413866569261283,
    0.0385501531786156291289624969468,
    0.0370551285402400460404151018096,
    0.0354722132568823838106931467152,
    0.0338051618371416093915654821107,
    0.0320579283548515535854675043479,
    0.0302346570724024788679740598195,
    0.0283396726142594832275113052002,
    0.0263774697150546586716917926252,
    0.0243527025687108733381775504091,
    0.0222701738083832541592983303842,
    0.0201348231535302093723403167285,
    0.0179517157756973430850453020011,
    0.0157260304760247193219659952975,
    0.0134630478967186425980607666860,
    0.0111681394601311288185904930192,
    0.0088467598263639477230309146597,
    0.0065044579689783628561173604000,
  ...
  static double w128[] = {
    0.0244461801962625182113259, 0.0244315690978500450548486,
  ...
  static double x256[] = {
    0.0061239123751895295011702, 0.0183708184788136651179263,
  ...
  static double w256[] = {
    0.0122476716402897559040703, 0.0122458343697479201424639,
  ...
  static double x512[] = {
    0.0030649621851593961529232, 0.0091947713864329108047442,
  ...
  static double w512[] = {
    0.0061299051754057857591564, 0.0061296748380364986664278,
  ...
  static double w1024[] = {
    0.0030664603092439082115513, 0.0030664314747171934849726,
...
void
rainbowRGB (double temperature, int *R, int *G, int *B)
{
  ...
  if (T > maxT)
    {
      error2 ("temperature rescaling needed.");
    }

  *R = (int) fabs (-5157.3 * T * T * T * T + 9681.4 * T * T * T -
		   5491.9 * T * T + 1137.7 * T + 6.2168);
...
void
GetIndexTernary (int *ix, int *iy, double *x, double *y, int itriangle, int K)
{
...
int
matby (double a[], double b[], double c[], int n, int m, int k)
...
int
matIout (FILE * fout, int x[], int n, int m)
{
  int i, j;
...
int
matout (FILE * fout, double x[], int n, int m)
{
  int i, j;
...
int
matout2 (FILE * fout, double x[], int n, int m, int wid, int deci)
{
  int i, j;
...
int
matinv (double x[], int n, int m, double space[])
{
  /* x[n*m]  ... m>=n
...
int
matexp (double Q[], double t, int n, int TimeSquare, double space[])
{
  ...
  if (TimeSquare < 2 || TimeSquare > 31)
    {
      error2 ("TimeSquare not good");
    }
...
int
splitline (char line[], int fields[])
{
...
static int
H_end (double x0[], double x1[], double f0, double f1, double e1, double e2,
       int n)
...
int
Hessian (int n, double x[], double f0, double g[], double H[],
...
static int
jacobi_gradient (double x[], double J[],
		 ...
		 double space[], int nx, int ny)
{
   ...
   */
  int i, j;
  double *x0 = space, *x1 = space + nx, *y0 = x1 + nx, *y1 =
    y0 + ny, eh0 = 1.0e-4, eh;
...
int
nls2 (FILE * fout, double *sx, double *x0, int nx,
      int (*fun) (double x[], double y[], int nx, int ny),
      ...
      int (*testx) (double x[], int nx), int ny, double e)
{
  ...
  if ((space = (double *) malloc (sspace)) == NULL)
    {
      error2 ("oom in nls2");
    }
  ...
  FOR (ii, maxround)
  {
	...
	(*jacobi) (x0, J, n, ny);
      ...
      {
	jacobi_gradient (x0, J, fun, space_J, n, ny);
      }
...
double
bound (int nx, double x0[], double p[], double x[],
...
double
LineSearch (double (*fun) (double x), double *f, double *x0,
	    double xb[2], double step, double e)
{
  ...
  for (ii = 0; ii < maxround; ii++)
      ...
      if (a1 > a2 + 1e-99 || a3 < a2 - 1e-99 || f2 > f1 + 1e-99 || f2 > f3 + 1e-99)	/* for linux */
	{
      ...
      if (fabs (a4) > 1e-100)
	a4 = ((a2 * a2 - a3 * a3) * f1 + (a3 * a3 - a1 * a1) * f2 +
	      (a1 * a1 - a2 * a2) * f3) / (2 * a4);

      if (a4 > a3 || a4 < a1)
	{
	  a4 = (a1 + a2) / 2;	/* out of range */
	}
      else
	{
	  percentUse++;
      ...
      if (fabs (f2 - f4) * (1 + fabs (f2)) <= e
	  && fabs (a2 - a4) * (1 + fabs (a2)) <= e)
	{
	  break;
	}
...
static double
fun_LineSearch (double t, double (*fun) (double x[], int n), double x0[],
...
static double
LineSearch2 (double (*fun) (double x[], int n), double *f, double x0[],
	     double p[], double step, double limit, double e, double space[],
	     int n)
{
  /* linear search using quadratic interpolation
  ...
  if (noisy > 2)
    printf ("\n%3d h-m-p %7.4f %6.4f %8.4f ", Iround + 1, step, limit,
	    norm (p, n));
  ...
  f1 = f0 = *f;
  a2 = a0 + step;
  f2 = fun_LineSearch (a2, fun, x0, p, x, n);
  ...
  if (f2 > f1)			/* reduce step length so the algorithm is decreasing */
      ...
      for (;;)
	{
	  ...
	  if (noisy > 2)	//CMV added correct #if
	    {
	      printf ("-");
	      nsymb++;
	    }
      ...
      for (;;)
	{
	  ...
	  a3 = a0 + step;
	  f3 = fun_LineSearch (a3, fun, x0, p, x, n);
	  ...
	  if (step >= limit)
		...
		for (; nsymb < 5; nsymb++)
	      ...
	      if (noisy > 2)
		printf (" %12.6f%3c %6.4f %5d", *f = f3, 'm', a3, NFunCall);
  ...
  for (ii = 0; ii < maxround; ii++)
      ...
      if (fabs (f2 - f4) < e * (1 + fabs (f2)))
	{
      ...
      if (noisy > 2 && ((a4 < a2 && f4 > f1) || (a4 > a2 && f4 > f3)))	//CMV added correct #if
	{
      ...
      if (a4 <= a2)		/* fig 2.2.10 */
	{
	  if (a2 - a4 > smallgapa * (a2 - a1))
		...
		{
		  a5 = (a2 + a3) / 2;
		  f5 = fun_LineSearch (a5, fun, x0, p, x, n);
	      ...
	      else
		{
		  ...
		  if (f5 >= f4)
		    ...
		    {
		      a6 = (a1 + a5) / 2;
		      f6 = fun_LineSearch (a6, fun, x0, p, x, n);
  ...
  if (noisy > 2)
    {
      printf (" %12.6f%3d %6.4f %5d", *f, ii, a4, NFunCall);
    }
...
static int
gradientB (int n, double x[], double f0, double g[],
...
int
ming2 (FILE * fout, double *f, double (*fun) (double x[], int n),
       ...
       double x[], double xb[][2], double space[], double e, int n)
{
   ...
   */
  int i, j, i1, i2, it, maxround = 10000, fail = 0, *xmark, *ix, nfree;	//CHHS maxround is the number of maximal optimization steps
  ...
  if (noisy > 2 && nfree < n && n < 50)
    {
      FPN (F0);
      FOR (j, n) printf (" %9.6f", x[j]);
      FPN (F0);
      FOR (j, n) printf (" %9.5f", xb[j][0]);
      FPN (F0);
      FOR (j, n) printf (" %9.5f", xb[j][1]);
      FPN (F0);

      if (nfree < n && noisy >= 3)
	{
	  printf ("warning: ming2, %d paras at boundary.", n - nfree);
	}
    }
  ...
  if (dfun)
    ...
    {
      gradientB (n, x0, f0, g0, fun, tv, xmark);
    }
  ...
  for (Iround = 0; Iround < maxround; Iround++)
	...
	FOR (j, nfree) p[ix[i]] -= H[i * nfree + j] * g0[ix[j]];
      ...
      else
	{
	  ...
	  if (Iround == 0 || SIZEp < sizep0
	      || (SIZEp < .001 && sizep0 < .001))
	    {
	      goodtimes++;
	    }
	  else
	...
	{
	  xtoy (H, C, nfree * nfree);
	  FOR (i1, nfree) FOR (i2, nfree) H[i1 * (nfree + 1) + i2] =
	    C[i1 * nfree + i2];
	  FOR (i1, nfree + 1) H[i1 * (nfree + 1) + nfree] =
	    H[nfree * (nfree + 1) + i1] = 0;
      ...
      if (fabs (v) < small)
	{
      ...
      FOR (i, nfree) FOR (j, nfree)
	H[i * nfree + j] +=
	((1 + w / v) * s[i] * s[j] - z[i] * s[j] - s[i] * z[j]) / v;
#endif
...
void
SaveToOctave (double *CVariable, char *OctaveVariable, FILE * FilePointer, int Rows, int Columns)	//CHHS
{
//CHHS This functions saves a C matrix / vector / scalar to file. It does not open or close files, this should be done before invoking this function. It can be invoked multiple times to store more than one object in the same file.
  ...
  fprintf (FilePointer, "# name: %s\n", OctaveVariable);	//CHHS We expect the user to use possible values 
