...
static void printtree (int timebranches);
...
hasbase (char *str)
{
  char *p = str;		//CMV no change
  const char *eqdel = ".-?";	//CMV const added
...
#if (defined MCMCTREE)
static void RemoveEmptySequences (void);
#endif
static int
GetSeqFileType (FILE * fseq, int *format)
{
  ...
  char fastastarter = '>';
  char line[1000], *p;		//CMV These are really writeable, the rest is const
  const char *paupstart = "begin data", *paupend = "matrix";
  const char *ntax = "ntax", *nchar = "nchar";

  while (isspace (ch = fgetc (fseq)))
    {
    }				//CMV make clean the empty body
  ...
  if (ch == fastastarter)
	...
	{
	  error2
	    ("The seq file appears to be in fasta format, but not aligned?");
	}
    }
...
static int printPatterns (FILE * fout);
static int RemoveIndel (void);
static void EncodeSeqs (void);
static void printSeqsMgenes (void);
static int BootstrapSeq (const char *seqf);

int
ReadSeq (FILE * fout, FILE * fseq, int cleandata)
{
  ...
  int h /* , b[3] = { 0 } */ ;	//CMV never used
  char *pch =
    ((com.seqtype <= 1
      || com.seqtype == CODON2AAseq) ? BASEs : (com.seqtype ==
						2 ? AAs : BINs));
  // char str[4] = "   "; //CMV set but unused
  double lst;

  /* str[0] = 0; *///CMV set but unused
  h = -1;
  /* b[0] = -1; *//* avoid warning *///CMV never used
  com.readpattern = 0;

  if (com.seqtype == 4)
    {
      error2 ("seqtype==BINs, check with author");
    }
  ...
  if (com.ns > NS)
    {
      error2 ("too many sequences.. raise NS?");
    }

  if (com.ls % n31 != 0)
    {
      printf ("\n%d nucleotides, not a multiple of 3!", com.ls);
      exit (-1);
    }
  ...
  for (j = 0; j < com.ns; j++)
      ...
      if (com.spname[j])
	{
      ...
      for (i = 0; i < lspname + 1; i++)
	{
	  com.spname[j][i] = 0;
	}

      if ((com.z[j] =
	   (char *) realloc (com.z[j], com.ls * sizeof (char))) == NULL)
	{
	  error2 ("oom z");
	}
    }
  ...
  if (!com.readpattern)
    {
      if ((com.pose =
	   (int *) realloc (com.pose, com.ls / n31 * sizeof (int))) == NULL)
	{
	  error2 ("oom pose");
	}

      for (j = 0; j < com.ls / n31; j++)
	{
	  com.pose[j] = 0;	/* gene #1, default */
	}
    }
  else
    {
  ...
  if (strchr (line, 'C'))	/* protein-coding DNA sequences */
      ...
      if (com.seqtype == 0)
	{
	  ...
	  if (com.readpattern)
	    error2
	      ("partterns for coding sequences (G C P) not implemented.");
	  else
	    for (i = 0; i < com.ls; i++)
	      {
		com.pose[i] = (char) (i % 3);
  ...
  for (j = 0; j < noptline; j++)
	...
	case ('G'):
	  ...
	  if (basecoding)
	    error2
	      ("Error in sequence data file: incorrect option format, use GC?\n");

	  if (fscanf (fseq, "%d", &com.ngene) != 1)
	    {
	      error2 ("expecting #gene here..");
	    }
	  ...
	  if (!blankline (line))	/* #sites in genes on the 2nd line */
	      ...
	      for (i = 0, p = line; i < com.ngene; i++)
		{
	      ...
	      for (; i < com.ngene; i++)
		if (fscanf (fseq, "%d", &com.lgene[i]) != 1)
		  {
		    error2 ("EOF at lgene");
		  }
	  ...
	  else			/* site marks on later line(s)  */
	    {
	      if (com.readpattern)
		error2
		  ("option PG: use number of patterns in each gene and not site marks");
	      ...
	      for (k = 0; k < com.ls / n31;)
		{
	      ...
	      if (!fgets (line, lline, fseq))
		{
		  error2 ("sequence file, gene marks");
		}
	    }
...
readseq:
  ...
  if (Sequential)		/* sequential */
    {
      if (noisy)
	{
	  printf ("Reading sequences, sequential format..\n");
	}
      ...
      for (j = 0; j < com.ns; j++)
	{
	  ...
	  if (blankline (line))
		...
		{
		  printf
		    ("error in sequence data file: empty line (seq %d)\n",
		     j + 1);
		  exit (-1);
		}
	    }
	  ...
	  for (k = 0; k < com.ls; p++)
	      ...
	      while (*p == '\n' || *p == '\0')
		{
	      ...
	      *p = (char) toupper (*p);
	      ...
	      if ((com.seqtype == BASEseq || com.seqtype == CODONseq)
		  && *p == 'U')
		{
		  *p = 'T';
		}
		...
		{
		  if (j == 0)
		    error2 ("Error in sequence data file: . in 1st seq.?");
	      ...
	      else if (isalpha (*p))
		{
		  printf
		     ...
		     *p, k + 1, j + 1);
		  puts
		    ("Perhaps you did not separate the sequence from its name by >2 spaces?");
		  exit (0);
		}
	      else if (*p == (char) EOF)
		{
  ...
  else				/* interlaved */
    {
      if (noisy)
	{
	  printf ("Reading sequences, interlaved format..\n");
	}
      ...
      for (igroup = 0;; igroup++)
	{
	  ...
	  FOR (j, com.ns)
	  {
		...
		if (j)
		  {
		    printf ("\n%d, seq %d group %d", lt[j] + 1, j + 1,
			    igroup + 1);
		    error2 ("empty line.");
		  }
		else if (PopEmptyLines (fseq, lline, line) == -1)
		  {
		    printf ("\n%d, seq %d group %d", lt[j] + 1, j + 1,
			    igroup + 1);
		    error2 ("EOF?");
  ...
  if (!miss)
      ...
      if (fout)
	{
	  ...
	  fprintf (fout, " %6d %6d\n", com.ns, com.ls);
	  printsma (fout, com.spname, com.z, com.ns, com.ls, com.ls, gap,
		    com.seqtype, 0, 0, NULL);
	}
  ...
  if (com.seqtype == CODON2AAseq)
      ...
      if (fout)
	{
	  ...
	  printsma (fout, com.spname, com.z, com.ns, com.ls, com.ls, 10,
		    com.seqtype, 0, 0, NULL);
	}
    }
  ...
  if (com.ngene == 1 && com.Mgene == 1)
    {
      com.Mgene = 0;
    }

  if (com.ngene > 1 && com.Mgene == 1 && com.verbose)
    {
      printSeqsMgenes ();
    }
...
#endif
static int printsmaCodon (FILE * fout, char *z[], int ns, int ls, int lline,
			  int simple);

static int
printPatterns (FILE * fout)
{
  int j, h, n31 = (com.seqtype == CODONseq
		   || com.seqtype == CODON2AAseq ? 3 : 1);
  int gap = (n31 == 3 ? 3 : 10);	//CHHS int nchar=(com.seqtype==AAseq?20:4) not used and removed
  ...
  if (com.seqtype == 1 && com.cleandata)
    ...
    printsma (fout, com.spname, com.z, com.ns, com.npatt, com.npatt,
	      gap, com.seqtype, 1, 0, NULL);
...
EncodeSeqs (void)
{
   ...
   */
  int n = com.ncode, nA, is, h, i, j, k, ic, ch, b[3];	//CHHS int indel=0 not used and removed
  char *pchar =
    ((com.seqtype == 0
      || com.seqtype == 1) ? BASEs : (com.seqtype == 2 ? AAs : BINs));
  char c[4] = "   ";		//CHHS unsigned char str[4]="   " not used and removed; variable c lost "unsigned" property to avoid compiler warning (should have no effect)
  //CMV better initialization
      ...
      for (is = 0; is < com.ns; is++)
	{
	  for (h = 0; h < com.npatt; h++)
	      ...
	      if (k < 0)
		{
	      ...
	      com.z[is][h] = (char) k;	//CMV to remove VC warning
	    }
	}
    }
      ...
      for (j = 0, nA = n; j < com.ns; j++)
	{
	  for (h = 0; h < com.npatt; h++)
	      ...
	      for (k = 0; k < 3; k++)
		{
		  ...
		  if (b[k] < 0)
		    printf ("strange nucleotide %c in seq %d\n", c[k], j + 1);
		}
	      ...
	      if (k == nA)
		{
		  if (++nA > 256)
		    error2
		      ("too many ambiguity codons in the data.  Contact author");
	  ...
	  com.z[j] = (char *) realloc (com.z[j], com.npatt);	//CHHS "unsigned" in cast dropped to unify data types of pointers
	}
...
SetMapAmbiguity (void)
{
  ...
  for (j = 0; j < n; j++)
    {
      nChara[j] = 1;
      CharaMap[j][0] = (char) j;	//CMV added cast to remove a warning on windows
    }
  ...
  if (com.seqtype == 0 || com.seqtype == 2)
      ...
      for (j = n, pchar += n; *pchar; j++, pchar++)
	{
	  if (com.seqtype == 0)
	    {
	      nChara[j] = (char) strlen (EquateBASE[j]);	//CMV added cast to remove a warning on windows

	      for (i = 0; i < nChara[j]; i++)
		CharaMap[j][i] =
		  (char) (strchr (BASEs, EquateBASE[j][i]) - BASEs);
	    }
	  else
	    {
	      nChara[j] = (char) n;	//CMV added cast to remove a warning on windows

	      for (i = 0; i < n; i++)
		{
		  CharaMap[j][i] = (char) i;	//CMV added cast to remove a warning on windows
		}
	    }
	}
    }
		...
		for (i2 = 0; i2 < nb[2]; i2++)
		  {
		    ...
		    if (GeneticCode[com.icode][ic] != -1)
		      {
			CharaMap[j][(int) nChara[j]++] = FROM64[ic];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
		      }
		  }
	    }
...
PatternWeight (void)
{
  ...
  //CHHS   int gap = (com.seqtype==CODONseq ? 3 : 10); //CHHS not used and removed
  ...
  if (noisy)
    {
      printf ("Counting site patterns.. %s\n", printtime (timestr));
    }
      ...
      for (h = 0; h < com.ls; h++)
	{
	  ...
	  if (!same)
	      ...
	      if (com.npatt > maxnpatt)
		{
	      ...
	      if (l > ip)
		{
		  ip++;		/* last comparison in bsearch had k > 0. */
		}

	      /* Insert new pattern at ip.  This is the expensive step. */

	      if (ip < com.npatt)
		memmove (p2s + ip + 1, p2s + ip,
			 (com.npatt - ip) * sizeof (int));
	  ...
	  if (debug)
	    {
	      printf (": %3d (%c ilu %3d%3d%3d) ", com.npatt, DS[same],
		      ip, l, u);
	  ...
	  if (noisy && ((h + 1) % 10000 == 0 || h + 1 == com.ls))
	    printf ("\r%12d patterns at %8d / %8d sites (%.1f%%), %s",
		    com.npatt, h + 1, com.ls, (h + 1.) * 100 / com.ls,
		    printtime (timestr));
...
static void
Chi2FreqHomo (double f[], int ns, int nc, double X2G[2])
{
...
InitializeBaseAA (FILE * fout)
{
   ...
   */
  char *pch =
    (com.seqtype == 0 ? BASEs : (com.seqtype == 2 ? AAs : BINs)),
    indel[] = "-?";
  ...
  for (ig = 0, zero (com.pi, nc); ig < com.ngene; ig++)
    {
      if (com.ngene > 1)
	fprintf (fout, "\n\nGene %2d (len %4d)", ig + 1,
		 com.lgene[ig] - (ig == 0 ? 0 : com.lgene[ig - 1]));

      fprintf (fout, "\n%*s", wname, "");

      for (k = 0; k < nc; k++)
	{
	  fprintf (fout, "%7c", pch[k]);
	}
      ...
      for (js = 0; js < com.ns; js++)
	{
	  ...
	  for (irf = 0; irf < nrf; irf++)
	      ...
	      else
		{
	      ...
	      if (com.cleandata || com.cleandata
		  || (t = distance (com.pi, pi0, nc)) < 1e-8)
		{
		  break;
		}
	  ...
	  for (k = 0; k < nc; k++)
	    {
	      fprintf (fout, "%7.4f", com.pi[k]);
	    }

	  for (k = 0; k < nc; k++)
	    {
	      com.piG[ig][k] += com.pi[k] / com.ns;
	    }
      ...
      if (com.ngene > 1)
	{
	  ...
	  for (k = 0; k < nc; k++)
	    {
	      fprintf (fout, "%7.4f", com.piG[ig][k]);
	    }
	}

      Chi2FreqHomo (pisg, com.ns, nc, X2G);

      fprintf (fout, "\n\nHomogeneity statistic: X2 = %.5f G = %.5f ",
	       X2G[0], X2G[1]);
  ...
  if (com.cleandata)
	...
	{
	  t = (ig ==
	       0 ? com.lgene[0] : com.lgene[ig] - com.lgene[ig -
							    1]) /
	    (double) com.ls;

	  for (k = 0; k < nc; k++)
	    {
	      com.pi[k] += com.piG[ig][k] * t;
	    }
	}
    }
  else
      ...
      for (ig = 0; ig < com.ngene; ig++)
	{
	  ...
	  for (irf = 0; irf < nrf; irf++)	/* com.piG[] */
	      ...
	      if (t < 1e-10)
		{
	      ...
	      if (distance (com.piG[ig], pi0, nc) < 1e-8)
		{
		  break;
		}
      ...
      for (irf = 0; irf < nrf; irf++)	/* com.pi[] */
	{
	  ...
	  if (distance (com.pi, pi0, nc) < 1e-8)
	    {
	      break;
	    }
  ...
  if (com.seqtype == BASEseq && com.model == 5)	/* T92 model */
    {
      ...
      for (ig = 0; ig < com.ngene; ig++)
	{
	  com.piG[ig][0] = com.piG[ig][2] =
	    (com.piG[ig][0] + com.piG[ig][2]) / 2;
	  com.piG[ig][1] = com.piG[ig][3] =
	    (com.piG[ig][1] + com.piG[ig][3]) / 2;
	}
    }
  ...
  if (com.cleandata && com.ngene == 1)
    {
      for (h = 0, lmax = -(double) com.ls * log ((double) com.ls);
	   h < com.npatt; h++)
	if (com.fpatt[h] > 1)
	  {
...
void
AddFreqSeqGene (int js, int ig, double pi0[], double pi[])
{
   ...
   */
  //CHHS   char *pch=(com.seqtype==0?BASEs:(com.seqtype==2?AAs:BINs)); //CHHS Variable not used and removed
  ...
  if (com.cleandata)
	...
	{
	  pi[(int) com.z[js][h]] += com.fpatt[h];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	}
    }
  else
    {
	  ...
	  if (b < nc)
	      ...
	      if (com.seqtype == BASEseq)
		{
		  for (k = 0, t = 0; k < nChara[b]; k++)
		    {
		      t += pi0[(int) CharaMap[b][k]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
		    }

		  for (k = 0; k < nChara[b]; k++)
		    {
		      pi[(int) CharaMap[b][k]] += pi0[(int) CharaMap[b][k]] / t * com.fpatt[h];	//CHHS Casting array subscript twice to int avoid compiler warning with gcc (should have no effect)
		    }
		}
	      else if (com.seqtype == AAseq)	/* unrecognized AAs are treated as "?". */
		for (k = 0; k < nc; k++)
		  {
		    pi[k] += pi0[k] * com.fpatt[h];
		  }
	    }
	}
    }
...
int
PatternWeightJC69like (FILE * fout)
{
  ...
  if (noisy)
    {
      printf ("Counting site patterns again, for JC69.\n");
    }
      ...
      else			/* recode only if there are no non-gap ambiguity characters */
	{
	  ...
	  for (j = 0, recode = 1; j < com.ns; j++)
	      ...
	      if (zh[j] < com.ncode)
		{
	      ...
	      if (nChara[(int) zh[j]] == com.ncode)	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
		{
		  zh[j] = gap;
		  continue;
		}
...
static int
print1seq (FILE * fout, char *z, int ls, int pose[])
{
   ...
   */
  int h, hp, gap = 10;
  char *pch = (com.seqtype == 0 ? BASEs : (com.seqtype == 2 ? AAs : BINs));	//CHHS char str[4]="" unused and removed
  ...
  for (h = 0; h < ls; h++)
	...
	{
	  fprintf (fout, "%c", pch[(int) z[hp]]);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	...
	{
	  fprintf (fout, "%s ", CODONs[(int) z[hp]]);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	}
    }
...
double
SeqDivergence (double x[], int model, double alpha, double *kappa)
{
  /* alpha=0 if no gamma
  ...
  P1 = x[0 * 4 + 1] + x[1 * 4 + 0];
  P2 = x[2 * 4 + 3] + x[3 * 4 + 2];
  Q = x[0 * 4 + 2] + x[0 * 4 + 3] + x[1 * 4 + 2] + x[1 * 4 + 3] +
    x[2 * 4 + 0] + x[2 * 4 + 1] + x[3 * 4 + 0] + x[3 * 4 + 1];
...
double
DistanceIJ (int is, int js, int model, double alpha, double *kappa)
{
...
int
DistanceMatNuc (FILE * fout, FILE * f2base, int model, double alpha)
{
  ...
  for (is = 0; is < com.ns; is++)
      ...
      if (fout)
	{
      ...
      if (f2base)
	{
	  fprintf (f2base, "%-15s   ", com.spname[is]);
	}
...
#ifdef BASEMLG
extern int CijkIs0[];		//CHHS Not moved to global.c and global.h, as not part of codeml
#endif

extern int nR;			//CHHS Not moved to global.c and global.h, as not part of codeml
extern double Cijk[], Root[];	//CHHS Not moved to global.c and global.h, as not part of codeml

int
RootTN93 (int model, double kappa1, double kappa2, double pi[],
...
int
EigenTN93 (int model, double kappa1, double kappa2, double pi[],
	   int *nR, double Root[], double Cijk[])
{
  ...
  *nR = nr = 2 + (model == K80 || model >= F84) + (model >= HKY85);
  ...
  FOR (i, 4) FOR (j, 4)
  {
	...
	for (k = 1; k < 4; k++)
	  Cijk[i * 4 * nr + j * nr + k] = U[i * 4 + k] * V[k * 4 + j];
...
static int
printsmaCodon (FILE * fout, char *z[], int ns, int ls, int lline, int simple)
{
  ...
  for (ig = 0, FPN (fout); ig < ngroup; ig++)
      ...
      for (is = 0; is < ns; is++)
	{
	  ...
	  for (il = ig * lline, pz = z[is] + il; lt < lline && il < ls;
	       il++, lt++, pz++)
	    {
	      b = *pz;
	      b = FROM61[b];
...
int
DistanceMatNG86 (FILE * fout, FILE * fds, FILE * fdn, FILE * ft, double alpha)
{
   ...
   */
  char *codon[2];
  int is, js, k, h, wname = 20, status = 0, ndiff, nsd[4];	//CHHS int i unused and removed
  double ns, na, nst, nat, S, N, St, Nt, dS, dN, dN_dS, y, bigD = 3, lst;
  //double SEds, SEdn, p; //CMV never used
  ...
  if (noisy > 1 && com.ns > 10)
    {
      puts ("NG distances for seqs.:");
    }
      ...
      for (js = 0; js < is; js++)
	{
	  ...
	  for (h = 0, lst = 0, nst = nat = S = N = 0; h < com.npatt; h++)
	      ...
	      if (com.z[is][h] >= com.ncode || com.z[js][h] >= com.ncode)
		{
	      ...
	      codon[0] = CODONs[(int) com.z[is][h]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	      codon[1] = CODONs[(int) com.z[js][h]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	      lst += com.fpatt[h];
	      ndiff =
		difcodonNG (codon[0], codon[1], &St, &Nt, &ns, &na, 0,
			    com.icode);
...
#if 0
//CMV the following variables where never used
	  if (alpha == 0 && dS < bigD)
	    {
	      p = nst / S;
	      /* SEds = sqrt(9 * p * (1 - p) / (square(3 - 4 * p) * S)); *///CMV never used
	    }

	  if (alpha == 0 && dN < bigD)
	    {
	      p = nat / N;
	      /* SEdn = sqrt(9 * p * (1 - p) / (square(3 - 4 * p) * N)); *///CMV never used
	    }
#endif
	}			/* for(js) */
  ...
  if (status)
    {
      fprintf (fout, "NOTE: -1 means that NG86 is inapplicable.\n");
    }
...
static int
fun_LS (double x[], double diff[], int np, int npair)
{
  int i, j, aa, it = -np;
  double dexp;

  if (SetBranch (x) && noisy > 2)
    {
      puts ("branch len.");
    }

  if (npair != com.ns * (com.ns - 1) / 2)
    {
      error2 ("# seq pairs err.");
    }
...
static int
LSDistance (double *ss, double x[], int (*testx) (double x[], int np))
{
  ...
  i = nls2 ((com.ntime > 20 && noisy >= 3 ? F0 : NULL),
	    ss, x, com.ntime, fun_LS, NULL, testx,
	    com.ns * (com.ns - 1) / 2, 1e-6);
...
static int
ReadTreeB (FILE * ftree, int popline)
{
  char line[255];
  ...
  for (j = 0; j < tree.nbranch; j++)
    {
      for (i = 0; i < 2; i++)
	{
	  if (fscanf (ftree, "%d", &tree.branches[j][i]) != 1)
	    {
	      state = -1;
	    }
      ...
      if (nodemark[tree.branches[j][0]] != 2)
	{
	  nodemark[tree.branches[j][0]] = 1;
	}
      ...
      if (tree.branches[j][0] < com.ns)
	{
      ...
      printf ("\nBranch #%3d: %3d -> %3d", j + 1, tree.branches[j][0] + 1,
	      tree.branches[j][1] + 1);
  ...
  if (popline)
    {
      fgets (line, 254, ftree);
    }

  for (i = 0, tree.root = -1; i < tree.nbranch; i++)
    if (nodemark[tree.branches[i][0]] != 2)
      {
	tree.root = tree.branches[i][0];
      }
...
int
GetTreeFileType (FILE * ftree, int *ntree, int *pauptree, int shortform)
{
  ...
  char line[32000];
  const char *paupstart = "begin tree", *paupend = "translate";	//CMV made const
  ...
  *pauptree = 0;
  ...
  if (k == 2)
    {
      if (i == com.ns)
	{
	  return (0);		/* old paml style */
	}
      else
	{
	  error2 ("Number of sequences different in tree and seq files.");
	}
    }
  else if (k == 1)
    {
  ...
  for (;;)
    {
      if (fgets (line, lline, ftree) == NULL)
	{
	  error2 ("tree err1: EOF");
	}
...
static void
DownTreeCladeLabel (int inode, int cLabel)
{
...
int
ReadTreeN (FILE * ftree, int *haslength, int *haslabel, int copyname,
	   int popline)
{
  ...
  if (com.ns <= 0)
    {
      error2 ("need to know ns before reading tree.");
    }
  ...
  *haslabel = 0;
  ...
  for (;;)
      ...
      else if (ch == '(')	/* left (  */
	{
	  ...
	  if (cfather >= 0)
		...
		{
		  printf
		    ("there are at least %d daughter nodes, raise MAXNSONS?",
		     nodes[cfather].nson);
		  exit (-1);
		}
	...
	{
	  *haslabel = 1;
	  fscanf (ftree, "%d", &CladeLabel[inodeb]);
	}
      else if (ch == '@' || ch == '=')
	{
  ...
  if (tree.nbranch > 2 * com.ns - 2)
    {
      ...
      puts ("too many branches in tree?");
    }
  ...
  if (com.clock > 1 || (com.seqtype == 1 && com.model >= 2))
	...
	for (i = 0; i < tree.nnode; i++)
	  if (i != tree.root && nodes[i].label == -1)
	    {
	      nodes[i].label = 0;
	    }
      ...
      for (i = 0, com.nbtype = 0; i < tree.nnode; i++)
	{
	  ...
	  if (j < 0 || j > tree.nbranch - 1)
	    error2
	      ("branch label in the tree (note labels start from 0 and are consecutive)");
	}
	...
	{
	  printf ("\n%d branch types are in tree. Stop if wrong.",
		  com.nbtype);
	}

#if(defined(CODEML))

      if (com.seqtype == 1 && com.NSsites == 2 && com.model == 3
	  && com.nbtype > NBTYPE)
	{
	  error2 ("nbtype too large.  Raise NBTYPE");
	}
      else if (com.seqtype == 1 && com.NSsites && com.model == 2
	       && com.nbtype != 2)
	{
...
//CMV duplicated int OutSubTreeN(FILE * fout, int inode, int spnames, int printopt,
//CMV duplicated                char *labelfmt);

static int
OutSubTreeN (FILE * fout, int inode, int spnames, int printopt,
	     const char *labelfmt)
{
  int i, dad = nodes[inode].father, nsib =
    (inode == tree.root ? 0 : nodes[dad].nson);
  ...
  if (nodes[inode].nson == 0)	/* inode is tip */
    {
      if (spnames)
	{
	  if (printopt & PrNodeNum)
	    {
	      fprintf (fout, "%d_", inode + 1);
	    }
  ...
  if ((printopt & PrBranch)
      && (inode != tree.root || nodes[inode].branch > 0))
    {
...
int
OutTreeN (FILE * fout, int spnames, int printopt)
{
  /* print the current tree.
     Can the block of print statements be moved inside the recursive function?
   */
  int i, intlabel = 1;
  const char *labelfmt[2] = { "'#%.5f'", "'#%.0f'" };

  if (printopt & PrLabel)
    {
      for (i = 0; i < tree.nnode; i++)
	if (nodes[i].label - (int) nodes[i].label != 0)
	  {
	    intlabel = 0;
	  }
    }
...
static int Nsonroot = -1;

int
PruneSubTreeN (int inode, int keep[])
{
  ...
  /* move down the root if the root has only one descendent */
      ...
      if (nodes[inode].nson == 1)
	{
	  ...
	  if (!com.clock && Nsonroot >= 3 && nodes[inode].nson == 2)
	    {
	      DeRoot ();
	    }
	}
...
static void
GetSubTreeN (int keep[], int space[])
{
   ...
   */
  int nsnew, i, j, k, nnode0 = tree.nnode, sumnumber =
    0, newnodeNO[2 * NS - 1];
  double *branch0;
  int debug = 0;
  ...
  if (sumnumber > nsnew)
      ...
      if (sumnumber != nsnew * (nsnew + 1) / 2)
	{
      ...
      if ((branch0 = (double *) malloc (nnode0 * sizeof (double))) == NULL)
	{
	  error2 ("oom#");
	}
      ...
      FOR (i, com.ns) if (keep[i])
	{
	  newnodeNO[i] = keep[i] - 1;
	}
      ...
      for (; i < nnode0; i++)
	{
      ...
      for (j = 0; j < tree.nbranch; j++)
	FOR (i, 2) tree.branches[j][i] = newnodeNO[tree.branches[j][i]];
  ...
  if (space)
    {
      memmove (space, newnodeNO, (com.ns * 2 - 1) * sizeof (int));
    }
}


static void
printtree (int timebranches)
{
  int i, j;
  ...
  if (timebranches)
    {
      printf ("%10s%10s%10s", "time", "branch", "label");
    }
  ...
  FOR (i, tree.nnode)
  {
    ...
    if (timebranches)
      printf (" %9.6f %9.6f %9.0f", nodes[i].age, nodes[i].branch,
	      nodes[i].label);
...
PointconPnodes (void)
{
   ...
   */
  size_t nintern = 0;
  int i;			//CMV it should match tree.branch signedness
...
int
SetxInitials (int np, double x[], double xb[][2])
{
...
double
GetBranchRate (int igene, int ibrate, double x[], int *ix)
{
  ...
  if (igene == 0 && ibrate == 0)
      ...
      if (igene == 0 && ibrate)
	{
      ...
      else if (igene && ibrate == 0)
	{
	  brate = rate00 * x[com.ntime + igene - 1];
	  k = -1;
	}
      else if (igene && ibrate)
	{
	  brate = x[k + ibrate - 1] * x[com.ntime + igene - 1];
	  k = -1;
	}
    }
  else if (com.clock == ClockCombined)
    {
      if (ibrate == 0 && igene)
	{
	  brate = x[k = com.ntime + igene - 1];
	}
      else
	{
	  brate = x[k += ibrate - 1 + igene * (com.nbtype - 1)];	/* ibrate>0 */
	}
    }
...
GetTipDate (void)
{
  ...
  if (ScaleTimes_TipDate == 0)
    {
      error2 ("All sequences of the same age?");
    }
...
SetBranch (double x[])
{
  ...
  if (com.clock == 0)
	...
	{
	  if (i != tree.root)
	    if ((nodes[i].branch = x[nodes[i].ibranch]) < small)
	      {
		status = -1;
	      }
	}
...
GetInitialsTimes (double x[])
{
  ...
  if (com.fix_blength == 2)
      ...
      for (i = 0; i < com.ntime; i++)
	{
      ...
      if (com.fix_blength == 0 && com.clock < 5 && ancestor
	  && com.ntime < 100)
	{
  ...
  if (com.clock < 5)
      ...
      else
	{
	  for (i = 0; i < tree.nnode; i++)
	    {
	      if (i != tree.root
		  && (j = (int) nodes[i].label + 1) > com.nbtype)
		{
		  com.nbtype = j;
	  ...
	  for (j = 0; j < com.nbtype; j++)
	    {
	      for (i = 0; i < tree.nnode; i++)
		if (i != tree.root && j == (int) nodes[i].label)
		  {
		    break;
		  }
	      ...
	      if (i == tree.nnode)
		printf
		  ("\nNot all branch labels (0, ..., %d) are found on tree?",
		   com.nbtype - 1);
	    }
	  ...
	  if (com.nbtype <= 1)
	    {
	      error2 ("use clock = 1 or add branch rate labels in tree");
	    }
  ...
  com.ntime =
    AbsoluteRate + (tree.nnode - com.ns - NFossils) + (com.nbtype - 1);
...
int
OutputTimesRates (FILE * fout, double x[], double var[])
{
  ...
  if (AbsoluteRate && com.clock < 5)
    {
      ...
      if (com.nbtype > 1)
	{
	  fprintf (fout, "Rates for branch groups\n");
	}
      ...
      for (i = 0; i < com.ngene; i++, FPN (fout))
	{
	  ...
	  for (j = 0; j < com.nbtype; j++)
		...
		{
		  if (k == -1)
		    error2 ("we are in trouble. k should not be -1 here.");

		  fprintf (fout, " +- %8.6f",
			   sqrt (var[k * com.np + k]) / scale);
		}
	    }
	}
    }
  else if (com.clock == 2)
    {
      ...
      for (k = tree.nnode - com.ns; k < com.ntime; k++)
	{
  ...
  if (TipDate)			/* DANGER! SE not printed if(TipDate && NFossil). */
      ...
      for (i = 0, k = 0; i < tree.nnode; i++, FPN (fout))
	{
	  ...
	  fprintf (fout, "Node %3d (Jeffnode %3d) Time %7.2f ", i + 1,
		   jeffnode, TipDate - nodes[i].age * scale);
      ...
      for (i = com.ns, k = 0; i < tree.nnode; i++, FPN (fout))
	{
	  ...
	  fprintf (fout, "Node %3d (Jeffnode %3d) Time %9.5f", i + 1,
		   tree.nnode - 1 + com.ns - i, nodes[i].age);

	  if (com.getSE && i >= com.ns && !nodes[i].fossil)
	    {
	      fprintf (fout, " +- %7.5f", sqrt (var[k * com.np + k]));

	      if (fabs (nodes[i].age - x[k]) > 1e-5)
		{
		  error2 ("node order wrong.");
		}
...
readx (double x[], int *fromfile)
{
  ...
  *fromfile = 0;
  ...
  if (finitials == NULL || (com.runmode > 0 && times > 1))
    {
      return (0);
    }
  ...
  if (com.runmode > 0 && com.seqtype == 1 && com.model)
    {
      error2 ("option or in.codeml");
    }
  ...
  *fromfile = 1;
  ...
  for (; i < npin; i++)
    if (fscanf (finitials, "%lf", &xin[i]) != 1)
      {
	break;
      }
...
static int
CollapsNode (int inode, double x[])
{
  ...
  if (inode == tree.root || inode < com.ns)
    {
      error2 ("err CollapsNode");
    }

  ibranch = nodes[inode].ibranch;
  ifather = nodes[inode].father;
  ...
  for (i = ibranch + 1; i < tree.nbranch; i++)
    for (j = 0; j < 2; j++)
      {
	tree.branches[i - 1][j] = tree.branches[i][j];
      }
  ...
  if (x)
    {
      if (com.clock)
	for (i = inode + 1; i < tree.nnode + 1; i++)
	  {
	    x[i - 1 - com.ns] = x[i - com.ns];
	  }
      else
	{
	  for (i = ibranch + 1; i < tree.nbranch + 1; i++)
	    {
	      x[i - 1] = x[i];
	    }
...
//void BranchPartition(char partition[], int parti2B[]); //CMV redundant

static char *PARTITION;

static void
DescentGroup (int inode)
{
  int i;
...
AddSpecies (int is, int ib)
{
  ...
  if (ib > tree.nbranch + 1 || (ib == tree.nbranch && !com.clock))
    {
      //return (-1);
      return;
    }
  ...
  if (ib == tree.nbranch && com.clock)
    {
      FOR (i, tree.nbranch) FOR (j, 2) if (tree.branches[i][j] >= is)
	{
	  tree.branches[i][j] += 2;
	}
...
static double TreeScore (double x[], double space[]);
static void StepwiseAdditionMP (double space[]);

int
Perturbation (FILE * fout, int initialMP, double space[])
{
  ...
  if (com.np > com.ntime)
    {
      ...
      for (i = com.ntime; i < com.np; i++)
	{
	  fprintf (fout, "%9.5f", x[i]);
	}
  ...
  fflush (fout);
  treebest.tree = tree;
  memcpy (treebest.nodes, nodes, sizetree);

  for (step = 0;; step++)
    {
      for (ineighb = 0, improve = 0;
	   ineighb < (tree.nbranch - com.ns) * 2; ineighb++)
	{
	  tree = treebest.tree;
	  memcpy (nodes, treebest.nodes, sizetree);
	  NeighborNNI (ineighb);
...
static double MPScoreStepwiseAddition (int is, double space[], int save);

static void
StepwiseAdditionMP (double space[])
{
  /* tree search by species addition.
   */
  //char *z0[NS]; //CMV never used
  ...
  if (noisy > 2)
    {
      printf ("\n%9ld bytes for MP (U0 & N0)\n", (long int) 2 * com.npatt * _mnnode * sizeof (int));	//CHHS cast to avoid warning in gcc 32 bit
    }

  if (_U0 == NULL || _step0 == NULL)
    {
      error2 ("oom U0&step0");
    }

  //FOR(i, ns0) z0[i] = com.z[i]; //CMV z0 never used
  tree.nbranch = tree.root = com.ns = 3;
  FOR (i, tree.nbranch)
  {
    tree.branches[i][0] = com.ns;
    tree.branches[i][1] = i;
  }
  BranchToNode ();
  FOR (h, com.npatt) FOR (i, com.ns)
  {
    _U0[h * _mnnode + i] = 1 << (com.z[i][h] - 1);
    _step0[h * _mnnode + i] = 0;
  }

  for (is = com.ns, tie = 0; is < ns0; is++)
    {
      treestar.tree = tree;
      memcpy (treestar.nodes, nodes, sizetree);

      for (j = 0; j < treestar.tree.nbranch; j++, com.ns--)
	{
	  tree = treestar.tree;
	  memcpy (nodes, treestar.nodes, sizetree);
	  com.ns++;
	  AddSpecies (is, j);
      ...
      if (noisy)
	{
	  printf ("\r  Added %d [%5.0f steps]", is + 1, -bestscore);
	  fflush (F0);
	}
    }
...
static double
MPScoreStepwiseAddition (int is, double space[], int save)
{
  ...
  for (h = 0, score = 0; h < com.npatt; h++, pU0 += _mnnode, pN0 += _mnnode)
	...
	{
	  if ((son2 = nodes[father].sons[0]) == ist)
	    {
	      son2 = nodes[father].sons[1];
	    }
      ...
      if (U3[0] & U3[1] & U3[2])
	{
      ...
      else if (U3[0] & U3[1] || U3[1] & U3[2] || U3[0] & U3[2])
	{
	  N[tree.root] = 1;
	}
...
static double
TreeScore (double x[], double space[])
{
  ...
  if (com.clock == 2)
    {
      error2 ("local clock in TreeScore");
    }
...
int
StepwiseAddition (FILE * fout, double space[])
{
  ...
  int ns0 = com.ns, is, i, j, /*bestbranch = 0, */ randadd = 0, order[NS];	//CMV never used
  int sizetree = (2 * com.ns - 1) * sizeof (struct TREEN);
  double bestscore = 0, score, *x = treestar.x;

  if (com.ns > 50)
    {
      printf ("if this crashes, increase com.sspace?");
    }

  if (com.ns < 3)
    {
      error2 ("2 sequences, no need for tree search");
    }
  ...
  for (is = com.ns; is < ns0; is++)	/* add the is_th species */
      ...
      for (j = 0; j < treestar.tree.nbranch + (com.clock > 0); j++, com.ns--)
	{
	  ...
	  if (j == 0 || score < bestscore
	      || (score == bestscore && rndu () < .2))
	    {
	      treebest.tree = tree;
	      memcpy (treebest.nodes, nodes, sizetree);
	      xtoy (x, treebest.x, com.np);
	      bestscore = score;
	      //bestbranch = j; //CMV never used
	    }
	}
...
static int DecompTree (int inode, int ison1, int ison2);

int
StarDecomposition (FILE * fout, double space[])
{
  /* automatic tree search by star decomposition, nhomo<=1
     returns (0,1,2,3) for the 4s problem.
   */
  int status = 0, stage = 0, i, j, itree, ntree = 0, ntreet, best =
    0, improve = 1, collaps = 0;
  int inode, nson = 0, ison1, ison2 /*, son1, son2 */ ;	//CMV son1 son2 set but never used
  int sizetree = (2 * com.ns - 1) * sizeof (struct TREEN);
  double x[NP];
  ...
  for (ntree = 0, stage = 1;; stage++)
      ...
      for (inode = treebest.tree.nnode - 1; inode >= 0; inode--)
	{
	  ...
	  else if (nson > 2 + (inode == treebest.tree.root))
	    {
	      break;
	    }
	}
      ...
      if (collaps && stage)
	{
      ...
      if (!com.clock && inode == treestar.tree.root && nson == 4)
	{
	  ntreet = 3;
	}
	...
	for (ison2 = ison1 + 1; ison2 < nson && itree < ntreet;
	     ison2++, itree++, ntree++)
	  {
	    DecompTree (inode, ison1, ison2);
	    //son1 = nodes[tree.nnode - 1].sons[0]; //CMV set but never used
	    //son2 = nodes[tree.nnode - 1].sons[1]; //CMV set but never used

	    for (i = com.np - 1; i > 0; i--)
	      {
		x[i] = treestar.x[i - 1];
	      }
	    ...
	    if (!com.clock)
	      for (i = 0; i < tree.nbranch; i++)
		x[i] =
		  max2 (nodes[tree.branches[i][1]].branch * 0.99, 0.0001);
	    else
	      for (i = 1, x[0] = max2 (x[0], .01); i < com.ntime; i++)
		{
		  x[i] = .5;
		}
	    ...
	    if (fsum)
	      {
		fprintf (fsum, "\nS=%d:%3d/%d  T=%4d  ", stage,
			 itree + 1, ntreet, ntree + 1);
		OutTreeN (fsum, 0, 0);
	      }

	    if (fout)
	      {
		fprintf (fout, "\nS=%d:%4d/%4d  T=%4d ", stage,
			 itree + 1, ntreet, ntree + 1);
		OutTreeN (fout, 0, 0);
	      }
      ...
      //son1 = treebest.nodes[tree.nnode - 1].sons[0]; //CMV set but never used
      //son2 = treebest.nodes[tree.nnode - 1].sons[1]; //CMV set but never used
    }				/* for (stage) */

  if (com.ns <= 4 && !improve && best)
    {
      error2 ("strange");
    }
...
static int
DecompTree (int inode, int ison1, int ison2)
{
  ...
  for (i = 0, bt = 0; i < tree.nnode; i++)
    if (i != tree.root)
      {
	bt += nodes[i].branch / tree.nbranch;
      }
...
int
MultipleGenes (FILE * fout, FILE * fpair[], double space[])
{
  ...
  if (com.ndata > 1)
    {
      error2 ("multiple data sets & multiple genes?");
    }
  ...
  for (; ig < ngene0; ig++)
    {
      ...
      com.ngene = 1;
      com.ls = com.lgene[0] =
	ig == 0 ? lgene0[0] : lgene0[ig] - lgene0[ig - 1];
      com.npatt =
	ig == ngene0 - 1 ? npatt0 - posG0[ig] : posG0[ig + 1] - posG0[ig];
      com.posG[0] = 0;
      com.posG[1] = com.npatt;
...
printSeqsMgenes (void)
{
  ...
  for (ig = 0, FPN (F0); ig < com.ngene; ig++)
	...
	if (com.pose[h] == ig)
      ...
      if ((fseq = fopen (seqf, "w")) == NULL)
	{
	  error2 ("file creation err.");
	}
...
static void UpPassScoreOnlyB (int inode);
...
static void
UpPassScoreOnly (int inode)
{
  /* => VU, VL, & MM, theorem 2 */
  int ison, i, j;
  char *K = Kspace, maxK;	/* chMark (VV) not used in up pass */

  FOR (i, nodes[inode].nson) if (nodes[nodes[inode].sons[i]].nson > 0)
    {
  ...
  for (i = 0, NchU[inode] = 0; i < com.ncode; i++)
    if (K[i] == maxK)
      {
	chU[inode * com.ncode + NchU[inode]++] = (char) i;
      }
...
UpPassScoreOnlyB (int inode)
{
  ...
  ison1 = nodes[inode].sons[0];
  ison2 = nodes[inode].sons[1];

  if ((chUB[inode] = (chUB[ison1] & chUB[ison2])) == 0)
    {
      chUB[inode] = (chUB[ison1] | chUB[ison2]);
      change = 1;
    }
...
MPScore (double space[])
{
  ...
  Nsteps = (int *) space;
  BitOperation =
    (tree.nnode == 2 * com.ns - 1 - (nodes[tree.root].nson == 3));
  BitOperation = (BitOperation && com.ncode < 32);

  if (BitOperation)
    {
      chUB = Nsteps + tree.nnode;
    }
  else
    {
      chU = (char *) (Nsteps + tree.nnode);
      NchU = chU + tree.nnode * com.ncode;
      Kspace = NchU + tree.nnode;
    }
  ...
  for (h = 0, score = 0; h < com.npatt; h++)
      ...
      if (BitOperation)
	{
	  ...
	  if (nodes[tree.root].nson > 2)
	      ...
	      if (U[0] & U[1] & U[2])
		{
	      ...
	      else if (U[0] & U[1] || U[1] & U[2] || U[0] & U[2])
		{
		  change = 1;
		}
      ...
      else			/* polytomies, use characters */
	{
	  FOR (i, com.ns)
	  {
	    chU[i * com.ncode] = (char) (com.z[i][h]);
	    NchU[i] = (char) 1;
	  }
...
static char /* *chMark, */ *chMarkU, *chMarkL;	/* VV, VU, VL *///CMV chMark unused
...
//int UpPass(int inode); //CMV redundant
//int DownPass(int inode); //CMV redundant

static void DownStatesOneNode (int ison, int father);

static void
DownStates (int inode)
{
  /* VU, VL => NCharaCur, CharaCur, theorem 4 */
  int i;

  FOR (i, nodes[inode].nson) if (nodes[inode].sons[i] >= com.ns)
    {
...
static void
DownStatesOneNode (int ison, int father)
{
  ...
  FOR (j, nodes[ison].nson) if (nodes[ison].sons[j] >= com.ns)
    {
      break;
    }

  if (j < nodes[ison].nson)
    {
      DownStates (ison);
    }
}

//int InteriorStatesMP(int job, int h, int *nchange, char NChara[NS - 1], //CMV redundant
...
static int
BootstrapSeq (const char *seqf)	//CMV made argument const
{
  ...
  enum
  { PAML = 0, PAUP };
  //CHHS    char *dt = (com.seqtype == AAseq ? "protein" : "dna"); //CHHS does not compile with g++ and used only once; dt removed and if statement used instead
  const char *paupstart = "paupstart", *paupblock = "paupblock", *paupend = "paupend";	//CMV made const
  int format = 0;		/* 0: paml-phylip; 1:paup-nexus */

  if (com.readpattern)
    {
      error2 ("work on bootstrapping pattern data.");
    }
  ...
  if (com.ngene > 1)
      ...
      if ((gmark = (int *) malloc (com.ls * sizeof (int))) == NULL)
	{
      ...
      for (ig = 0; ig < com.ngene; ig++)
	{
	  com.lgene[ig] = gpos[ig] = 0;
	}

      for (h = 0; h < com.ls; h++)
	{
	  com.lgene[com.pose[h]]++;
	}
	...
	{
	  printf ("Bootstrap uses stratefied sampling for %d partitions.",
		  com.ngene);
  ...
  for (iboot = 0; iboot < nboot; iboot++, FPN (fseq))
      ...
      if (format == PAUP)
	{
	  ...
	  if (com.seqtype == AAseq)	//CHHS if statement added avoid ?/: operator above, which caused a problem on g++
	    fprintf (fseq, "   format datatype=%s missing=? gap=-;\n   matrix\n", "protein");	//CHHS String dt removed and substituted by "protein"
	  else
	    fprintf (fseq, "   format datatype=%s missing=? gap=-;\n   matrix\n", "dna");	//CHHS String dt removed and substituted by "dna"
	  ...
	  if (com.ngene > 1)
	    {
	      ...
	      for (ig = 0; ig < com.ngene; ig++)
		fprintf (fseq,
			 "   charset partition%-2d = %-4d - %-4d;\n",
			 ig + 1, (ig == 0 ? 1 : com.lgene[ig - 1] + 1),
			 com.lgene[ig]);
      ...
      else
	{
      ...
      if (noisy && (iboot + 1) % 10 == 0)
	{
	  printf ("\rdid sample #%d", iboot + 1);
	}
    }				/* for(iboot) */
...
int
rell (FILE * flnf, FILE * fout, int ntree)
{
  ...
  if (ntree0 != -1 && ntree0 != ntree)
    {
      error2 ("rell: input data file strange.  Check.");
    }
  ...
  if (com.sspace < s)
    {
      if (noisy)
	{
	  printf ("resetting space to %lu bytes in rell.\n", (long unsigned int) s);	//CHHS cast to avoid warning in gcc 32 bit
	}
  ...
  lnf = com.space;
  lnL0 = lnf + ntree * com.npatt;
  lnL = lnL0 + ntree;
  pRELL = lnL + ntree * nr;
  pSH = pRELL + ntree;
  vdl = pSH + ntree;
  btrees = (int *) (vdl + ntree);
  fpattB = (int *) malloc ((com.npatt + com.ls + com.ngene) * sizeof (int));

  if (fpattB == NULL)
    {
      error2 ("oom fpattB in rell.");
    }

  sitelist = fpattB + com.npatt;
  lgeneB = sitelist + com.ls;

  lline = (com.seqtype == 1 ? ns0 * 8 : ns0) + 100;
  lline = max2 (16000, lline);

  if ((line = (char *) malloc ((lline + 1) * sizeof (char))) == NULL)
    {
      error2 ("oom rell");
    }
  ...
  for (itree = 0, mltree = 0; itree < ntree; itree++)
    {
      ...
      if (j != itree + 1)
	{
	  printf ("\nerr: lnf, reading tree %d.", itree + 1);
	  return (-1);
	}
      ...
      for (h = 0, lnL0[itree] = 0; h < com.npatt; h++)
	{
      ...
      if (itree && lnL0[itree] > lnL0[mltree])
	{
	  mltree = itree;
	}
    }
  ...
  for (ir = 0; ir < nr; ir++)
      ...
      for (h = 0; h < com.npatt; h++)
	{
      ...
      for (ig = 0, psitelist = sitelist; ig < com.ngene;
	   psitelist += lgeneB[ig++])
	{
	...
	printf ("\r\tRELL Bootstrapping.. replicate: %6d / %d %s",
		ir + 1, nr, printtime (timestr));

    }

  free (fpattB);

  if (fabs (1 - sum (pRELL, ntree)) > 1e-6)
    {
      error2 ("sum pRELL != 1.");
    }
  ...
  if ((lnLmSH = (double *) malloc (nr * sizeof (double))) == NULL)
    {
      error2 ("oom in rell");
    }

  for (j = 0; j < ntree; j++)	/* step 3: centering */
    for (ir = 0, y = sum (lnL + j * nr, nr) / nr; ir < nr; ir++)
      {
	lnL[j * nr + ir] -= y;
      }
  ...
  for (ir = 0; ir < nr; ir++)
	...
	if (lnL[j * nr + ir] > lnLmSH[ir])
	  {
	    lnLmSH[ir] = lnL[j * nr + ir];
	  }
    }
  ...
  FOR (j, ntree)
  {
	...
	status = -1;
      ...
      {
	y = 1 - CDFNormal (-mdl / vdl[j]);
      }

    fprintf (fout, "%6d%c%12.3f %9.3f %9.3f%8.3f%10.3f%9.3f\n",
	     j + 1, (j == mltree ? '*' : ' '), lnL0[j], mdl, vdl[j], y,
	     pSH[j], pRELL[j]);
  }
  ...
  if (status)
    {
      fputs ("(-1 for P values means N/A)\n", fout);
    }
...
static int
ProbSitePattern (double x[], double *lnL, double fhsiteAnc[], double ScaleC[])
{
  ...
  if (com.ncatG <= 1)
      ...
      for (ig = 0, *lnL = 0; ig < com.ngene; ig++)
	{
	  ...
	  for (h = com.posG[ig]; h < com.posG[ig + 1]; h++)
		...
		fhsiteAnc[h] +=
		  com.pi[i] * nodes[tree.root].conP[h * com.ncode + i];
	  ...
	  for (ir = 0; ir < com.ncatG; ir++)
	    {
	      ...
	      if (com.seqtype == 1 && com.NSsites /* && com.model */ )
		{
		  IClass = ir;
		}
...
static int
PostProbNode (int inode, double x[], double fhsiteAnc[],
	      double ScaleC[], double *lnL, double pChar1node[], char za[],
	      double pnode[])
{
  ...
  *lnL = 0;
  ...
  if (com.ncatG > 1 && com.method != 1)
      ...
      for (ig = 0; ig < com.ngene; ig++)
	{
	  ...
	  for (ir = 0; ir < com.ncatG; ir++)
	      ...
	      for (h = com.posG[ig]; h < com.posG[ig + 1]; h++)
		{
		  ...
		  for (i = 0, fh = 0; i < n; i++)
		    {
		      y = com.freqK[ir] * com.pi[i] *
			nodes[tree.root].conP[h * n + i] * S;
		      fh += y;
		      pChar1node[h * n + i] += y;
		    }
		}
      ...
      for (h = 0; h < com.npatt; h++)
	{
	  for (i = 0, y = 0; i < n; i++)
	    {
	      y += (pChar1node[h * n + i] /= fhsiteAnc[h]);
	    }
	      ...
	      for (h = com.posG[ig]; h < com.posG[ig + 1]; h++)
		{
		  if (com.NnodeScale)
			...
			for (k = 0, Sir[ir] = 0; k < com.NnodeScale; k++)
			  Sir[ir] +=
			    com.nodeScaleF[ir * com.NnodeScale *
					   com.npatt + k * com.npatt + h];
		  ...
		  for (i = 0, best = 0; i < n; i++)
		    {
		      ...
		      if (i
			  && pChar1node[h * n + best] < pChar1node[h * n + i])
			{
			  best = i;
			}
		    }
		  ...
		  za[(inode - com.ns) * com.npatt + h] = (char) best;
		  pnode[(inode - com.ns) * com.npatt + h] =
		    pChar1node[h * n + best];
		  *lnL -= log (fh) * com.fpatt[h];

		  if (com.NnodeScale)
		    {
		      *lnL -= Sir[it] * com.fpatt[h];
		    }
		}
	    }
	}
...
static void getCodonNode1Site (char codon[], char zanc[], int inode,
			       int site);
static int ChangesSites (FILE * frst, int coding, char *zanc);
static int print1site (FILE * fout, int h);

static int
AncestralMarginal (FILE * fout, double x[], double fhsiteAnc[], double Sir[])
{
   ...
   */
  char *pch =
    (com.seqtype == 0 ? BASEs : (com.seqtype == 2 ? AAs : BINs)), *zanc;
  char str[4] = "", codon[2][4] = { "   ", "   " }, aa[4] = "";
  //CHHS    char *sitepatt = (com.readpattern ? "pattern" : "site"); //CHHS g++ does not like it, we keep the variable but rewrite this initialization
  char *sitepatt;		//CHHS New, see above

  if (com.readpattern)		//CHHS New, see above
    {
      //CHHS New, see above
      sitepatt = (char *) malloc (8 * sizeof (char));	//CHHS New, see above; size for "pattern"+\0

      if (sitepatt == NULL)
	{
	  error2 ("oom sitepatt");	//CHHS New, see above
	}

      strcpy (sitepatt, "pattern");	//CHHS New, see above
    }				//CHHS New, see above
  else				//CHHS New, see above
    {
      //CHHS New, see above
      sitepatt = (char *) malloc (5 * sizeof (char));	//CHHS New, see above; size for "site"+\0
      ...
      if (sitepatt == NULL)
	{
      ...
      strcpy (sitepatt, "site");	//CHHS New, see above
    }				//CHHS New, see above

  int n = com.ncode, inode, ic = 0, b[3], i, j, k1 = -1, k2 = -1, k3, lsc = com.ls;	//CHHS Variables int c1,c2 unused and removed
  int lst = (com.readpattern ? com.npatt : com.ls);
  ...
  if (com.seqtype == 0 && com.ls % 3 == 0 && com.coding)
    {
      coding = 1;
      lsc = com.ls / 3;
    }
  ...
  if (coding == 1)
    {
      if ((pbestAA =
	   (double *) malloc (nid * lsc * 2 * sizeof (double))) == NULL)
	{
	  error2 ("oom pbestAA");
	}
		  ...
		  for (k3 = 0; k3 < 4; k3++)
		    {
		      ...
		      fh = pChar1node[b[0]] * pChar1node[b[1]] *
			pChar1node[b[2]];
  ...
  if (com.seqtype == 0 && coding && !com.readpattern)	/* coding seqs analyzed by baseml */
      ...
      for (h = 0; h < com.ls / 3; h++, FPN (fout))
	{
	  ...
	  for (j = 0; j < tree.nnode - com.ns; j++)
	    {
	      fprintf (fout, " %1c (%5.3f)", AAs[(int) bestAA[j * com.ls / 3 + h]], pbestAA[j * com.ls / 3 + h]);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	    }
	}
    }
  ...
  for (inode = 0; inode < tree.nnode - com.ns; inode++)
	...
	{
	  pMAPnode[inode] +=
	    com.fpatt[h] * pnode[inode * com.npatt + h] / com.ls;
	  pMAPnodeA[inode] *=
	    pow (pnode[inode * com.npatt + h], com.fpatt[h]);
	}
    }
  ...
  for (h = 0; h < lst; h++)
      ...
      for (j = 0; j < nid; j++)
	{
	  if (com.seqtype != CODONseq)
	    fprintf (fout, "%c(%5.3f) ",
		     pch[(int) zanc[j * com.npatt + hp]],
		     pnode[j * com.npatt + hp]);
	  ...
	  else
	    {
	      ...
	      fprintf (fout, " %s %1c %5.3f (%1c %5.3f)",
		       CODONs[ic], AAs[i], pnode[j * com.npatt + hp],
		       AAs[(int) bestAA[j * com.npatt + hp]],
		       pbestAA[j * com.npatt + hp]);
	    }
	...
	{
	  printf ("\r\tprinting, %d sites done", h + 1);
	}
    }
      ...
      for (j = 0; j < tree.nbranch; j++, FPN (fout))
	{
	  ...
	  if (coding)
	      ...
	      for (h = 0, nst = nat = 0; h < lsc; h++)
		{
		  ...
		  difcodonNG (codon[0], codon[1], &S, &N, &ns, &na, 0,
			      com.icode);
		  nst += ns;
		  nat += na;
		}
	  ...
	  for (h = 0; h < lst; h++)
	      ...
	      if (com.seqtype != CODONseq)
		{
		  ...
		  k1 = pch[(int) zanc[(tree.branches[j][0] - com.ns) * com.npatt + hp]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	      ...
	      else
		{
		  if (inode < com.ns)
		    {
		      strcpy (codon[1], CODONs[(int) com.z[inode][hp]]);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
		      k2 = GetAASiteSpecies (inode, hp);
		    }
		  else
		    {
		      strcpy (codon[1], CODONs[(int)
					       zanc[(inode -
						     com.ns) * com.npatt +
						    hp]]);
		      k2 = AAs[(int)
			       bestAA[(inode - com.ns) * com.npatt + hp]];
		      p2 = pbestAA[(inode - com.ns) * com.npatt + hp];
		    }

		  strcpy (codon[0], CODONs[(int)
					   zanc[(tree.branches[j][0] -
						 com.ns) * com.npatt + hp]]);
		  k1 = AAs[(int)
			   bestAA[(tree.branches[j][0] -
				   com.ns) * com.npatt + hp]];
		  p1 = pbestAA[(tree.branches[j][0] -
				com.ns) * com.npatt + hp];
	      ...
	      if (sitelabels)
		{
	      ...
	      if (inode < com.ns)
		{
		  fprintf (fout, "%c %.3f -> %1c\n", k1, p1, k2);
		}
	      else
		{
		  fprintf (fout, "%c %.3f -> %1c %.3f\n", k1, p1, k2, p2);
		}
  ...
  matout2 (fout, pMAPnode, 1, nid, 9, 5);
  fputs ("for a site.\n", fout);
  matout2 (fout, pMAPnodeA, 1, nid, 9, 5);
  fputs ("for the sequence.\n", fout);
  ...
  free (sitepatt);		//CHHS New, needed due to new malloc statement
  return (0);
}


static void
getCodonNode1Site (char codon[], char zanc[], int inode, int site)
{
  ...
  if (com.seqtype == CODONseq)
    {
      ...
      if (inode >= com.ns)
	{
	  strcpy (codon, CODONs[(int) zanc[(inode - com.ns) * com.npatt + hp]]);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	}
      else
	{
	  strcpy (codon, CODONs[(int) com.z[inode][hp]]);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	}
	...
	for (i = 0; i < 3; i++)
	  codon[i] = BASEs[(int)
			   zanc[(inode - com.ns) * com.npatt +
				com.pose[site * 3 + i]]];
      else
	for (i = 0; i < 3; i++)
	  {
	    codon[i] = BASEs[(int) com.z[inode][com.pose[site * 3 + i]]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	  }
    }
...
static int
ChangesSites (FILE * frst, int coding, char *zanc)
{
      ...
      for (h = 0; h < ls1; h++)
	{
	  ...
	  for (inode = 0, S = N = Sd = Nd = 0; inode < tree.nnode; inode++)
	      ...
	      if (inode == tree.root)
		{
	      ...
	      difcodonNG (codon[0], codon[1], &S1, &N1, &Sd1, &Nd1, 0,
			  com.icode);
	      S += S1 * b / btotal;
	      N += N1 * b / btotal;
	  ...
	  fprintf (frst, "(S N: %7.3f%7.3f Sd Nd: %6.1f %5.1f)\n", S * 3,
		   N * 3, Sd, Nd);
	  fprintf (frst1, "%4d S N: %7.3f%7.3f Sd Nd: %6.1f %5.1f ",
		   h + 1, S * 3, N * 3, Sd, Nd);
      ...
      for (h = 0; h < ls1; h++)
	{
	  ...
	  for (inode = 0, d = 0; inode < tree.nnode; inode++)
	      ...
	      if (inode == tree.root)
		{
	      ...
	      k1 = pch[(int)
		       zanc[(nodes[inode].father - com.ns) * com.npatt + hp]];

	      if (inode < com.ns)
		{
		  k2 = pch[(int) com.z[inode][hp]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
		}
	      else
		k2 = pch[(int) zanc[(inode - com.ns) * com.npatt + hp]];
...
static int parsimony = 0, *nBestScore, *icharNode[NBESTANC], *combIndex;	//CMV added static
static double *fhsiteAnc, *lnPanc[NBESTANC], *PMatTips, *combScore;	//CMV added static
static char *charNode[NBESTANC], *ancSeq, *ancState1site;	//CMV added static
static FILE *fanc;		//CMV added static
static int largeReconstruction;	//CMV added static

static void DownPassPPSG2000OneSite (int h, int inode, int inodestate,
				     int ipath);
static void PrintAncState1site (char ancState1site[], double prob);
...
//double piroot[NCODE] = { 0 }; //CMV unused
...
static void
UpPassPPSG2000 (int inode, int igene, double x[])
{
   ...
   */
  int debug = 0;
  int n = com.ncode, it, ibest, i, j, k, h, ison, nson = nodes[inode].nson;	//CHHS int *pc not used and removed
  ...
  for (i = 0, ncomb = 1; i < nson; i++)
    ncomb *= (nBestScore[i] =
	      (nodes[nodes[inode].sons[i]].nson > 0 ? NBESTANC : 1));
  ...
  if (debug)
    {
      ...
      for (i = 0; i < nson; i++)
	{
	  printf (" %2d", nodes[inode].sons[i] + 1);
	}
  ...
  if (inode != tree.root)	/* calculate log{P(t)} from father to inode */
	...
	{
	  if (com.clock)
	    t *= GetBranchRate (igene, (int) nodes[inode].label, x, NULL);
	  else
	    {
	      t *= com.rgene[igene];
	    }
      ...
      for (ichar = 0; ichar < (inode != tree.root ? n : 1); ichar++)	/* ichar for father */
	{
	  ...
	  for (icomb = 0; icomb < n * ncomb; icomb++)
	      ...
	      if (inode == tree.root && parsimony)
		{
	      ...
	      if (debug)
		{
		  printf ("comb %2d %c", icomb + 1, BASEs[jchar]);
		}
	      ...
	      for (i = 0, it = icomb % ncomb; i < nson; i++)	/* The ibest-th state in ison. */
		{
		  ...
		  if (nodes[ison].nson)	/* internal node */
		    y = lnPanc[ibest][(ison - com.ns) * com.npatt * n +
				      h * n + jchar];
		    ...
		    {
		      for (k = 0, y = 0; k < nChara[(int) com.z[ison][h]]; k++)	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
			{
			  y += PMatTips[ison * n * n + jchar * n + CharaMap[(int) com.z[ison][h]][k]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
			}
	  ...
	  if (debug)
	    {
	      ...
	      for (i = 0; i < n * ncomb; i++)
		{
		  printf (" %4.1f", combScore[i]);
		}

	      FPN (F0);
	    }

	  indexing (combScore, n * ncomb, combIndex, 0,
		    combIndex + n * ncomb);
	  ...
	  for (j = 0; j < (inode != tree.root ? NBESTANC : n * ncomb); j++)
	    {
	      jchar = (it = combIndex[j]) / ncomb;
	      it %= ncomb;

	      if (j < NBESTANC)
		{
		  lnPanc[j][(inode - com.ns) * com.npatt * n + h * n +
			    ichar] = combScore[combIndex[j]];
		  charNode[j][(inode - com.ns) * com.npatt * n + h * n +
			      ichar] = jchar;
		}

	      if (debug)
		printf ("\t#%d: %6.1f %c ", j + 1,
			combScore[combIndex[j]], BASEs[jchar]);
	      ...
	      for (i = 0, ipath = 0; i < nson; i++)
		{
	      ...
	      if (j < NBESTANC)
		icharNode[j][(inode - com.ns) * com.npatt * n + h * n +
			     ichar] = ipath;
	      ...
	      if (inode == tree.root)
		{
		  ...
		  if (parsimony)
		    {
		      y = combScore[combIndex[j]];
		    }
		  else
		    psum1site += y =
		      exp (-combScore[combIndex[j]] - fhsiteAnc[h]);
      ...
      if (inode == tree.root)
	{
	  fprintf (fanc, " (total %6.3f)\n", psum1site);
	}

      if (largeReconstruction && (h + 1) % 2000 == 0)
	printf ("\r\tUp pass for gene %d node %d sitepatt %d.",
		igene + 1, inode + 1, h + 1);
...
static void
DownPassPPSG2000OneSite (int h, int inode, int inodestate, int ipath)
{
  ...
  for (i = 0; i < nodes[inode].nson; i++)
      ...
      if (nodes[ison].nson > 1)
	{
	  ...
	  ancState1site[ison - com.ns] = sonstate =
	    charNode[ibest][(ison - com.ns) * com.npatt * n + h * n +
			    inodestate];
	  DownPassPPSG2000OneSite (h, ison, sonstate,
				   icharNode[ibest][(ison -
						     com.ns) * com.npatt *
						    n + h * n + inodestate]);
	}
    }
...
#ifdef CODEML
	  fprintf (fanc, "%s ",
		   getcodon (codon, FROM61[(int) ancState1site[i]]));
#endif
	}
      else
	{
...
DownPassPPSG2000 (int inode)
{
  ...
  for (h = 0; h < com.npatt; h++)
	...
	= charNode[0][(inode - com.ns) * com.npatt * com.ncode +
		      h * com.ncode + c0];
    }
...
static void
AncestralJointPPSG2000 (FILE * fout, double x[])
{
   ...
   */
  int n = com.ncode, nintern = tree.nnode - com.ns, i, igene;	//CHHS Variables int h,j,k,hp unused and removed
  int maxnson = 0, maxncomb;	//CHHS int lst=(com.readpattern?com.npatt:com.ls) unused and removed
  double t;
  ...
  if (noisy)
    {
      puts ("Joint reconstruction.");
    }

  for (i = 0; i < tree.nnode; i++)
    {
      maxnson = max2 (maxnson, nodes[i].nson);
    }

  /* for int at least 32 bits */
  //CMV Split into two tests to remove a warning on windows
#if NBESTANC > 4
  error2 ("NBESTANC too large.");
#else
  if (maxnson > 16)
    {
      error2 ("Too many sons.");
    }
#endif

  for (i = 0, maxncomb = 1; i < maxnson; i++)
    {
      maxncomb *= NBESTANC;
    }

  if ((PMatTips =
       (double *) malloc (com.ns * n * n * sizeof (double))) == NULL)
    {
      error2 ("oom PMatTips");
    }
  ...
  if (s > sconPold)
    {
      com.sconP = s;
      printf ("\n%9lu bytes for conP, adjusted\n", (long unsigned int) com.sconP);	//CHHS cast to avoid warning in gcc 32 bit
  ...
  s =
    ((s * sizeof (int) + (s + nintern) * sizeof (char) +
      16) / sizeof (double)) * sizeof (double);

  if (s > com.sspace)
    {
      com.sspace = s;
      printf ("\n%9lu bytes for space, adjusted\n", (long unsigned int) com.sspace);	//CHHS cast to avoid warning in gcc 32 bit
  ...
  for (i = 0; i < NBESTANC; i++)
    {
      ...
      charNode[i] =
	(char *) ((int *) com.space +
		  NBESTANC * nintern * com.npatt * n)
	+ i * nintern * com.npatt * n;
      ancState1site = charNode[0] + NBESTANC * nintern * com.npatt * n;
    }

  if ((ancSeq =
       (char *) malloc (nintern * com.npatt * n * sizeof (char))) == NULL)
    {
      error2 ("oom charNode");
    }

  if ((combScore =
       (double *) malloc ((3 * n * maxncomb + com.ns) * sizeof (double))) ==
      NULL)
    {
  ...
  fprintf (fout,
	   "(eqn. 2 in Yang et al. 1995 Genetics 141:1641-1650), using ");
  fprintf (fout,
	   "the algorithm of Pupko et al. (2000 Mol Biol Evol 17:890-896),\n");
  ...
  for (igene = 0; igene < com.ngene; igene++)
      ...
      for (i = 0; i < com.ns; i++)
	{
	  ...
	  if (com.clock < 5)
	    {
	      if (com.clock)
		t *= GetBranchRate (igene, (int) nodes[i].label, x, NULL);
	      else
		{
		  t *= com.rgene[igene];
		}
...
int
AncestralSeqs (FILE * fout, double x[])
{
  ...
  if (com.NnodeScale && com.ncatG > 1)
    if ((ScaleC =
	 (double *) malloc (max2 (com.npatt, com.ncatG) *
			    sizeof (double))) == NULL)
      {
  ...
  if (!com.verbose)
    {
      fputs ("Constant sites not listed for verbose=0\n", fout);
    }

  if (!com.cleandata)
    {
      fputs ("Unreliable at sites with alignment gaps\n", fout);
    }
...
static int SetNodeScale (int inode);	//CMV local to this file

void
InitializeNodeScale (void)
{
   ...
   */
  int i;
  size_t nS, ii;		//CMV added ii to remove singed/unsigned problems
  ...
  com.NnodeScale = 0;
  com.nodeScale =
    (char *) realloc (com.nodeScale, tree.nnode * sizeof (char));

  if (com.nodeScale == NULL)
    {
      error2 ("oom");
    }
  ...
  if (com.NnodeScale)
    {
      if ((com.nodeScaleF =
	   (double *) realloc (com.nodeScaleF, nS * sizeof (double))) == NULL)
	{
	  error2 ("oom nscale");
	}
...
void
NodeScale (int inode, int pos0, int pos1)
{
  /* scale to avoid underflow
   */
  int h, k, j, n = com.ncode;
  double t;			//CHHS Variable double smallw=1e-12 unused and removed
...
int
HessianSKT2004 (double xmle[], double lnLm, double g[], double H[])
{
  ...
  double *x, *lnL[2], *df[2], eh0 = Small_Diff * 2, eh;	//CHHS Variable double small unused and removed
  ...
  if (df[0] == NULL)
    {
      error2 ("oom space in HessianSKT2004");
    }
  ...
  for (i = 0; i < com.np; i++)
    {
      ...
      for (h = 0; h < com.npatt; h++)
	df[0][i * com.npatt + h] =
	  (df[1][i * com.npatt + h] - df[0][i * com.npatt + h]) / (eh * 2);
    }
  ...
  for (i = 0; i < com.np; i++)
	...
	for (h = 0; h < com.npatt; h++)
	  H[i * com.np + j] -=
	    df[0][i * com.npatt + h] * df[0][j * com.npatt +
					     h] * com.fpatt[h];
    }
...
int
lfunRates (FILE * fout, double x[], int np)
{
  ...
  fprintf (fout, "\nrate:");
  FOR (j, com.ncatG) fprintf (fout, " %8.5f", com.rK[j]);
  fprintf (fout, "\nfreq:");
  FOR (j, com.ncatG) fprintf (fout, " %8.5f", com.freqK[j]);
  FPN (fout);
  ...
  if ((fhsite = (double *) malloc (com.npatt * sizeof (double))) == NULL)
    {
      error2 ("oom fhsite");
    }
	  ...
	  for (h = 0; h < lst; h++, FPN (fout))
	    {
	      ...
	      for (ir = 0; ir < com.ncatG; ir++)
		fprintf (fout, " %9.4f",
			 com.freqK[ir] * com.fhK[ir * com.npatt +
						 hp] / fhsite[hp]);
	    }
	}
...
int
GetPMatBranch (double Pt[], double x[], double t, int inode)
{
  ...
  if (com.model <= K80)
    ...
    {
      if (com.nhomo == 2)
	EigenTN93 (com.model, nodes[inode].kappa, -1, com.pi, &nR, Root,
		   Cijk);
      else if (com.nhomo > 2)	/* need kappa on each node if fix_kappa ==0 */
	EigenTN93 (com.model, nodes[inode].kappa, -1, nodes[inode].pi,
		   &nR, Root, Cijk);
...
GetPMatBranch (double Pt[], double x[], double t, int inode)
{
   ...
   */
  int iUVR = 0, nUVR = NBTYPE + 2, ib = (int) nodes[inode].label;	//CHHS Variable in updateUVR=0 unused and removed
  double *pkappa, w, mr = 1, Qfactor = 1;

  pkappa = (com.hkyREV
	    || com.codonf ==
	    FMutSel ? x + com.ntime + com.nrgene : &com.kappa);
  ...
  if (com.seqtype == CODONseq && com.NSsites && com.model)
  ...
  else if (com.seqtype == CODONseq && BayesEB == 2 && com.model > 1)	/* BEB for A&C */
    {
      ...
      Qfactor = Qfactor_NS_branch[ib];
	   ...
	   && (com.model == 1 || com.model == 2) && com.nbtype <= nUVR)
    {
      ...
      if (com.aaDist == AAClasses)	/* AAClass model */
	{
	  ...
	  EigenQcodon (0, -1, NULL, NULL, NULL, Root, U, V, &mr, pkappa,
		       -1, Pt);
	}
      else if (com.nbtype > nUVR)	/* branch models, with more than 8 omega */
	{
	  EigenQcodon (0, -1, NULL, NULL, NULL, Root, U, V, &mr, pkappa,
		       nodes[inode].omega, Pt);
	}
    }
  ...
  if (com.seqtype == AAseq && com.model == Poisson)
    ...
    {
      t *= Qfactor;
      PMatUVRoot2 (Pt, t, com.ncode, U, V, Root);
    }
...
static void
print_lnf_site (int h, double logfh)
{
#if(defined BASEML || defined CODEML)
...
int
SetPSiteClass (int iclass, double x[])
{
  ...
  mr = 1 / Qfactor_NS;
  pkappa = (com.hkyREV
	    || com.codonf == FMutSel ? xcom + com.nrgene : &com.kappa);
	  ...
	  if (com.aaDist)
	    {
	      if (com.aaDist < 10)
		{
		  com.pomega = xcom + k + com.ncatG - 1 + 2 * iclass;
		}
	      else if (com.aaDist == 11)
		{
		  com.pomega = xcom + k + com.ncatG - 1 + 4 * iclass;
		}
	      else if (com.aaDist == 12)
		{
		  com.pomega = xcom + k + com.ncatG - 1 + 5 * iclass;
		}
	    }
...
fx_r (double x[], int np)
{
   ...
   */
  int h, ir, i, k, ig;		/*, FPE = 0 *///CMV never used;
  double fh;			/* for testing site class with w=0 *///CHHS Variable double smallw=1e-12 unused and removed
  ...
  for (ig = 0; ig < com.ngene; ig++)	/* alpha may differ over ig */
      ...
      for (ir = 0; ir < com.ncatG; ir++)
	{
	  if (ir && com.conPSiteClass)	/* shift com.nodeScaleF & conP */
	      ...
	      if (com.NnodeScale)
		{
	      ...
	      for (i = com.ns; i < tree.nnode; i++)
		nodes[i].conP +=
		  (tree.nnode - com.ns) * com.ncode * (size_t) com.npatt;
	    }
	  ...
	  for (h = com.posG[ig]; h < com.posG[ig + 1]; h++)
	      ...
	      if (fh <= 0)
		{
		  if (fh < -1e-10 /* && !FPE */ )	/* note that 0 may be o.k. here */
		    {
		      //FPE = 1; //CMV never used
		      matout (F0, x, 1, np);
		      printf ("\nfx_r: h = %d  r = %d fhK = %.5e ", h + 1,
			      ir + 1, fh);
		...
		for (k = 0, com.fhK[ir * com.npatt + h] = log (fh);
		     k < com.NnodeScale; k++)
		  com.fhK[ir * com.npatt + h] +=
		    com.nodeScaleF[k * com.npatt + h];
	    }			/* for (h) */
	}			/* for (ir) */
      ...
      if (com.conPSiteClass)	/* shift pointers conP back */
	{
	  if (com.NnodeScale)
	    com.nodeScaleF -=
	      (com.ncatG - 1) * com.NnodeScale * (size_t) com.npatt;

	  for (i = com.ns; i < tree.nnode; i++)
	    nodes[i].conP -=
	      (com.ncatG - 1) * (tree.nnode -
				 com.ns) * com.ncode * (size_t) com.npatt;
	}
    }				/* for(ig) */
}
...
print1site (FILE * fout, int h)
{
   ...
   */
  char *pch = (com.seqtype == 0 ? BASEs : (com.seqtype == 2 ? AAs : BINs));	//CHHS Variable char compatibleAAs[20]="" unused and removed
  int i, b, aa = 0;		//CHHS Variable int n=com.ncode unused and removed
...
static int noisy_minbranches;
static double *space_minbranches, *g_minbranches, *varb_minbranches, e_minbranches;	//CMV added static

double minbranches (double xcom[], int np);
int lfunt (double t, int a, int b, double x[], double *l, double space[]);
int lfuntdd (double t, int a, int b, double x[], double *l, double *dl,
	     double *ddl, double space[]);
static int lfunt_SiteClass (double t, int a, int b, double x[], double *l,
			    double space[]);
int lfuntdd_SiteClass (double t, int a, int b, double x[], double *l,
		       double *dl, double *ddl, double space[]);

int
minB (FILE * fout, double *lnL, double x[], double xb[][2], double e0,
      double space[])
{
  ...
  if (com.conPSiteClass)
    {
      s = (2 * com.ncode * com.ncode +
	   com.ncode * (size_t) com.npatt) * sizeof (double);

      if (com.sspace < s)	/* this assumes that space is com.space */
	{
	  printf ("\n%lu bytes in space, %lu bytes needed\n", (long unsigned int) com.sspace, (long unsigned int) s);	//CHHS cast twice to avoid warning in gcc 32 bit
  ...
  s = (3 * com.ncode * com.ncode +
       (com.conPSiteClass) * 4 * (size_t) com.npatt) * sizeof (double);
  ...
  if (com.ntime == 0)
    {
      error2 ("minB: should not come here");
    }
  ...
  if (com.NSsites == 0)
    {
      com.pomega = xcom + com.nrgene + !com.fix_kappa;
    }
  ...
  for (ir = 0; (npcom == 0 || com.method) && ir < maxr; ir++)
      ...
      if (npcom)
	{
	  ...
	  ming2 (NULL, lnL, com.plfun, NULL, xcom, xbcom, com.space, e,
		 npcom);

	  if (noisy > 2)
	    {
	      FPN (F0);
	      FOR (i, npcom) printf ("%12.6f", xcom[i]);
	      printf ("%8s%s\n", "", printtime (timestr));
	    }
	}
      ...
      if (noisy > 2)
	{
      ...
      if ((dl = fabs (*lnL - lnL0)) < e0 && e <= 0.02)
	{
	  break;
	}

      if (dl < small_improvement)
	{
	  small_times++;
	}
      else
...
int
minB2 (FILE * fout, double *lnL, double x[], double xb[][2], double e0,
       double space[])
{
  /*
   */
  ...
  int status = 0, i, npcom = com.np - com.ntime;
  size_t s;
  double *xcom = x + com.ntime;	//CHHS Variable double lnL0= *lnL unused and removed
  double (*xbcom)[2] = xb + ntime0;
  ...
  if (com.ntime == 0 || npcom == 0)
    {
      error2 ("minB2: should not come here");
    }
...
minbranches (double x[], int np)
{
  ...
  double lnL, lnL0 = 0, l0, l, dl = 0, ddl = -1, t, t0, /*t00, */ p, step = 1, small = 1e-20, y;	//CMV initialized dl to silence warning, t00 removed it is unused
  double tb[2] = { 1e-8, 50 }, e = e_minbranches, *space = space_minbranches;
  double *xcom = x + com.ntime;	/* this is incorrect as com.ntime=0 */

  if (com.ntime)
    {
      error2 ("ntime should be 0 in minbranches");
    }

  lnL0 = l0 = l = lnL = com.plfun (xcom, -1);

  if (noisy_minbranches > 2)
    {
      printf ("\tlnL0 =    %14.6f\n", -lnL0);
    }
  ...
  for (icycle = 0; icycle < maxcycle; icycle++)
	...
	{
	  t = t0 = /* t00 = */ nodes[tree.branches[ib][1]].branch;	//CMV removed t00 set but never used
	  l0 = l;
	  ...
	  for (icycleb = 0; icycleb < ncycleb; icycleb++)	/* iterating a branch */
		...
		{
		  step = min2 (1., (tb[0] - t0) / p);
		}
	      else
		{
		  step = min2 (1., (tb[1] - t0) / p);
		}
	      ...
	      for (i = 0; step > small; i++, step /= 4)
		{
		  ...
		  if (!com.conPSiteClass)
		    {
		      lfunt (t, a, b, xcom, &l, space);
		    }
		  else
		    {
		      lfunt_SiteClass (t, a, b, xcom, &l, space);
		    }
	      ...
	      if (step <= small)
		{
	      ...
	      if (fabs (t - t0) < e * fabs (1 + t) && fabs (l - l0) < e)
		{
		  break;
		}
...
int
lfunt (double t, int a, int b, double xcom[], double *l, double space[])
{
  ...
  for (ig = 0; ig < com.ngene; ig++)
    {
      if (com.Mgene > 1)
	{
	  SetPGene (ig, 1, 1, 0, xcom);	/* com.ntime=0 */
	}
      ...
      for (k = 0, expt = 1; k < nroot; k++)
	{
      ...
      for (h = com.posG[ig]; h < com.posG[ig + 1]; h++)
	{
	  n1 = (b < com.ns ? nChara[(int) com.z[b][h]] : n);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	  ...
	  for (i = 0, fh = 0; i < n1; i++)
	    {
	      ...
	      if (b < com.ns)
		{
		  piqi = com.pi[xb = CharaMap[(int) com.z[b][h]][i]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
		}
	      else
		{
		  piqi = com.pi[i] * nodes[b].conP[h * n + i];
		}
...
int
lfuntdd (double t, int a, int b, double xcom[], double *l, double *dl,
	 double *ddl, double space[])
{
  ...
  int n1 = (com.cleandata && b < com.ns ? 1 : n), xb;
  double expt, uexpt = 0, multiply;
  double *P = space, *dP = P + n * n, *ddP =
    dP + n * n, piqi, pqj, dpqj, ddpqj, fh, dfh, ddfh;
  double *pkappa, mr = 0;
      ...
      for (k = 0, expt = 1; k < nroot; k++)
	{
		...
		if (k)
		  {
		    ...
		    ddP[i * n + j] +=
		      uexpt * V[k * n + j] * multiply * multiply;
		  }
	      }
	  ...
	  for (i = 0; i < n; i++)
		...
		if (k)
		  {
		    dP[i * n + j] +=
		      Cijk[i * n * nroot + j * nroot + k] * expt * multiply;
		    ddP[i * n + j] +=
		      Cijk[i * n * nroot + j * nroot +
			   k] * expt * multiply * multiply;
		  }
	      }
...
static int
lfunt_SiteClass (double t, int a, int b, double xcom[], double *l,
		 double space[])
{
  ...
  if (com.NnodeScale == 0)
      ...
      for (h = 0; h < com.npatt; h++)
	{
	  for (ir = 0, it = 0; ir < com.ncatG; ir++)
		...
		y += com.nodeScaleF[ir * com.NnodeScale * com.npatt +
				    k * com.npatt + h];
	  ...
	  for (ir = 0; ir < com.ncatG; ir++)
	    pK[ir * com.npatt + h] =
	      com.freqK[ir] * exp (pK[ir * com.npatt + h] - Sh[h]);
	}
    }
      ...
      for (ig = 0; ig < com.ngene; ig++)
	{
	  ...
	  for (h = com.posG[ig]; h < com.posG[ig + 1]; h++)
	      ...
	      for (i = 0; i < n1; i++)
		{
		  ...
		  if (b < com.ns)
		    {
		      piqi = pK[ir * com.npatt + h] * com.pi[xb = CharaMap[(int) com.z[b][h]][i]];	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
		    }
		  else
		    piqi =
		      pK[ir * com.npatt +
			 h] * com.pi[i] * nodes[b].conP[h * n + i];
  ...
  for (i = com.ns; i < tree.nnode; i++)	/* shift position */
    nodes[i].conP -=
      (com.ncatG - 1) * (tree.nnode - com.ns) * n * (size_t) com.npatt;
...
int
lfuntdd_SiteClass (double t, int a, int b, double xcom[],
		   double *l, double *dl, double *ddl, double space[])
{
  ...
  for (h = 0, *l = *dl = *ddl = 0; h < com.npatt; h++)
      ...
      if (com.NnodeScale)
	{
      ...
      *ddl -=
	(fh[h] * ddfh[h] - dfh[h] * dfh[h]) / (fh[h] * fh[h]) * com.fpatt[h];
    }
...
void
BranchLengthBD (int rooted, double birth, double death, double sample,
		double mut)
{
  ...
  if (sample == 0)		/* coalescent model.  Check this!!!  */
    for (i = com.ns, y = 0; i > 1; i--)
      nodes[com.ns * 2 - i].age = y +=
	-log (rndu ()) / (i * (i - 1.) / 2.) * mut / 2;
...
int
RandomLHistory (int rooted, double space[])
{
...
ProcessFossilInfo ()
{
  ...
  for (i = sptree.nspecies; i < tree.nnode; i++)
	...
	if ((pch = strstr (nodes[i].nodeStr, fossils[j])))
	  {
	    break;
	  }
	...
	case (LOWER_F):
	  ...
	  sscanf (pch, "%lf,%lf,%lf,%lf", &sptree.nodes[i].pfossil[0],
		  &sptree.nodes[i].pfossil[1],
		  &sptree.nodes[i].pfossil[2], &sptree.nodes[i].pfossil[3]);
	  break;
	case (UPPER_F):
	  sptree.nodes[i].pfossil[2] = tailR;
	  sscanf (pch, "%lf,%lf", &sptree.nodes[i].pfossil[1],
		  &sptree.nodes[i].pfossil[2]);
	  break;
	case (BOUND_F):
	  ...
	  if (sptree.nodes[i].pfossil[0] > sptree.nodes[i].pfossil[1])
	    {
	      printf ("fossil bounds (%.4f, %.4f)",
		      sptree.nodes[i].pfossil[0], sptree.nodes[i].pfossil[1]);
	      error2 ("fossil bounds in tree incorrect");
	    }

	  break;
	case (GAMMA_F):
	  sscanf (pch, "%lf,%lf", &sptree.nodes[i].pfossil[0],
		  &sptree.nodes[i].pfossil[1]);
	  break;
	case (SKEWN_F):
	  sscanf (pch, "%lf,%lf,%lf", &sptree.nodes[i].pfossil[0],
		  &sptree.nodes[i].pfossil[1], &sptree.nodes[i].pfossil[2]);
	  break;
	case (SKEWT_F):
	  sscanf (pch, "%lf,%lf,%lf,%lf", &sptree.nodes[i].pfossil[0],
		  &sptree.nodes[i].pfossil[1],
	  ...
	  break;
	case (S2N_F):
	  sscanf (pch, "%lf,%lf,%lf,%lf,%lf,%lf,%lf",
		  &sptree.nodes[i].pfossil[0],
		  &sptree.nodes[i].pfossil[1],
		  &sptree.nodes[i].pfossil[2],
		  &sptree.nodes[i].pfossil[3],
		  &sptree.nodes[i].pfossil[4],
...
static void GenerateGtree (int locus);

static void
ReadTreeSeqs (FILE * fout)
{
   ...
   */
  FILE *fseq, *ftree;
  int i, j, locus, clean0 = com.cleandata;	//CHHS Variable int h unused and removed
  double tailL = 0.025, tailR = 0.025, p_LOWERBOUND = 0.1, c_LOWERBOUND = 1.0;
  ...
  if (i)
	...
	if (i != tree.root && nodes[i].branch > 0)
	  {
	    j++;
	  }
      ...
      if (j == tree.nbranch)
	printf
	  ("\aTree with fossil calibrations should not have branch lengths!");
    }
  ...
  for (i = 0; i < tree.nnode; i++)
    if (nodes[i].label < 0)
      {
	nodes[i].label = 0;	/* change -1 into 0 */
      }
  ...
  if ((gnodes =
       (struct TREEN **) malloc (sizeof (struct TREEN *) * data.ngene)) ==
      NULL)
    {
      error2 ("oom");
    }
  ...
  for (locus = 0; locus < data.ngene; locus++)
	...
	{
	  if (com.sspace <
	      max2 (com.ngene + 1, com.ns) * (64 + 12 + 4) * sizeof (double))
	    {
	      com.sspace =
		max2 (com.ngene + 1,
		      com.ns) * (64 + 12 + 4) * sizeof (double);

	      if ((com.space =
		   (double *) realloc (com.space, com.sspace)) == NULL)
		{
...
static void copySptree (void);

static void
GenerateGtree (int locus)
{
   ...
   */
  int ns = data.ns[locus], i, j, /*ipop[NS], */ keep[NS], newnodeNO[2 * NS - 1];	//CMV ipop set but never used
  ...
  for (i = 0; i < ns; i++)
    {
      for (j = 0; j < sptree.nspecies; j++)
	if (!strcmp (com.spname[i], sptree.nodes[j].name))
	  {
	    break;
	  }
  ...
  for (i = 0; i < sptree.nnode; i++)
    if (newnodeNO[i] != -1)
      {
	nodes[newnodeNO[i]].ipop = i;
      }

  /* printGtree(0);  */

  gnodes[locus] =
    (struct TREEN *) malloc ((ns * 2 - 1) * sizeof (struct TREEN));

  if (gnodes[locus] == NULL)
    {
      error2 ("oom gtree");
    }
...
copySptree (void)
{
  ...
  nodes = nodes_t;
  com.ns = sptree.nspecies;
  tree.root = sptree.root;
  tree.nnode = sptree.nnode;
  tree.nbranch = sptree.nbranch;

  for (i = 0; i < sptree.nnode; i++)
    {
      if (i < com.ns)
	{
	  com.spname[i] = sptree.nodes[i].name;
	}
      ...
      for (j = 0; j < nodes[i].nson; j++)
	{
      ...
      if (i != tree.root)
	nodes[i].branch =
	  sptree.nodes[nodes[i].father].age - sptree.nodes[i].age;
    }
}
...
GetMemPUVR (int nc, int nUVR)
{
  ...
  PMat =
    (double *) malloc ((nc * nc + nUVR * nc * nc * 2 + nUVR * nc) *
		       sizeof (double));

  if (PMat == NULL)
    {
      error2 ("oom getting P&U&V&Root");
    }
  ...
  for (i = 1; i < nUVR; i++)
    {
      _UU[i] = _UU[i - 1] + nc * nc * 2 + nc;
      _VV[i] = _VV[i - 1] + nc * nc * 2 + nc;
...
GetUVRoot_codeml (void)
{
  ...
  if (com.seqtype == 1 && (!com.fix_kappa || !com.fix_omega))
    {
      nUVR = 1;
    }
  ...
  if (nUVR > 6)
    {
      error2 ("The maximum number of proteins is set to 6.");
    }
  ...
  if (com.seqtype == 2)
      ...
      for (locus = 0; locus < data.ngene; locus++)
	{
	  ...
	  EigenQcodon (0, -1, NULL, NULL, NULL, _Root[locus], _UU[locus],
		       _VV[locus], &mr, &com.kappa, com.omega, PMat);
	}
    }
}
...
static int
UseLocus (int locus, int copycondP, int setmodel, int setSeqName)
{
  /* This point nodes to the gene tree at locus gnodes[locus] and set com.z[]
     etc. for likelihood calculation for the locus.
   */
  int i;
  size_t nS, ii;		//CMV added ii to remove signed/unsigned problems
  double mr = 0;
  ...
  if (setmodel)
      ...
      else if (com.model == REV)
	{
      ...
      if ((com.seqtype == 1 && com.codonf)
	  || (com.seqtype == 2 && com.model == 3))
	{
	  xtoy (data.pi[locus], com.pi, com.ncode);
	}
      ...
      else
	{
      ...
      if (com.alpha)
	DiscreteGamma (com.freqK, com.rK, com.alpha, com.alpha,
		       com.ncatG, DGammaMean);
...
GetMemBC (void)
{
  ...
  size_t maxsizeScale = 0, nS, sfhK = 0, s1, snode, jj;	//CMV added jj to remove signed/unsigned problems
  ...
  if (com.alpha)
    {
      ...
      if ((com.fhK = (double *) realloc (com.fhK, sfhK)) == NULL)
	{
	  error2 ("oom");
	}
    }
  ...
  if (sptree.nspecies > 20)
      ...
      for (locus = 0; locus < data.ngene; locus++)
	{
	  ...
	  com.NnodeScale = 0;
	  com.nodeScale = data.nodeScale[locus] =
	    (char *) malloc (tree.nnode * sizeof (char));
	  ...
	  if (com.NnodeScale)
	    {
	      ...
	      FOR (j, tree.nnode) if (com.nodeScale[j])
		{
		  printf (" %2d", j + 1);
		}
	...
	{
	  if ((com.nodeScaleF =
	       (double *) malloc (maxsizeScale * sizeof (double))) == NULL)
	    {
	      error2 ("oom nscale");
	    }

	  for (jj = 0; jj < maxsizeScale; jj++)
	    {
	      com.nodeScaleF[jj] = 0;
	    }
	}
...
static double nu_AHRS = 0.001, *varb_AHRS;


//double funSS_AHRS(double x[], int np); //CMV redundant


static double
lnLfunHeteroData (double x[], int np)
{
   ...
   */
  int i, k, locus;
  double lnL = 0, /*lnLt, */ *pbrate;	//CMV lnLt set but never used
  ...
  if (!com.fix_kappa)
    {
      FOR (locus, data.ngene) data.kappa[locus] = x[k++];
    }

  if (!com.fix_omega)
    {
      FOR (locus, data.ngene) data.omega[locus] = x[k++];
    }

  if (!com.fix_alpha)
    {
      FOR (locus, data.ngene) data.alpha[locus] = x[k++];
    }
  ...
  for (locus = 0, pbrate = x + com.ntime - 1; locus < data.ngene; locus++)
	...
	if (i != tree.root)
      ...
      //lnL += lnLt = com.plfun(x, -1); //CMV lnLt set but unused
      lnL += com.plfun (x, -1);
...
funSS_AHRS (double x[], int np)
{
  ...
  double b, be, t, t0, t1, r, rA, w, y, small = 1e-20, smallage =
    AgeLow[sptree.root] * small;
  ...
  for (locus = 0, lnLb = lnLr = 0; locus < data.ngene;
       varb += com.ns * 2 - 1, locus++)
    {
      UseLocus (locus, -1, 0, 0);
      ...
      if (data.fix_nu == 2)
	{
      ...
      else if (data.fix_nu == 3)
	{
	  nu = x[np - 1 - (data.ngene - 1 - locus)];
	}
      ...
      if (t0 + t1 < 1e-7)
	{
      ...
      nodes[root].label =
	(nodes[son0].label * t1 + nodes[son1].label * t0) / (t0 + t1);
	  ...
	  if (j == root)
	    {
	      ...
	      be = (nodes[j].age -
		    nodes[son0].age) * (nodes[root].label +
					nodes[son0].label) / 2 +
		(nodes[j].age - nodes[son1].age) * (nodes[root].label +
						    nodes[son1].label) / 2;
	    }
	  else
	    {
	      b = nodes[j].branch;
	      be = (nodes[pa].age - nodes[j].age) * (nodes[pa].label +
						     nodes[j].label) / 2;
	    }
      ...
      for (j = 0, lnLri = 0; j < tree.nnode; j++)
	{
	  ...
	  if (rA < small || t < small || r < small)
	    {
	      puts ("small r, rA, or t");
	    }
      ...
      if (data.fix_nu > 1)
	{
	  lnLri += -nu / nu_AHRS - log (nu);	/* exponential prior */
	}
...
SetBranchRates (int inode)
{
  ...
  if (inode < com.ns)
    nodes[inode].label =
      (nodes[inode].label + nodes[nodes[inode].father].label) / 2;
  else
    for (i = 0; i < nodes[inode].nson; i++)
      {
...
static void
GetInitialsClock6Step1 (double x[], double xb[][2])
{
  ...
  if (com.seqtype == 1 && !com.fix_omega)
    {
      com.np++;
    }
...
static void
GetInitialsClock56Step3 (double x[])
{
  /* This is for clock 5 or clock 6 step 3
   */
  int i, j, k = 0;		//CHHS Variable int naa=20 unused and removed
  ...
  com.np =
    com.ntime - 1 + (1 + !com.fix_kappa + !com.fix_omega +
		     !com.fix_alpha) * data.ngene;

  if (com.clock == 5)
    for (i = com.ntime - 1; i < com.np; i++)
      {
	x[i] = .2 + rndu ();
      }
  else if (com.clock == 6)
...
int
AdHocRateSmoothing (FILE * fout, double x[NS * 3], double xb[NS * 3][2],
		    double space[])
{
   ...
   */
  int model0 = com.model;	/* is this useful? *///CHHS Variable int ntime0=com.ntime unused and removed
  int fix_kappa0 = com.fix_kappa, fix_omega0 =
    com.fix_omega, fix_alpha0 = com.fix_alpha;
  int ib, son0, son1;
  double kappa0 = com.kappa, omega0 = com.omega, alpha0 =
    com.alpha, t0, t1, *varb;
  double f, e = 1e-8, pb = 0.00001, rb[] = { 0.001, 99 }, lnL, lnLsum = 0;
  double mbrate[20], Rj[20], r, minr, maxr, beta, *pnu =
    &nu_AHRS, nu, mr[NGENE];
  ...
  for (locus = 0; locus < data.ngene; locus++)
    {
      if (!com.fix_kappa)
	{
	  data.kappa[locus] = com.kappa;
	}

      if (!com.fix_omega)
	{
	  data.omega[locus] = com.omega;
	}

      if (!com.fix_alpha)
	{
	  data.alpha[locus] = com.alpha;
	}
    }

  for (locus = 0, varb = varb_AHRS; locus < data.ngene;
       varb += com.ns * 2 - 1, locus++)
    {
      UseLocus (locus, -1, 1, 1);
      ...
      else
	{
	  ...
	  for (j = 0; j < com.ns * 2 - 1; j++)
	    {
	      ...
	      if (j != tree.root)
		varb[j] = (x[ib] > 1e-8 ? -1 / varb_minbranches[ib] : 999);
	    }
      ...
      if (!com.fix_kappa)
	{
	  data.kappa[locus] = x[com.ntime];
	}

      if (!com.fix_omega)
	{
	  data.omega[locus] = x[com.ntime + !com.fix_kappa];
	}

      if (!com.fix_alpha)
	data.alpha[locus] = x[com.ntime + !com.fix_kappa + !com.fix_omega];
      ...
      printf
	("   Locus %d: %d sequences, %d blengths, lnL = %15.6f mr=%.5f%10s\n",
      ...
      for (i = 0; i < com.np; i++)
	{
      ...
      for (i = 0; i < tree.nbranch; i++)
	{
	  fprintf (fout, " %8.5f", sqrt (varb[tree.branches[i][1]]));
	}
      ...
      for (i = 0; i < tree.nbranch; i++)
	{
      ...
      for (i = 0; i < tree.nbranch; i++)
	{
	  fprintf (fBV, " %12.9f", sqrt (varb[tree.branches[i][1]]));
	}
  ...
  if (com.np > NS * 6)
    {
      error2 ("change NP for ad hoc rate smoothing.");
    }
  ...
  if (!nodes[tree.root].fossil)
    {
      xb[0][0] = AgeLow[tree.root] * 1.0001;
      xb[0][1] = max2 (AgeLow[tree.root] * 10., 50.);
    }
  ...
  for (locus = 0, k = k0 = com.ntime - 1; locus < data.ngene;
       k0 += data.nbrate[locus++])
    {
      ...
      for (i = 0; i < tree.nnode; i++, FPN (fout))
	{
      ...
      if (data.nbrate[locus] > maxnbrate)
	{
	  error2 ("too many rate classes?  Change source.");
	}
      ...
      for (i = 0; i < tree.nnode; i++)
	{
	  ...
	  for (j = 0; j < i; j++)
	    if (j != tree.root)
	      fprintf (fdist, " %9.6f",
		       fabs (nodes[i].label - nodes[j].label));
       ...
       */
      beta =
	pow (1 / (data.nbrate[locus] + 1.), 1 / (data.nbrate[locus] - 1.));
      ...
      if (beta > 1)
	{
      ...
      for (j = 0; j < data.nbrate[locus]; j++)
	Rj[j] =
	  minr + (maxr - minr) * pow (beta, data.nbrate[locus] - 1. - j);
      ...
      if (j)
	{
	  ...
	  for (j = 0, Rj[data.nbrate[locus] - 1] = maxr;
	       j < data.nbrate[locus] - 1; j++)
	    {
	      scanf ("%lf", &Rj[j]);
	    }
	}
  ...
  com.fix_kappa = fix_kappa0;
  com.kappa = kappa0;
  com.fix_omega = fix_omega0;
  com.omega = omega0;
  com.fix_alpha = fix_alpha0;
  com.alpha = alpha0;
...
DatingHeteroData (FILE * fout)
{
   ...
   */
  char timestr[64];
  int i, j, k, s, np, locus;	//CHHS Variable int sconP0=0 unused and removed
  ...
  for (j = sptree.nspecies, com.ntime = j - 1, sptree.nfossil = 0;
       j < sptree.nnode; j++)
    {
  ...
  com.sspace = max2 (com.sspace, (size_t) spaceming2 (maxnpADRS));
  ...
  j = ming2 (noisy > 2 ? frub : NULL, &lnL, lnLfunHeteroData, NULL, x, xb,
	     com.space, e, np);
  ...
  if (com.getSE)
    {
      ...
      for (i = 0; i < np; i++)
	fprintf (fout, " %9.5f",
		 (var[i * np + i] > 0. ? sqrt (var[i * np + i]) : -1));

      FPN (fout);

      if (com.getSE == 2)
	{
	  matout2 (fout, var, np, np, 15, 10);
	}
    }
  ...
  for (j = 0, k = com.ntime - 1; j < data.ngene; j++, FPN (fout))
      ...
      for (i = 0; i < data.nbrate[j]; i++, k++)
	{
	  ...
	  if (com.getSE)
	    {
	      fprintf (fout, " +- %.5f", sqrt (var[k * np + k]));
	    }
	}
