...
extern "C"
{
//#include "cblas.h" //CHHS cblas not used; take care of row major order scheme for BLAS and LAPACK functions!
}
static int sortwM3 (double x[]);
static void DetailOutput (FILE * fout, double x[], double var[]);
static int GetOptions (char *ctlf);
static int AA2Codonf (double faa[20], double fcodon[]);
static int DistanceMatAA (FILE * fout);
static int SelectionCoefficients (FILE * fout, double kappa[], double ppi[],
				  double omega);
static int Qcodon2aa (double Qc[], double pic[], double Qaa[], double piaa[]);
static int SetAA1STEP (void);
static void GetOmegaAA (int OmegaAA[]);
static int VariancedSdN (double t, double omega, double vtw[2 * 2],
			 double vdSdN[2 * 2]);
static int lfunNSsites_rate (FILE * fout, double x[], int np);
static int lfunNSsites_M2M8 (FILE * frst, double x[], int np);
static int lfunNSsites_AC (FILE * frst, double x[], int np);
static double CDFdN_dS (double x, double par[]);
static int DiscreteNSsites (double par[]);
...
main (int argc, char *argv[])
{
  ...
  char pairfs[6][32] =
    { "2NG.dS", "2NG.dN", "2NG.t", "2ML.dS", "2ML.dN", "2ML.t" };
  char ctlf[96] = "codeml.ctl", timestr[64];	//CMV as is
  const char *pmodel;		//CMV added const
  const char *seqtypestr[3] = { "CODONML", "AAML", "CODON2AAML" };	//CHHS const added
  const char *Mgenestr[] = { "diff. rate", "separate data", "diff. rate & pi",
...
//CHHS  seed = 840863437;               //CHHS This is a fixed (useful) seed for testing output
  printf ("seed is %d\n", seed);
  SetSeed (seed);
  ...
  fprintf (frst,
	   "Supplemental results for CODEML (seqf: %s  treef: %s)\n",
	   com.seqf, com.treef);
  ...
  for (idata = 0; idata < com.ndata; idata++)
      ...
      if (nnsmodels > 1)
	{
	  if (com.seqtype != 1)
	    {
	      error2 ("batch run of site models requires codon seqs.");
	    }
      ...
      if (com.ngene > 1)
	{
	  ...
	  if (com.model)
	    {
	      error2 ("NSbranchsites with ngene.");
	    }

	  if (com.NSsites)
	    {
	      error2 ("NSsites with ngene.");
	    }
      ...
      if ((nodes = (struct TREEN *) malloc (i)) == NULL)
	{
      ...
      pmodel =
	(com.seqtype ==
	 CODONseq ? NSbranchmodels[com.model] : aamodels[com.model]);
      fprintf (fout, "%s (in %s)  %s\n", seqtypestr[com.seqtype - 1],
	       VerStr, com.seqf);
      fprintf (fout, "Model: %s for branches", pmodel);

      if (com.clock)
	{
	  fprintf (fout, " %s ", clockstr[com.clock]);
	}
      ...
      if (com.seqtype == CODONseq)
	{
	  ...
	  if (com.alpha)
	    fputs
	      ("Warning: Gamma model for codons.  See documentation.", fout);
	}

      if ((com.seqtype == CODONseq || com.model == FromCodon)
	  && (com.aaDist && com.aaDist < 10 && com.aaDist != AAClasses))
	fprintf (fout, "%s, %s\n", com.daafile,
		 (com.aaDist > 0 ? "geometric" : "linear"));
      ...
      if (com.NSsites)
	{
      ...
      com.sspace =
	max2 (5000000UL, 3UL * com.ncode * com.ncode * sizeof (double));

      if (com.NSsites)
	{
	  if (com.sspace <
	      2 * com.ncode * com.ncode + 4 * com.npatt * sizeof (double))
	    com.sspace =
	      2 * com.ncode * com.ncode + 4 * com.npatt * sizeof (double);
	}
	...
	{
	  printf ("\nfailed to get %9lu bytes for space", (long unsigned int) com.sspace);	//CHHS cast to avoid warning in gcc 32 Bit
	  error2 ("oom space");
	}
      ...
      else
	{
	  ...
	  printf ("\n%9u bytes for distance", (unsigned int) (com.ns * (com.ns - 1) / 2 * sizeof (double)));	//CHHS made parameter 2 fit to %9u
	  printf ("\n%9u bytes for conP\n", (unsigned int) com.sconP);	//CHHS made parameter 2 fit to %9u
	  printf ("%9u bytes for fhK\n%9u bytes for space\n", s2, (unsigned int) com.sspace);	//CHHS made parameter 3 fit to %9u
...
Forestry (FILE * fout)
{
  ...
  double x[NP], xb[NP][2], xcom[NP - NBRANCH], lnL = 0, lnL0 = 0, e =
    1e-8, tl = 0, nchange = -1;
  ...
  if (com.seqtype == 1 && com.aaDist >= FIT1)
    {
      ...
      zero (paa0, 20);
      FOR (i, com.ncode) paa0[GeneticCode[com.icode][FROM61[i]]] +=
	pcodon0[i];
      pcodonClass = (double *) malloc (com.ncatG * 64 * sizeof (double));

      if (pcodonClass == NULL)
	{
	  error2 ("oom pcodonClass");
	}
    }
  ...
  for (itree = 0; ntree == -1 || itree < ntree; itree++, iteration = 1)
      ...
      if (com.print)
	{
      ...
      if (com.fix_blength == 2 && !haslength)
	{
	  error2 ("no branch lengths in tree");
	}

      if (com.fix_blength > 0 && !haslength)
	{
	  com.fix_blength = 0;
	}
	...
	{
	  OutTreeN (F0, 0, 0);
	  printf ("   MP score: %.0f", nchange);
	}

      OutTreeN (fout, 0, 0);
      fprintf (fout, "   MP score: %.0f", nchange);
      ...
      if (i == -1)
	{
      ...
      if (np > NP || np - com.ntime > NP - NBRANCH)
	{
	  error2 ("raise NP");
	}
      ...
      if (com.sspace < spaceming2 (np))
	{
	  ...
	  printf ("\nspace adjusted to %9u bytes\n", (unsigned int) com.sspace);	//CHHS made parameter 2 fit to %9u

	  if ((com.space =
	       (double *) realloc (com.space, com.sspace)) == NULL)
	    {
	      printf ("\ntrying to get %d bytes for ming2", (int) com.sspace);	//CHHS made parameter 2 fit to %9u
	      error2 ("oom space");
	    }
	}
      ...
      if (iteration && np)
	{
	  if (com.method == 1)
	    ...
	    j = ming2 (noisy > 2 ? frub : NULL, &lnL, com.plfun, NULL,
		       x, xb, com.space, e, np);

	  if (j == -1 || lnL <= 0 || lnL > 1e7)
	    {
	      status = -1;
	    }
	  else
	...
	{
	  lnL0 = lnL;
	  FOR (i, np - com.ntime) xcom[i] = x[com.ntime + i];
	}
      else if (!j)
	for (i = 0; i < np - com.ntime; i++)
	  {
	    xcom[i] = xcom[i] * .2 + x[com.ntime + i] * 0.8;
	  }
	      ...
	      || com.NSsites == NSfreqs || com.NSsites == NS3normal))
	{
	  ...
	  if (com.nparK)	/* HMM model for w */
		...
		{
		  f_and_x_fromx (x + k, x + k, com.ncatG, 0);
		}
	    }
	  else
	    {
	      ...
	      if (com.model && com.model <= NSbranch2)
		{
		  j = 3;
		}

	      f_and_x_fromx (x + k, x + k, j, 0);
	    }
	}
	...
	{
	  fprintf (fout,
		   "Note: Branch length is defined for the first gene (site partition).\n");
...
double *
PointOmega (double xcom[], int igene, int inode, int isiteclass)
{
  ...
  int nka = (com.hkyREV ? 5 : 1), nw =
    (com.aaDist == AAClasses ? com.nOmegaType : 1);
  ...
  if (com.NSsites == 0 && com.model == 0)	/* simple case: one ratio */
      ...
      if (com.ngene <= 1)
	{
	  if (com.fix_omega)
	    {
	      return (&com.omega_fix);	/* fix_omega */
	    }
	  else
	    {
	    }			//CHHS Semicolon substituted by pair of brackets
	}
      else if (com.Mgene >= 3)
	{
	  k += igene * (nka + nw) + nka;
...
sortwM3 (double x[])
{
  ...
  double space[NCATG];
  int wspace[NCATG];		//CMV added to remove cray warning
  ...
  if (fabs (1 - sum (com.freqK, com.ncatG)) > 1e-6)
    {
      error2 ("sortwM3: freqK");
    }

  if (com.nparK)
    {
      puts ("\asortwM3 for HMM not implemented yet..");
      return -1;
    }

  indexing (com.rK, com.ncatG, index, 0, wspace);
...
static void
printParametersNSsites (FILE * fout, double x[])
{
  ...
  if (com.model == 0)
      ...
      for (i = 0; i < com.ncatG; i++)
	{
      ...
      for (i = 0; i < com.ncatG; i++)
	{
	  fprintf (fout, " %8.5f", com.rK[i % 2]);
	}

      fprintf (fout, "\nforeground w   ");

      for (i = 0; i < com.ncatG - 2; i++)
	{
	  fprintf (fout, " %8.5f", com.rK[i % 2]);
	}
      ...
      for (i = 0; i < com.nbtype; i++)
	{
	  ...
	  w[i][com.ncatG - 1] = (i == com.nbtype - 1
				 && com.fix_omega ? com.omega_fix : x[k++]);
	}
...
static void
DetailOutput (FILE * fout, double x[], double var[])
{
  /* var[] is used for codon models if com.getSE=1 to calculate the variances
     of dS and dN.
   */
  int i, j, k = com.ntime, np = com.np, npclass;	//CHHS int variable ibtype currently not in use, removed
  double om = -1, N = -1, S = 0, dN = 0, dS = 0, dSt, dNt, mr =
    0, vtw[4], vSN[4], omclass[NCATG];
  double t, *tdSdNb = NULL, y;	//CHHS double variables phi1=0,phi2=0 not in use, removed
  //CHHS   double mu[3]={0,1,2},sig[3]={-1}; /* 3normal: mu0=0 fixed. mu2 estimated */ //CHHS variables currently not in use
  double fb3x4[12];
  ...
  if (com.seqtype == CODONseq || com.model == FromCodon)
      ...
      else if (!com.fix_kappa && com.Mgene <= 2)
	{
      ...
      if (com.npi)
	{
	  if (com.codonf == F1x4 || com.codonf == F1x4MG
	      || com.codonf >= FMutSel0)
	    {
	      for (j = 0, fb3x4[3] = 1; j < 3; j++)
		{
		  fb3x4[j] = x[k + j];
		}
      ...
      if (com.NSsites && com.model == 0)
	{
	  for (j = 0, dS = dN = 0; j < com.ncatG; j++)
	      ...
	      if (com.aaDist)
		{
	      ...
	      mr = -1;
	      EigenQcodon (1, 1, &S, &dSt, &dNt, NULL, NULL, NULL, &mr,
			   com.pkappa, com.rK[j], PMat);
      ...
      if (!com.fix_omega && com.NSsites == 0 && com.model == 0
	  && com.aaDist != 7 && com.Mgene <= 2)
	{
	  fprintf (fout, "\nomega (dN/dS) = %8.5f\n", x[k++]);
	}
      ...
      if (com.NSsites >= NSgamma)
	{
	  ...
	  if (com.NSsites == NSgamma)
	    ...
	    fprintf (fout,
		     " p0=%9.5f  p=%9.5f q=%9.5f\n (p1=%9.5f) w=%9.5f\n",
		     x[k], x[k + 1], x[k + 2], 1 - x[k],
		     (com.fix_omega ? com.omega : x[k + 3]));
	    ...
	    fprintf (fout,
		     "p0=%9.5f  p1=%9.5f  u2=%9.5f  s1=%9.5f  s2=%9.5f\n",
		     x[k], x[k + 1], x[k + 2], x[k + 3], x[k + 4]);
	  else if (com.NSsites == NS3normal)
	    fprintf (fout,
		     "p0=%9.5f  p1=%9.5f (p2=%9.5f)\n u2=%9.5f  s0=%9.5f  s1=%9.5f  s2=%9.5f\n",
		     x[k], x[k + 1], 1 - x[k] - x[k + 1], x[k + 2],
		     x[k + 3], x[k + 4], x[k + 5]);
	}
      ...
      else if (com.aaDist && com.aaDist >= 11)	/* fitness, one class */
	{
	  ...
	  k = com.ntime + com.nrgene + com.nkappa + com.npi;
	  FOR (i, 4 + (com.aaDist == FIT2)) fprintf (fout, " %9.5f", x[k++]);
	  FPN (fout);
	}
  ...
  for (j = 0; j < com.nalpha; j++)
    {
      if (!com.fix_alpha)
	fprintf (fout, "\nalpha (gamma, K = %d) = %8.5f", com.ncatG,
		 (com.alpha = x[k++]));
  ...
  if (com.rho)
    {
      if (!com.fix_rho)
	{
	  fprintf (fout, "rho (correlation) = %8.5f\n", x[k]);
	}

      fprintf (fout, "transition probabilities between rate categories:\n");

      for (i = 0; i < com.ncatG; i++, FPN (fout))
	FOR (j, com.ncatG)
  ...
  if (com.aaDist == AAClasses)
      ...
      if (com.model == 0)
	{
	  ...
	  for (k = 0; k < com.nOmegaType; k++)
	    {
	      fprintf (fout, " %9.5f: ",
		       x[com.ntime + com.nrgene + com.nkappa + k]);

	      for (i = 0; i < 20; i++)
		for (j = 0; j < i; j++)
		  if (OmegaAA[i * (i - 1) / 2 + j] == k)
		    {
		      fprintf (fout, " %c%c", AAs[i], AAs[j]);
		    }

	      if (k == 0)
		{
		  fprintf (fout, " (background ratio)");
		}
  ...
  if (com.seqtype == CODONseq && com.ngene == 1
	...
	{
	  fprintf (fout, "%4d..%-3d ", tree.branches[i][0] + 1,
		   tree.branches[i][1] + 1);
	  k = com.ntime + com.nrgene + com.nkappa + com.npi;
	  /*       if(com.codonf >= FMutSel0)
	  ...
	  if (com.NSsites == 0)
	    {
	      if (com.aaDist)
		{
		  om = -1;	/* not used in EigenQcodon() */
		}
	      else if (com.model == 0 || com.model == FromCodon)
		{
		  om = (com.fix_omega ? com.omega : x[k]);
		}
	      else if (com.model == NSbranchB)
		{
		  om = x[k + i];
		}
	      else if (com.model == NSbranch2)
		{
		  om = nodes[tree.branches[i][1]].omega;
		}

	      if (com.model && com.aaDist)
		com.pomega =
		  x + com.ntime + com.nrgene + !com.fix_kappa +
		  com.npi +
		  ...
		  && com.aaDist != AAClasses)
		{
		  ...
		  fprintf (fout,
			   "  dN = %7.4f +- %.4f dS = %7.4f +- %.4f", dN,
			   (vSN[3] > 0 ? sqrt (vSN[3]) : -0), dS,
			   (vSN[0] > 0 ? sqrt (vSN[0]) : -0));
	    ...
	    {
	      fprintf (fout,
		       "%9.3f %8.1f %8.1f %8.4f %8.4f %8.4f %6.1f %6.1f\n",
...
ReadNSsitesModels (char *line)
{
  ...
  for (nnsmodels = 0; nnsmodels < nNSsitesModels; nnsmodels++)
    {
      if (sscanf (pline, "%d", &nsmodels[nnsmodels]) != 1)
	{
	  break;
	}
      ...
      for (pop_digit = 0;;)
	{
	  if (isdigit (*pline))
	    ...
	    {
	      error2 (".ctl file NSsites line strange.");
	    }
	}
...
static int
GetOptions (char *ctlf)
{
  int iopt, i, j, nopt = 36, lline = 255;
  char line[255], *pline, opt[99];
  const char *comment = "*#";	//CMV added const
  const char *optstr[] = {
    "seqfile", "outfile", "treefile", "seqtype", "noisy",
    "cleandata", "runmode", "method",
    "clock", "getSE", "RateAncestor", "CodonFreq", "estFreq",
    "verbose",
  ...
  const char *daafiles[] = { "", "grantham.dat", "miyata.dat",
      ...
      for (iopt = 0; iopt < nopt; iopt++)
	{
	  if (strncmp (opt, optstr[iopt], 8) == 0)
		...
		case (17):
		  ...
		  if (com.seqtype == 2 && com.ndata > 1
		      && (com.clock == 5 || com.clock == 6))
		    {
		...
		case (20):
		  ...
		  if (com.seqtype == 1 && (com.clock == 5 || com.clock == 6))
			...
			if (!sscanf
			    (pline + markgenes[j], "%d", &data.icode[j]))
			  {
			    break;
			  }
		...
		case (23):
		  ...
		  if (com.seqtype == 1 && com.fix_kappa
		      && (com.clock == 5 || com.clock == 6))
		    {
		      ng = splitline (++pline, markgenes);
		      ...
		      for (j = 0; j < min2 (ng, com.ndata); j++)
			if (!sscanf
			    (pline + markgenes[j], "%lf", &data.kappa[j]))
			  {
			    break;
			  }
		...
		case (25):
		  ...
		  if (com.seqtype == 1 && com.fix_omega
		      && (com.clock == 5 || com.clock == 6))
		    {
		      ...
		      for (j = 0; j < min2 (ng, com.ndata); j++)
			if (!sscanf
			    (pline + markgenes[j], "%lf", &data.omega[j]))
			  {
			    break;
			  }
		...
		case (27):
		  ...
		  if (com.fix_alpha && t
		      && (com.clock == 5 || com.clock == 6))
		    {
		      ...
		      for (j = 0; j < min2 (ng, com.ndata); j++)
			if (!sscanf
			    (pline + markgenes[j], "%lf", &data.alpha[j]))
			  {
			    break;
			  }
  ...
  if ((com.fix_kappa || (com.fix_alpha && com.alpha))
      && (com.clock == 5 || com.clock == 6))
    {
  ...
  if (com.seqtype == AAseq || com.seqtype == CODON2AAseq)
    {
      if (com.NSsites)
	{
	  error2 ("use NSsites=0 for amino acids?");
	}
	...
	case (FromCodon):
	  ...
	  if (com.codonf)
	    {
	      com.codonf = 0;
	      puts ("CodonFreq=0 reset for model=6.");
	    }
	...
	case (REVaa):
	  ...
	  break;
	default:
	  error2 ("model unavailable");
	}

      if (com.Mgene > 2
	  || (com.Mgene == 2 && (com.model == Fequal || com.model == 2)))
	{
	  error2 ("Mgene && model");
	}

      if (com.seqtype == 2 && com.model != FromCodon
	  && com.model != AAClasses)
	{
	  com.fix_kappa = com.fix_omega = 1;
	  com.kappa = com.omega = 0;
	}
    }
  else if (com.seqtype == CODONseq)
    {
      if (com.nparK)
	if (com.model || com.aaDist || com.NSsites != NSdiscrete
	    || com.alpha || com.rho)
	  {
	    error2 ("HMM model option");
	  }

      if (com.Mgene > 1 && com.model)
	{
	  error2 ("Mgene & model?");
	}
	...
	error2
	  ("runmode = -2 not implemented for codon models with frequencies");
      ...
      if (com.hkyREV && (com.aaDist || com.Mgene > 1))
	{
      ...
      if (com.NSsites < 0 || com.NSsites > 13)
	{
      ...
      if (com.aaDist != AAClasses)
	{
	  ...
	  if (com.runmode == -2 && (com.NSsites || com.alpha || com.aaDist))
	    error2
	      ("err: incorrect model for pairwise comparison.\ncheck NSsites, alpha, aaDist.");

	  if (com.runmode > 0 && com.model == 2)
	    {
	      error2 ("tree search & model");
	    }
	  ...
	  if (com.aaDist == 0)
	  ...
	  else
	    {
	      if (com.aaDist <= 6)
		{
		  com.nrate += 2;	/* a & b, PSB2000 */
		}
	      else if (com.aaDist == FIT1)
		{
		  com.nrate += 4;	/* fitness models: */
		}
	      else if (com.aaDist == FIT2)
		{
		  com.nrate += 5;	/* ap, p*, av, v*, b */
		}
		...
		for (i = 0; i < 2; i++)
		  for (j = 0; j < 20; j++)
		    {
		      AAchem[i][j] /= AAchem[i][20];
		    }
	    }

	  if (com.Mgene >= 3 && com.nrate == 0)
	    {
	      error2 ("Mgene");
	    }
	  ...
	  if (com.NSsites)
		...
		if ((com.model != 2 && com.model != 3)
		    || (com.NSsites != NSpselection
			&& com.NSsites != NSdiscrete))
		  error2 ("only NSsites=2,3 & model=2,3 are compatible.");

	      if (com.alpha || com.fix_alpha == 0)
		{
	      ...
	      if (com.model == 2)	/* branchsite models A & B */
		{
		  if (com.ncatG != 3)
		    {
		      puts ("\abranch-site model: use ncatG=3 only.");
		    }
	      ...
	      else if (com.NSsites == NSdiscrete && com.aaDist)
		{
		  if (com.aaDist <= 6)
		    {
		      com.nrate += com.ncatG;	/* a&b PSB2000 */
		    }
		  else		/* fitness models */
		    {
  ...
  if (com.runmode == -2 && com.cleandata == 0)
    {
      ...
      if (noisy)
	{
	  puts ("gaps are removed for pairwise comparison.");
	}
    }
  ...
  if (com.method && (com.clock || com.rho))
    {
      com.method = 0;
      puts ("Iteration method reset: method = 0");
    }

  if (com.method && com.seqtype == 2 && com.model == FromCodon)
    {
      com.method = 0;
      puts ("\awork on method = 1 for model = 6");
    }
  ...
  if (com.runmode == 3 && (com.clock))
    {
      error2 ("runmode+clock");
    }
  ...
  if (com.alpha || com.NSsites)
    {
      if (com.ncatG < 2 || com.ncatG > NCATG)
	{
	  error2 ("ncatG");
	}
    }
  ...
  if ((com.NSsites == NSgamma || com.NSsites == NS2gamma
       || com.NSsites >= NSbetagamma) && com.ncatG < 10)
    {
      error2 ("need more categories for NSsites");
    }
  ...
  if (com.bootstrap && com.ndata != 1)
    {
      error2 ("ndata=1 for bootstrap.");
    }
...
///
int
testx (double x[], int np)
{
  //double tb[] = { .4e-6, 99 };
  double tb0 = .4e-6, tb1 = 99.;

  //FOR(i, com.ntime)
  for (int i = 0; i < com.ntime; ++i)
    if (x[i] < tb0 || x[i] > tb1)
      return -1;
...
int
SetxBound (int np, double xb[][2])
{
  int i = -1, j, k, K = com.ncatG;
  double rgeneb[] = { 0.01, 99 }, rateb[] =
  {
  1e-4, 999};			//CHHS double variables tb[]={4e-6,50} and tb0=1e-8 not used, removed
  double alphab[] = { 0.002, 99 }, betab[] =
  {
  0.005, 99}, omegab[] =
  {
  0.0001, 999};
  double rhob[] = { 0.01, 0.99 }, pb[] =
  {
  .00001, .99999};
  ...
  for (i = 0; i < com.nrgene; i++)
    for (j = 0; j < 2; j++)
      {
	xb[com.ntime + i][j] = rgeneb[j];
      }

  for (i = 0; i < com.nrate; i++)
    for (j = 0; j < 2; j++)
      {
	xb[com.ntime + com.nrgene + i][j] = rateb[j];
      }
  ...
  if (com.seqtype == CODONseq && com.npi > 3
      && (com.codonf == Fcodon || com.codonf == FMutSel0
	  || com.codonf == FMutSel))
    {
      for (; j < com.npi; j++)
	{
  ...
  if (com.aaDist < 0 && (com.seqtype == 1 || com.model == FromCodon))
    {
      ...
      if (com.nrate != !com.fix_kappa + 1 + (com.seqtype == 1))
	{
	  error2 ("in Setxbound");
	}
  ...
  for (i = 0; i < com.nalpha; i++, k++)
    {
      FOR (j, 2) xb[k][j] = alphab[j];
    }

  if (!com.fix_rho)
    {
      FOR (j, 2) xb[np - 1][j] = rhob[j];
    }
...
static void
getpcodonClass (double x[], double pcodonClass[])
{
  ...
  FOR (iclass, nclass)
  {
    ...
    if (fabs (1 - sum (pcodonClass + iclass * 64, com.ncode)) > 1e-5)
      {
	error2 ("pcodon!=1");
      }
  ...
  if (nclass == 1)
    {
      FOR (i, com.ncode) com.pi[i] = pcodonClass[i];
    }
}
...
static int
GetInitialsCodon (double x[])
{
  /* This sets the initials and count com.np for codon models.
   */
  int k = com.ntime + com.nrgene, i, j = 0, K = com.ncatG, nsyncodon[20];	//CMV Initialized j to mask the bug below
  double mr = 0;
	  ...
	  if (com.codonf == FMutSel0 || com.codonf == FMutSel)
	      ...
	      if (com.npi > 3 && com.codonf == FMutSel0)
		{
		  ...
		  for (i = 0; i < 20 - 1; i++)	/* amino acid fitness, ignoring nsyncodon */
		    x[k++] =
		      log ((com.piAA[i] / nsyncodon[i] +
			    .001) / (com.piAA[19] / nsyncodon[19] +
				     .002 * rndu ()));
		}
	      else if (com.npi > 3 && com.codonf == FMutSel)
		{
		  for (i = 0; i < com.ncode - 1; i++)	/* codon fitness */
		    x[k++] =
		      log ((com.pi[i] +
			    .001) / (com.pi[com.ncode - 1] + .002 * rndu ()));
		}
	    }
	  else if (com.npi)
	    {
  ...
  if (com.model && com.model <= NSbranch3)	/* branch models */
      ...
      if (com.NSsites == 0)
	{
	  ...
	  if (com.aaDist == 0)
	    ...
	    com.nrate +=
	      (com.model ==
	       NSbranchB ? tree.nbranch : com.nOmega - 1 + !com.fix_omega);
  ...
  if (com.NSsites == 0 && com.nrate == 0)
    EigenQcodon (0, -1, NULL, NULL, NULL, Root, U, V, &mr,
		 (com.nkappa >
		  1 ? x + com.ntime + com.nrgene : &com.kappa),
		 com.omega, PMat);
  ...
  if (com.model && com.NSsites)
      ...
      if (com.model == NSbranch2)	/* branch-site models A & B */
	{
	  ...
	  if (com.NSsites == NSdiscrete)
	    ...
	    com.nrate =
	      com.nkappa + com.npi + 1 + !com.fix_omega + com.nbtype - 1 - 1;
	}
	...
	case (NSfreqs):	/* p0...pK.  w's are fixed  */
	  ...
	  for (i = 0; i < com.ncatG - 1; i++)
	    {
	      x[k++] = (com.ncatG - j) / 2.;	//CMV Who initializes j here?
	    }
...
int
GetInitials (double x[], int *fromfile)
{
  ...
  static int times = 0;
  int i, j, k = 0, naa = 20;
  //CHHS   int K=(com.model==2&&com.NSsites?com.ncatG-1:com.ncatG); //CHHS variable unused, removed
  size_t sconP_new =
    (size_t) (tree.nnode - com.ns) * com.ncode * com.npatt * sizeof (double);
  double t;
...
//CHHS    if (com.sconP < 0 || sconP_new < 0) //CHHS size_t always >= 0
//CHHS    {
//CHHS        error2("data set too large.");
//CHHS    }
  ...
  if (com.rho)
    ...
    DiscreteGamma (com.freqK, com.rK, com.alpha, com.alpha, com.ncatG,
		   DGammaMean);

  if (com.fix_blength == -1)
    for (i = 0; i < com.np; i++)
      {
	x[i] = (i < com.ntime ? .1 + 0.5 * rndu () : 0.5 + rndu ());
      }
  ...
  if (finitials)
    {
      ...
      if (com.runmode > 0 && fromfile && com.NSsites)
	{
	  LASTROUND = 1;
	}
    }
...
int
SetPGene (int igene, int _pi, int _UVRoot, int _alpha, double x[])
{
   ...
   */
  int nr1 = (com.nrate + 1) / com.ngene, k =
    com.nrgene + (com.Mgene >= 3) * igene * nr1;
  ...
  if (_UVRoot)
      ...
      if (com.seqtype == CODONseq)
	{
	  ...
	  if (!com.fix_omega)
	    ...
	    com.omega = (com.Mgene > 2
			 && igene <
			 com.ngene - 1 ? xcom[k++] : com.omega_fix);

	  if (!com.NSsites)
	    EigenQcodon (0, -1, NULL, NULL, NULL, Root, U, V, &mr,
			 (com.hkyREV
			  || com.codonf ==
			  FMutSel ? &xcom[com.nrgene] : &com.kappa),
			 com.omega, PMat);
	}
      else
	{
...
SetParametersNSsites (double x[])
{
  ...
  int K = com.ncatG, i, j;
  double w[NBTYPE][3], t, S, dS, dN, spaceP2PI[NCATG * (NCATG + 1)],
    small = 1e-4;
  double mr, f;

  if (com.NSsites == 0)
    {
      error2 ("SetParametersNSsites : strange.");
    }
  ...
  switch (com.NSsites)
	  ...
	  for (i = 0; i < K; i++, k += K - 1)
	    {
	      if (!LASTROUND)
		{
		  f_and_x_fromx (x + k, com.MK + i * K, K, 0);	/* x->f */
		}
	      else
		{
    ...
    case (NSfreqs):
      ...
      if (!LASTROUND)
	{
	  f_and_x_fromx (x + k, com.freqK, K, 1);	/* x->f */
	  k += K - 1;
	}
      else
	{
      ...
      else if (com.NSsites == NSpselection)
	{
	  com.rK[0] = x[k++];
	  com.rK[1] = 1;
	  com.rK[2] = (com.fix_omega
		       && com.model <= 2 ? com.omega_fix : x[k++]);
	}
  ...
  if (com.model == 0)		/* NSsites models */
      ...
      for (j = 0, Qfactor_NS = 0; j < com.ncatG; j++)
	{
	  if (com.aaDist)
		...
		{
		  com.pomega =
		    x + k + com.ncatG - 1 + j * (4 + (com.aaDist == FIT2));
		  xtoy (pcodonClass + j * 64, com.pi, com.ncode);
		}
	    }

	  mr = -1;
	  EigenQcodon (1, -1, &S, &dS, &dN, NULL, NULL, NULL, &mr,
		       com.pkappa, com.rK[j], PMat);
      ...
      for (i = 0; i < 2; i++)	/* i=0 back (2 site classes); i=1 fore (3 classes) */
	{
	  ...
	  for (j = 0; j < (i == 0 ? 2 : 3); j++)
	      ...
	      else if (j == 2)
		{
	      ...
	      if (NFunCall == 1)
		printf ("branch=%d  freq=%.6f w%d = %.6f\n", i, f, j,
			com.omega);

	      mr = -1;
	      EigenQcodon (1, -1, &S, &dS, &dN, NULL, NULL, NULL, &mr,
			   com.pkappa, com.omega, PMat);
	  ...
	  if (NFunCall == 1)
	    printf ("\t\t\tQfactor for branch %d = %.6f\n", i,
		    Qfactor_NS_branch[i]);
	}
      ...
      for (i = 0; i < 3; i++)	/* (w0,w1,w2) */
	{
	  ...
	  mr = 1;
	  EigenQcodon (0, -1, NULL, NULL, NULL, _Root[i], _UU[i], _VV[i],
		       &mr, com.pkappa, w[1][i], PMat);
	}
    }
  else				/* NSbranch3: Clade models C and D */
    {
...
int
Set_UVR_BranchSite (int iclass, int branchlabel)
{
  ...
  if (com.model <= NSbranch2)	/* branch-site models A & B */
    {
      if (branchlabel == 0)
	{
	  iUVR = iclass % 2;	/* back, w0 w1 */
	}
      else
	{
	  iUVR = (iclass <= 1 ? iclass : 2);	/* fore, w0 w1 w2 */
	}
    }
  else				/* clade models C & D */
    {
      if (iclass < com.ncatG - 1)
	{
	  iUVR = iclass;
	}
      else
	{
	  iUVR = com.ncatG - 1 + branchlabel;
	}
    }
...
GetCodonFreqs (void)
{
   ...
   */
  int n = com.ncode, i, j, ic, iaa, b[3];	//CHHS unused int variable k removed
  ...
  if ((com.codonf >= F1x4 && com.codonf <= F3x4MG) || com.npi > 3)
      ...
      for (i = 0; i < n; i++)
	{
	  ...
	  b[2] = ic % 4;
	  com.pi[i] =
	    com.pf3x4[b[0]] * com.pf3x4[4 + b[1]] * com.pf3x4[8 + b[2]];
	}
    }
...
SetParameters (double x[])
{
  ...
  if (com.clock && AbsoluteRate)
    {
      com.rgene[0] = x[0];	/* so that rgene are abs rates */
    }
	  ...
	  for (i = 0; i < com.nbtype; i++)
	    {
	      if (com.aaDist == AAClasses)
		com.pomega =
		  x + com.ntime + com.nrgene + com.nkappa + com.npi +
		  i * com.nOmegaType;
	      else
		w0 = (i == com.nOmega - 1
		      && com.fix_omega ? com.omega_fix : com.pomega[i]);

	      EigenQcodon (0, -1, NULL, NULL, NULL, _Root[i], _UU[i],
			   _VV[i], &mr, com.pkappa, w0, PMat);
	    }
	}
  ...
  /* branch models */
  if (com.seqtype == CODONseq && com.model && com.NSsites == 0
      && com.aaDist == 0)
    {
      FOR (j, tree.nnode)
      {
  ...
  if (!com.fix_rho)
    {
      com.rho = x[k++];
      AutodGamma (com.MK, com.freqK, com.rK, &t, com.alpha, com.rho,
		  com.ncatG);
    }
...
DiscreteNSsites (double par[])
{
   ...
   */
  int status = 0, j, off, K = com.ncatG - (com.NSsites == NSbetaw
					   || com.NSsites == NS02normal);
  double xb[2] = { 1e-7, 99 };	/* bounds for omega.  */
  int UseK1K2 = 0;		//CHHS Unused int variables K1=6, K2=4 removed
  double p, w0, lnbeta;		//CHHS Unused variable double p01=0 removed
  ...
  if ((com.NSsites == NSgamma || com.NSsites == NS2gamma
	...
	if (j < K1)
	  ...
	  {
	    p = p01 + (1 - p01) * ((j - K1) * 2. + 1) / (2. * K2);
	    w0 = 1.01 + (j - K1) / K2;
	  }

	com.rK[j] = InverseCDF (CDFdN_dS, p, w0, par, xb);
	com.freqK[j] = (j < K1 ? p01 / K1 : (1 - p01) / K2);
      thread}
    }
  ...
  if (!UseK1K2)			/* this is currently used */
      ...
      if (com.NSsites == NSbeta || com.NSsites == NSbetaw)
	{
	  ...
	  lnbeta =
	    LnGamma (par[off]) + LnGamma (par[off + 1]) -
	    LnGamma (par[off] + par[off + 1]);
  ...
  if (com.NSsites == NSbetaw)
    {
      if (!com.fix_omega)
	{
	  com.rK[com.ncatG - 1] = par[3];
	}
      else
	{
	  com.rK[com.ncatG - 1] = com.omega_fix;
	}

      com.freqK[K] = 1 - par[0];
      FOR (j, K) com.freqK[j] *= par[0];
    }
  ...
  if (com.NSsites >= NSgamma)
	...
	for (j = 1; j < com.ncatG; j++)
	  if (com.rK[j] + 1e-7 < com.rK[j - 1])
	    {
	      status = 1;
	    }
      ...
      if (status)
	{
	  ...
	  FOR (j, (com.NSsites == 7 ? 2 : 4)) printf (" %12.6f", par[j]);
	  FPN (F0);
	  FOR (j, com.ncatG) printf ("%13.5f", com.freqK[j]);
	  FPN (F0);
	  FOR (j, com.ncatG) printf ("%13.5e", com.rK[j]);
	  FPN (F0);
	}
    }
...
CDFdN_dS (double x, double p[])
{
  ...
  switch (com.NSsites)
    {
    case (NSgamma):
      cdf = CDFGamma (x, p[0], p[1]);
      break;
    case (NS2gamma):
      cdf =
	p[0] * CDFGamma (x, p[1], p[2]) + (1 - p[0]) * CDFGamma (x, p[3],
								 p[3]);
      break;
    case (NSbeta):
      cdf = CDFBeta (x, p[0], p[1], 0);
      break;
    case (NSbetaw):
      cdf = CDFBeta (x, p[1], p[2], 0);
      break;
    case (NSbetagamma):
      cdf =
	p[0] * CDFBeta (x, p[1], p[2], 0) + (1 - p[0]) * CDFGamma (x,
								   p[3],
								   p[4]);
      break;
    ...
    case (NSbeta1gamma):
      ...
      if (x <= 1)
	{
	  cdf = p[0] * CDFBeta (x, p[1], p[2], 0);
	}
      else
	{
	  cdf = p[0] + (1 - p[0]) * CDFGamma (x - 1, p[3], p[4]);
	}
    ...
    case (NS02normal):
      ...
      cdf =
	1 -
	f[1] * CDFNormal (-(x - mu[1]) / sig[1]) / CDFNormal (mu[1] /
							      sig[1])
	-
...
static void
GetSNphysical (double pi[], double *Sphysical, double *Nphysical, double *S4)
{
...
static double
GetOmega (int aa1, int aa2, double omega, double pomega[])
{
  ...
  if (com.aaDist == AAClasses)
	...
	{
	  if (noisy)
	    printf ("aa1 & aa2 & iw & w: %d %d %d %.5f\n",
		    aa1, aa2, OmegaAA[k], pomega[OmegaAA[k]]);

	  pomega[OmegaAA[k]] = 0;
	}

      if (com.seqtype == AAseq && com.nrate > 65 && aa1 * 20 + aa2 == ijAAref);	/* if estimating grantham's matrix with aa sequences */
      else
	{
	  w = pomega[OmegaAA[k]];
	}
...
static double
GetMutationMultiplier (int i, int j, int pos, int from[3], int to[3])
{
   ...
   */
  int b1, b2;			//CHHS Unused int variable n=com.ncode removed
...
static int
SelectionCoefficients (FILE * fout, double kappa[], double ppi[],
		       double omega)
{
  ...
  double Qmut[NCODE * NCODE], Qsub[NCODE * NCODE], Qsubw[NCODE * NCODE],
    Ns[NCODE * NCODE], mNs = 0, mNsp = 0, mNsn = 0;
  double maxNs = 0, fNsMut[50] = { 0 }, fNsSub[50] =
  {
  0}, fNsSubw[50] =
  {
  0}, small = min2 (1e-6, 1. / com.ls);
  int ncat = 21;
  ...
  for (i = 1; i < n; i++)
      ...
      for (j = 0; j < i; j++)
	{
	  ...
	  if (com.hkyREV)	/* REV-GTR model */
	    {
	      ...
	      if (b1 == 0 && b2 == 1)
		{
		  q = kappa[0];	/* TC or CT, relative to AG */
		}
	      else if (b1 == 0 && b2 == 2)
		{
		  q = kappa[1];	/* TA or AT */
	  ...
	  if (!com.NSsites
	      && GeneticCode[com.icode][ic1] != GeneticCode[com.icode][ic2])
	    {
	  ...
	  if (fabs (Ns[i * n + j]) > 1e-20)	/* non-neutral mutations */
	    {
	      summutp +=
		(Ns[i * n + j] > 0 ? Qmut[i * n + j] : Qmut[j * n + i]);
	      mNsp +=
		(Ns[i * n + j] >
		 0 ? Qmut[i * n + j] * Ns[i * n + j] : Qmut[j * n +
							    i] * Ns[j *
								    n + i]);
	      mNsn +=
		(Ns[i * n + j] <
	  ...
	  fprintf (fout, "%c%c%c\t", BASEs[from[0]], BASEs[from[1]],
		   BASEs[from[2]]);
	  fprintf (fout, "%c%c%c\t", BASEs[to[0]], BASEs[to[1]],
		   BASEs[to[2]]);
	  fprintf (fout, "%c%c", BASEs[from[pos]], BASEs[to[pos]]);
	  fprintf (fout, "\t%.5f\t%.5f\t%.5f", Ns[i * n + j],
		   Qmut[i * n + j], Qsub[i * n + j]);
	  fprintf (fout, "\t%.5f\t%.5f\t%.5f", Ns[j * n + i],
		   Qmut[j * n + i], Qsub[j * n + i]);

	  if (!com.NSsites)
	    fprintf (fout, "\t%.5f\t%.5f", Qsubw[i * n + j],
		     Qsubw[j * n + i]);
  ...
  for (i = 0; i < n; i++)
      ...
      for (j = 0; j < n; j++)
	{
	  ...
	  for (k = 0; k < ncat - 1; k++)
	    {
	      if (Ns[i * n + j] < (-1 + (k + 1.) * 2 / ncat) * maxNs)
		{
		  break;
		}
	    }
  ...
  for (k = 0; k < ncat; k++)
    {
      fprintf (fout, "%.5f\t%.5f\t%.5f",
	       (-1 + (k + 0.5) * 2 / ncat) * maxNs, fNsMut[k], fNsSub[k]);
...
/// This constructs the rate matrix Q for codon substitution and gets the eigen
...
///
/// @param[in] getstats The kind of statistics returned
/// @param blength
/// @param S One statistics calculated
/// @param dS Another statistics calculated
/// @param dN Another statistics calculated
/// @param Root
/// @param U
/// @param V
/// @param meanrate
/// @param[in] kappa
/// @param[in] omega
/// @param[out] Q The rate matrix for codon substitution
///
void
EigenQcodon (int getstats, double blength, double *S, double *dS,
	     double *dN, double Root[], double U[], double V[],
	     double *meanrate, double kappa[], double omega, double Q[])
{
  int n = Nsensecodon, i, j, k, ic1, ic2, aa1, aa2, b1, b2;
  int ndiff, pos = 0, from[3], to[3];
  double q, mr, rs0, ra0, rs, ra;	//CHHS unused double variable y removed
  double Sphysical, Nphysical, S4, dSnew, dNnew;
  double d4 = 0, d0[3], d[3] /*, ts[3], tv[3] */ ;	/* rates at positions and 4-fold sites *///CMV ts tv not used
  ...
  if (blength >= 0 && (S == NULL || dS == NULL || dN == NULL))
    {
      error2 ("EigenQcodon");
    }

  memset (Q, 0, n * n * sizeof (double));	//CMV used right function to initialize
  ...
  for (i = 1; i < n; i++)
      ...
      for (j = 0; j < i; j++)
	{
	  ...
	  if (com.codonf >= F1x4MG && com.codonf <= FMutSel
	      && com.codonf != Fcodon)
	    {
  ...
  if (getstats == 0)		/* get Root, U, & V */
  ...
  else
    {
      for (i = 0; i < 3; i++)
	{
	  d[i] = d0[i] = /* ts[i] = tv[i] = */ 0;	//CMV ts tv not used
	}
	  ...
	  *dN = blength * ra / (3 * ra0);
	  ...
	  if (noisy >= 9)
	    {
	      printf ("\nd123[*] =%9.5f%9.5f%9.5f  average%9.5f\n", d[0],
		      d[1], d[2], (d[0] + d[1] + d[2]) / 3);
	      printf ("    [B] =%9.5f%9.5f%9.5f  average%9.5f\n", d0[0],
		      d0[1], d0[2], (d0[0] + d0[1] + d0[2]) / 3);
	      printf ("accept  =%9.5f%9.5f%9.5f\n\n", d[0] / d0[0],
		      d[1] / d0[1], d[2] / d0[2]);
	      printf
		("w =%9.5f dN =%9.5f dS =%9.5f d4 =%9.5f (%.1f four-fold sites)\n",
		 w, *dN, *dS, d4, S4 * com.ls);
	      printf ("%12s dN*=%9.5f dS*=%9.5f S* =%7.2f N* =%7.2f\n",
		      "", dNnew, dSnew, Sphysical * com.ls,
		      Nphysical * com.ls);
	    }
	  ...
	  if (com.verbose && times == 1 && com.ns == 2)
	    fprintf (frst1, "\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f",
		     *dN * 2, *dS * 2, dNnew * 2, dSnew * 2, d0[2] * 2,
		     d4 * 2);
	}
      else if (com.NSsites)
	{
...
static int
Qcodon2aa (double Qc[], double pic[], double Qaa[], double piaa[])
{
  /* Qc -> Qaa
...
void
ConditionalPNode (int inode, int igene, double x[])
{
  ...
  for (i = 0; i < nodes[inode].nson; ++i)
	...
	&& !com.oldconP[nodes[inode].sons[i]])
      {
  ...
  for (i = 0; i < nodes[inode].nson; i++)
      ...
      if (com.clock < 5)
	{
	  if (com.clock)
	    {
	      t *= GetBranchRate (igene, (int) nodes[ison].label, x, NULL);
	    }
	  else
	    {
	      t *= com.rgene[igene];
      ...
      if (nodes[ison].nson < 1)	/* tip */
	{
	  //if (nodes[ison].nson < 1 && com.cleandata)    /* tip && clean */
	  if (com.cleandata)	/* tip && clean */
	    {
	      for (h = pos0; h < pos1; h++)
		for (j = 0; j < n; j++)
		  nodes[inode].conP[h * n + j] *=
		    PMat[j * n + com.z[ison][h]];
	    }
	  else			/* tip & unclean */
	    {
	      for (h = pos0; h < pos1; h++)
		for (j = 0; j < n; j++)
		  {
		    for (k = 0, t = 0; k < nChara[(int) com.z[ison][h]]; k++)	//CHHS casting to int avoids compiler warning
		      {
			t += PMat[j * n + CharaMap[(int) com.z[ison][h]][k]];	//CHHS casting to int avoids compiler warning
		      }
	...
	{
	  const int columns_B = pos1 - pos0;
	  double TMP[61];	//CHHS might not always be 61
	  int cb_m = n, cb_n = columns_B, cb_k = n, lda = n, ldb = n, ldc = n;
	  const double alpha = 1.0, beta = 0.0;

	  for (h = pos0; h < pos1; h++)
	    {
//CHHS Step 6, compute P*x; notice that in this approach P is not symmetric
//CHHS DGEMV(TRANS,M,N,ALPHA,A,LDA,X,INCX,BETA,Y,INCY)
	      dgemv_ ("T", &n, &n, &alpha, PMat, &lda,
		      &nodes[ison].conP[h * n], &ONE, &beta, TMP, &ONE);
	      for (j = 0; j < n; j++)
		nodes[inode].conP[h * n + j] *= TMP[j];
	    }
	}
    }				/*  for (ison)  */
...
int
PMatJC69like (double P[], double t, int n)
{
  int i;
  double pii = 1. / n + (1. - 1. / n) * exp (-n / (n - 1.) * t), pij =
    (1. - pii) / (n - 1.);
...
static double
Fcodon_3x4 (double fcodon[], double fb3x4[])
{
...
static void
OutFb3x4 (FILE * fout, double fb3x4[])
{
  int j, k;
  ...
  for (k = 0; k < 4; k++)
    fprintf (fout, "%5c:%7.5f", BASEs[k],
	     (fb3x4[0 * 4 + k] + fb3x4[1 * 4 + k] + fb3x4[2 * 4 + k]) / 3);
}


static void
CountCodons (FILE * fout, double fcodonsg[], double fb3x4sg[], double fb4g[])
{
  ...
  for (j = 0; j < com.ns; j++)
      ...
      for (h = 0; h < com.npatt; h++)
	{
	  for (k = 0; k < 3; k++)
	    {
	      NucListall (CODONs[(int) com.z[j][h]][k], &nb[k], ib[k]);	//CHHS Casting array subscript to int avoid compiler warning with gcc (should have no effect)
	    }
  ...
  for (ig = 0; ig < com.ngene; ig++)
    for (k = 0; k < nc; k++)
      {
	fcodonsg[com.ngene * nc + k] += fcodonsg[ig * nc + k];
      }
  ...
  for (ig = 0; ig < com.ngene + 1; ig++)
    for (j = 0; j < 3; j++)
      for (k = 0; k < 4; k++)
	{
	  fb4g[ig * 4 + k] += fb3x4sg[ig * 12 + j * 4 + k] / 3;
	}
...
static void
AddCodonFreqSeqGene (int js, int ig, double fcodon0[], double fcodon[],
  ...
  for (h = com.posG[ig]; h < com.posG[ig + 1]; h++)
    {
      for (k = 0; k < 3; k++)
	{
	  NucListall (CODONs[(int) com.z[js][h]][k], &nb[k], ib[k]);	//CHHS casting array subscript to int avoids compiler warning (should have no effect)
	}
	...
	FOR (i1, nb[1]) FOR (i2, nb[2])
	{
      ...
      if (k == 0)
	printf ("%s in seq. %d is stop (icode=%d)\n",
	...
	if (ft[ic])
	  fcodon[ic] +=
	    (t > 0 ? com.fpatt[h] * fcodon0[ic] / t : com.fpatt[h] / k);
    }
}


int
InitializeCodon (FILE * fout, double space[])
{
  /* Count codons for genes, calculate site patterns and fpatt.
     ...
     duplicated.
   */
  int j, k, nc = NCODE, ig;	//CHHS unused int variables ic[3], wrongorder[4]={2,1,3,0} removed
  int irf, nrf = 20;
  double *fcodonsg = space;
  //double *fb3x4sg = space + max2((com.ngene + 1), com.ns) * nc;
  double *fb3x4sg = space + (((com.ngene + 1) > com.ns) ? (com.ngene + 1) : com.ns) * nc;	//CMV Expanded max2, here the only use with double *
  ...
  if (com.codonf != Fequal && !com.cleandata)	/* iteration to resolve ambiguities */
	...
	{
	  axtoy (1 / sum (fcodonsg + ig * nc, nc), fcodonsg + ig * nc,
		 fcodon0, nc);
	  ...
	  for (irf = 0; irf < nrf; irf++)
	      ...
	      for (j = 0; j < com.ns; j++)
		{
	      ...
	      for (k = 0; k < 3; k++)
		abyx (1 / sum (fb3x4sg + ig * 12 + k * 4, 4),
		      fb3x4sg + ig * 12 + k * 4, 4);
      ...
      axtoy (1 / sum (fcodonsg + com.ngene * nc, nc),
	     fcodonsg + com.ngene * nc, fcodon0, nc);
      ...
      for (irf = 0; irf < nrf; irf++)	/* calculate com.pi[] */
	{
	  ...
	  for (ig = 0; ig < com.ngene; ig++)
	    for (j = 0; j < com.ns; j++)
	      {
		AddCodonFreqSeqGene (j, ig, fcodon0,
				     fcodonsg + com.ngene * nc, fb3x40,
				     fb3x4sg + com.ngene * 12, fb40,
				     fb4g + com.ngene * 4);
	      }

	  abyx (1 / sum (fcodonsg + com.ngene * nc, nc),
		fcodonsg + com.ngene * nc, nc);

	  for (k = 0; k < 3; k++)
	    abyx (1 / sum (fb3x4sg + com.ngene * 12 + k * 4, 4),
		  fb3x4sg + com.ngene * 12 + k * 4, 4);
  ...
  for (ig = 0; ig < com.ngene + 1; ig++)
      ...
      if (com.codonf == Fequal)
	{
      ...
      else if (com.codonf == Fcodon || com.codonf == FMutSel0
	       || com.codonf == FMutSel)
	{
	  for (k = 0; k < nc; k++)
	    if (FROM64[k] > -1)
	      {
		ppi[FROM64[k]] = fcodonsg[ig * nc + k];
	      }
	}
      else if (com.codonf == F3x4 || com.codonf == F3x4MG)
	{
	  for (k = 0; k < nc; k++)
	    if (FROM64[k] > -1)
	      ppi[FROM64[k]] =
		fb3x4sg[ig * 12 + k / 16] * fb3x4sg[ig * 12 + 4 +
						    (k / 4) % 4] *
		fb3x4sg[ig * 12 + 8 + k % 4];
	}
      else if (com.codonf == F1x4 || com.codonf == F1x4MG)
      ...
      if (ig < com.ngene)
	{
	  ...
	  if (com.codonf == FMutSel0 || com.codonf == FMutSel
	      || com.codonf == F1x4 || com.codonf == F1x4MG)
	    {
	      for (k = 0; k < 4; k++)
		{
		  d1 = com.f3x4[ig][0 * 4 + k] + com.f3x4[ig][1 * 4 +
							      k] +
		    com.f3x4[ig][2 * 4 + k];
  ...
  if (com.verbose && com.ngene == 1)
	...
	{
	  fprintf (fout, "%12.8f",
		   GeneticCode[com.icode][k] == -1 ? 0 : com.pi[FROM64[k]]);
...
static int
AA2Codonf (double faa[20], double fcodon[])
{
  ...
  FOR (ic, 64) if ((iaa = GeneticCode[com.icode][ic]) != -1)
    {
      NCsyn[iaa]++;
    }
...
GetDaa (FILE * fout, double daa[])
{
  ...
  if (com.model == REVaa_0 || com.model == REVaa)
    {
      puts (", to get initial values.");
    }
...
SetAA1STEP (void)
{
   ...
   */
  int ncode0 = com.ncode, nc, naa =
    20, i, j, k, ic1, ic2, ndiff, from[3], to[3];
  int *Q = (int *) PMat;
...
GetOmegaAA (int OmegaAA[])
{
   ...
   */
  const char *OmegaAAf = "OmegaAA.dat";	//CMV added const
  char line[1024];
  FILE *fin;
  ...
  if (com.nOmegaType < 1 || com.nOmegaType > 65 - 1)
      ...
      if (com.seqtype == AAseq)
	{
	  ...
	  FOR (i, naa) FOR (j, i) if (i * naa + j != ijAAref
				      && AA1STEP[i * (i - 1) / 2 + j])
	    {
      ...
      FOR (i, naa) FOR (j, i) if (AA1STEP[i * (i - 1) / 2 + j])
	{
	  OmegaAA[i * (i - 1) / 2 + j] = com.nOmegaType++;
	}

      printf
	("%d dN/dS ratios estimated from data.\nCrtl-C if wrong. Enter to continue.",
	 com.nOmegaType);
      getchar ();
    }
  else
	...
	if (j != iomega + 1)
	  {
	    printf ("err data file %s.", OmegaAAf);
	    exit (-1);
	  }
	...
	for (j = 0, npair = 0;
	     j < nline - 1 && line[j] && line[j] != '\n'; j++)
	  {
	    ...
	    iaa = CodeChara ((char) iaa, AAseq);
	    jaa = CodeChara ((char) jaa, AAseq);
...
GetCodonFreqs2 (void)
{
  ...
  double *pi = com.pi, fb3x4[12], fb4[4] /*, GC[3] = { 0 } */ ;	//CMV GC never used
...
lfun2dSdN (double x[], int np)
{
  ...
  if (com.hkyREV == 0)
	...
	{
	  com.kappa = x[k];	/* Is this necessary? */
	}
    }
  ...
  if (!com.fix_kappa || !com.fix_omega)
    EigenQcodon (0, -1, NULL, NULL, NULL, Root, U, V, &mr, com.pkappa,
		 com.omega, PMat);
...
static int
VariancedSdN (double t, double omega, double vtw[2 * 2], double vdSdN[2 * 2])
{
   ...
   */
  int np = 2;
  double JacobiSN[2 * 2], T1[2 * 3], T2[2 * 3], S, dS, dN, dS1, dN1, dS2,
    dN2, eh, mr = 0;
  ...
  /* printf("\nt & w: %.5f %.5f\n", t, omega);
     matout(F0,vtw, 2,2); */
  EigenQcodon (1, t, &S, &dS, &dN, NULL, NULL, NULL, &mr, com.pkappa,
	       omega, PMat);

  eh = (t + 1) * Small_Diff;
  EigenQcodon (1, t + eh, &S, &dS1, &dN1, NULL, NULL, NULL, &mr,
	       com.pkappa, omega, PMat);
  EigenQcodon (1, t - eh, &S, &dS2, &dN2, NULL, NULL, NULL, &mr,
	       com.pkappa, omega, PMat);
  JacobiSN[0 * np + 0] = (dS1 - dS2) / (2 * eh);
  JacobiSN[1 * np + 0] = (dN1 - dN2) / (2 * eh);

  eh = (omega + 1) * Small_Diff;
  EigenQcodon (1, t, &S, &dS1, &dN1, NULL, NULL, NULL, &mr, com.pkappa,
	       omega + eh, PMat);
  EigenQcodon (1, t, &S, &dS2, &dN2, NULL, NULL, NULL, &mr, com.pkappa,
	       omega - eh, PMat);
...
static double distanceHKY85 (double x[], double *kappa, double alpha);

static int
distance3pos (double dHKY[], double kHKY[], int *sites4, char *z1, char *z2)
{
...
int
PairwiseCodon (FILE * fout, FILE * fds, FILE * fdn, FILE * ft, double space[])
{
  ...
  double x[10] = { .9, 1, .5, .5, .5, .5, .3 }, xb[10][2] =
  {
    {
    1e-5, 50}
  };				//CHHS Unused double variable large=50 removed
  double kappab[2] = { .01, 999 }, omegab[2] =
  {
  .001, 99};
  double lnL, e = 1e-7, *var = space + NP, S, dS, dN, mr = 0;
  double JacobiSN[2 * 3], T1[2 * 3], T2[2 * 3], vSN[2 * 2], dS1, dN1,
    dS2, dN2, y[3], eh;
  ...
  for (is = 0; is < com.ns; is++)
      ...
      for (js = 0; js < is; js++)
	{
	  ...
	  if (com.cleandata)
		...
		{
		  FOR (i0, 2) FOR (k, 3) codon[i0][k] =
		    pz0[i0 == 0 ? is : js][h * 3 + k];
		  ...
		  for (i0 = 0, missing = 0; i0 < 2; i0++)
			...
			ic[i0] =
			  FROM64[ib[0][0] * 16 + ib[1][0] * 4 + ib[2][0]];
		    }
	  ...
	  if (noisy >= 9)
	    {
	      FPN (F0);
	      FOR (k, np) printf (" %12.6f", x[k]);
	      FPN (F0);
	      FOR (k, np) printf (" %12.6f", xb[k][0]);
	      FPN (F0);
	      FOR (k, np) printf (" %12.6f", xb[k][1]);
	      FPN (F0);
	    }
	  ...
	  if (np)
	    ming2 (noisy > 3 ? frub : NULL, &lnL, lfun2dSdN, NULL, x,
		   xb, space, e, np);
	  else
	    {
	      x[1] = x[2] = com.kappa = com.omega = 0;
	      lnL = 0;
	    }

	  lnLmodel = lnL;
	  fprintf (fout, "\nlnL =%12.6f\n", -lnL);
	  FOR (k, np) fprintf (fout, " %8.5f", x[k]);
	  FPN (fout);

	  if (noisy > 2)
	    {
	      printf ("\n\nt_NG = %.5f\tMLEs: ",
		      SeqDistance[is * (is - 1) / 2 + js]);
	  ...
	  FPN (fout);
	  EigenQcodon (1, x[0], &S, &dS, &dN, NULL, NULL, NULL, &mr,
		       com.pkappa, com.omega, PMat);
	  ...
	  if (noisy >= 3)
		...
		("\nNucleotide-based analysis (approximate MLEs; use baseml to get proper MLEs):");
	      printf ("\ndHKY (123-4):");
	      FOR (k, 4) printf (" %8.5f", dHKY[k]);
	      printf ("\nkHKY (123-4):");
	      FOR (k, 4) printf (" %8.5f", kHKY[k]);
	  ...
	  fprintf (frst, "%8.1f %8.1f %8.4f %8.4f %8.4f", com.ls * 3 - S,
		   S, dN, dS, com.omega);

	  for (k = 0; k < np; k++)
	    {
	      fprintf (frst, " %8.4f", x[k]);
	    }
	  ...
	  if (com.getSE)
	    fprintf (frst, " +-%6.4f",
		     (var[k * np + k] > 0. ? sqrt (var[k * np + k]) : -1));
	  ...
	  if (com.getSE && !com.fix_omega)
		...
		EigenQcodon (1, y[0], &S, &dS1, &dN1, NULL, NULL, NULL,
			     &mr, com.pkappa, com.omega, PMat);
		y[k] -= 2 * eh;
		...
		com.omega = y[1 + !com.fix_kappa];
		EigenQcodon (1, y[0], &S, &dS2, &dN2, NULL, NULL, NULL,
			     &mr, com.pkappa, com.omega, PMat);
  ...
  com.npatt = npatt0;
  FOR (h, npatt0) com.fpatt[h] = fpatt0[h];
  free (fpatt0);
  return (0);
}
...
lfun2AA (double t)
{
  ...
  if (al == 0)
    {
      FOR (k, n) expt[k] = exp (t * Root[k]);
    }
  else
    {
      FOR (k, n) expt[k] = pow (al / (al - t * Root[k]), al);
    }
...
lfun2AArev (double x[], int np)
{
   ...
   */
  int i, j, k, n = 20;
  double pi[3][20], *p, Q[3][400], *Fe = Q[0], t, t1, t2, m1, m2, lnL = 0;	//CHHS Unused double *T=Q[0] removed
...
PairwiseAArev (int is, int js)
{
  ...
  if (_nestS)
    {
      for (i = 1, k = 0; i < n; i++)
	FOR (j, i) if (i * n + j != ijAAref)
	  x[k++] = (com.daa[i * n + j] + .001) * (0.8 + 0.4 * rndu ());
    }
  ...
  ming2 (noisy > 2 ? frub : NULL, &lnL, lfun2AArev, NULL, x, xb,
	 com.space, e, np);
...
int
PairwiseAA (FILE * fout, FILE * f2AA)
{
  ...
  for (is = 0; is < com.ns; is++, FPN (F0), FPN (fout), FPN (f2AA))
      ...
      for (js = 0; js < is; js++)
	{
	  ...
	  xb[0] = SeqDistance[is * (is - 1) / 2 + js];
	  x = xb[0] * 1.5;
	  step = xb[0];
	  LineSearch (lfun2AA, &lnL, &x, xb, step, 1e-7);
	  fprintf (f2AA, " %7.4f", x);
	  fprintf (fout, " %7.4f", x);
	  //if (com.getSE); //CMV commented because does nothing
	}			/* for (js) */
    }				/* for (is) */
...
char
GetAASiteSpecies (int species, int sitepatt)
{
   ...
   */
  int n = com.ncode, c, naa = 0, k;	//CHHS Initialization of naa added, avoids compiler warning in gcc regarding possibly using naa unitialized (should have no effect)
  char aa, newaa;
  int inewaa;

  if (com.seqtype != 1)
    error2
      ("GetAASiteSpecies() right now works for codon seqs only.  Check.");
  ...
  if (c < n)
	...
	{
	  inewaa = GeneticCode[com.icode][FROM61[(int) CharaMap[c][k]]];	//CHHS conversion of array subscript avoids a compiler warning (should have no effect)
	  ...
	  newaa = AAs[inewaa];	//CHHS conversion of array subscript avoids a compiler warning (should have no effect)
...
static int
PrintProbNSsites (FILE * frst, double prob[], double meanw[], double varw[],
		  int ncat, int refsp)
{
  ...
  double mpostp[NCATG];
  const char *sig;		//CMV added const
  char aa;
  ...
  if (com.model == 0)		/* NSsites models */
	...
	if (BayesEB == 1
	    || (com.freqK[ir] > .1 / com.ls && com.rK[ir] > wpos))
	  {
	    it = 1;
	  }
      ...
      if (!com.aaDist && it)
	{
	  ...
	  for (h = 0; h < lst; h++)
	      ...
	      if (psel > cutoff)
		{
		  ...
		  aa = GetAASiteSpecies (refsp, hp);
		  fprintf (fout, "%6d %c %10.3f%-8s %.3f", h + 1, aa,
			   psel, sig, meanw[hp]);
		  fprintf (frst, "%6d %c %10.3f%-8s %.3f", h + 1, aa,
			   psel, sig, meanw[hp]);
...
static int
lfunNSsites_rate (FILE * frst, double x[], int np)
{
   ...
   */
  int h, hp, ir, it = 0, refsp = 0;	//CHHS Variable k=com.ntime+com.nrgene+com.nkappa not used, removed
  double lnL = 0, fh;
  double w2 = x[com.np - 1], psel = 0, *meanw, maxmw, minmw, wpos =
    1.1, cutoff = 0.5;
  const char *sig;
  char aa;
  ...
  char sitelabel[96];
  const char *colors[5] = { "darkblue", "lightblue", "purple", "pinkred", "red" };	//CMV added const
  const char *colorvalues[5] = { "[2,2,120]", "[133,57,240]", "[186,60,200]", "[200,60,160]", "[250,5,5]" };	//CMV added const
  ...
  if (com.nparK)
    {
      error2 ("lfunNSsites_rate to be done for HMM.");
    }
  ...
  for (h = 0; h < com.npatt; h++)
      ...
      for (ir = 0, fh = meanw[h] = 0; ir < com.ncatG; ir++)
	{
      ...
      for (ir = 0, meanw[h] /= fh; ir < com.ncatG; ir++)
	{
  ...
  fprintf (frst,
	   "\nNaive Empirical Bayes (NEB) probabilities for %d classes",
	   com.ncatG);
	  ...
	  for (h = 0; h < lst; h++)
	    {
	      ...
	      if (com.rK[0] > wpos)
		{
		  psel = com.fhK[0 * com.npatt + hp];
		}

	      if (com.rK[1] > wpos)
		{
		  psel += com.fhK[1 * com.npatt + hp];
		}
	  ...
	  for (h = 0; h < lst; h++)
	    {
	      ...
	      aa = GetAASiteSpecies (refsp, hp);
	      psel =
		com.fhK[2 * com.npatt + hp] + com.fhK[3 * com.npatt + hp];
  ...
  if (com.verbose && com.model == 0)
      ...
      if (fsites)
	{
	  ...
	  printf
	    ("Choose color scheme (0: %d colors, 1: white->red, 2: rainbow) ",
	     ncolors);
	  scanf ("%d", &continuous);
	  ...
	  if (continuous == 0)
	    for (it = 0; it < ncolors; it++)
	      printf ("\t%-10s %-20s mean_w < %7.5f\n",
		      colors[it], colorvalues[it],
		      (it + 1) * (maxmw - minmw) / ncolors);
	  ...
	  for (h = 0; h < lst; h++)
	    {
	      ...
	      if (it - 1 != h)
		{
		  puts ("Site number wrong.  Giving up.");
		  break;
		}
	      ...
	      if (continuous == 0)
		{
		  for (it = 0; it < ncolors; it++)
		    if (meanw[hp] <
			minmw + (it + 1.) * (maxmw - minmw) / ncolors + 1e-9)
		      {
			break;
		      }
		...
		{
		  rainbowRGB ((meanw[hp] - minmw) / (maxmw - minmw +
						     1e-9), &R, &G, &B);
		  fprintf (fras, "select %s\n\t\tcolor [%d, %d, %d]\n",
			   sitelabel, R, G, B);
		}
  ...
  if (com.model == 0 && (com.NSsites == 2 || com.NSsites == 8) && (com.fix_omega != 1 || com.omega != 1))	/* BEB for M2 & M8 */
    {
...
SimulateData2s61 (void)
{
  ...
  double y, x[6] =
    { 1, 1, 1 }, xb[6][2], S, dN, dS, dNt, dSt, om, lnL, mr = 0;
  ...
  for (ip = 0; ip < 99; ip++)
      ...
      for (i = 0; i < com.ncatG; i++)
	{
      ...
      printf ("\nw: ");
      FOR (i, com.ncatG) printf ("%7.4f", com.rK[i]);
      FPN (F0);
      fprintf (frst, "\nSite classe model (K=%d)\np: ", com.ncatG);
      ...
      for (i = 0; i < com.ncatG; i++)
	{
      ...
      fputs ("\nw: ", frst);
      FOR (i, com.ncatG) fprintf (frst, "%7.4f", com.rK[i]);
      FPN (frst);
      ...
      for (j = 0, Qfactor_NS = 0, dS = dN = 0; j < com.ncatG; j++)
	{
      ...
      EigenQcodon (1, t0, &S, &dS, &dN, NULL, NULL, NULL, &mr, &kappa0,
		   omega0, space);
      om = omega0;
#endif
      printf ("\nCorrect values");
...
#if (DSDN_MC_SITES)
//CHHS code not needed, removed
#else
      EigenQcodon (0, -1, NULL, NULL, NULL, Root, U, V, &mr, &kappa0,
		   omega0, PMat);
      PMatUVRoot2 (PMat, t0, n, U, V, Root);

      for (i = 0, h = 0; i < n; i++)
	for (j = 0; j <= i; j++)	/* why for each il? */
	  {
      ...
      for (i = h = 0, com.ls = 1, com.npatt = npatt0; i < n; i++)
	for (j = 0; j <= i; j++)
	  {
	    com.z[0][h] = (char) i;
	    com.z[1][h] = (char) j;
      ...
      if (fabs (1 - sum (Efij, npatt0)) > 1e-6)
	{
	  error2 ("sum Efij != 1");
	}
      ...
      for (il = 0; il < nil + 1; il++)
	{
	  ...
	  for (ir = 0, zero (mx, 6), zero (vx, 6), zero (mse, 6); ir < nr;
	       ir++)
	    {
	      MultiNomial (com.ls, npatt0, Efij, nobs, NULL);

	      for (i = 0, com.npatt = 0, zero (com.pi, n); i < n; i++)
		for (j = 0; j <= i; j++)
		  if (nobs[k = i * (i + 1) / 2 + j])
		    {
		      com.z[0][com.npatt] = i;
	      ...
	      for (i = 0, zero (com.pi, n); i < com.npatt; i++)
		{
	      ...
	      x[2] = omega0;
	      /* printf("\nlnL=%9.6f\n",-lfun2dSdN(x,3)); */
	      ming2 ((noisy ? F0 : NULL), &lnL, lfun2dSdN, NULL, x, xb,
		     space, 1e-7, 3);
	      EigenQcodon (1, x[0], &S, &x[3], &x[4], NULL, NULL, NULL,
			   &mr, &x[1], x[2], space);
	      FOR (j, 5)
	      {
		...
		mx[j] = (mx[j] * ir + x[j]) / (ir + 1.);
	      ...
	      printf ("\r%4d%8.4f%8.4f%8.4f  %8.4f%8.4f%8.4f%8.4f%8.4f",
		      ir + 1, x[0], x[1], x[2], mx[0], mx[1], mx[2],
		      mx[3], mx[4]);
	    }

	  if (nr > 1)
	    {
	      FOR (j, 5) vx[j] = sqrt (vx[j] / (nr - 1.) / nr);
	      mse[0] = sqrt (mse[0] / nr);
	    }

	  fprintf (frst, "%4d ", com.ls);
	  FOR (i, 5) fprintf (frst, "%7.4f +%7.4f", mx[i], vx[i]);
	  FPN (frst);
	}			/* for (ii) */
    }				/* for(ip) */
...
Get4foldSites (void)
{
  ...
  if ((mark4 = (char *) malloc (com.ls * sizeof (char))) == NULL)
    {
      error2 ("oom mark");
    }
  ...
  for (h = 0, ls4 = 0; h < com.ls; h++)
      ...
      if (nb[0] == 1 && nb[2] == 1 && FourFold[ib[0][0]][ib[1][0]])
	{
	  for (j = 1; j < com.ns; j++)
	    for (k = 0; k < 2; k++)
	      if (com.z[j][h * 3 + k] != com.z[0][h * 3 + k])
		{
		  goto nextsite;
		}
...
static double
distanceHKY85 (double x[], double *kappa, double alpha)
{
...
void
get_grid_para_like_M2M8 (double para[4][100], int n1d, int dim,
			 int M2a, int ternary, double p0b[],
			 double p1b[], double w0b[], double wsb[],
			 double p_beta_b[], double q_beta_b[],
			 double x[], double *S)
{
  ...
  if (noisy > 3)
    for (k = 0; k < com.ncatG; k++)
      printf ("S%d w log{f(x|w)}: %9.4f  %12.6f\n",
	      site, com.rK[k],
	      (com.NnodeScale ? com.fhK[k * com.npatt +
					site] : log (com.fhK[k *
							     com.npatt +
							     site])));
...
static void
get_pclassM_iw_M2M8 (int *iw, double *pclassM,
		     int iclassM, int ip[], double para[][100],
		     int n1d, int M2a, int ternary)
{
  ...
  if (com.NSsites == NSpselection)	/* M2 & M2a */
      ...
      else
	{
	  if (iclassM < 2)
	    {
	      *pclassM = para[iclassM][ip[iclassM]];	/* p0 or p1 */
	    }
	  else
	    {
	      *pclassM = 1 - para[0][ip[0]] - para[1][ip[1]];	/* p2 */
	    }

	  //*pclassM = max2(*pclassM, 0);
	  *pclassM = (*pclassM > 0) ? *pclassM : 0;
	}
      ...
      if (iclassM < n1d)	/* w from beta */
	{
	  ...
	  if (iclassM > 0)
	    {
	      cdf0 = CDFBeta (iclassM / (double) n1d, p, q, 0);
	    }

	  if (iclassM < n1d - 1)
	    {
	      cdf1 = CDFBeta ((iclassM + 1.0) / n1d, p, q, 0);
	    }
...
static int
lfunNSsites_M2M8 (FILE * frst, double x[], int np)
{
  ...
  0, 1}, w0b[] =
  {
  0, 1};			/* w0b for M2a only. */
  double wsb[] = { 1, 11 };	/* for w2 in M2 & M2a, or for ws in M8 */
  double p_beta_b[] = { 0, 2 }, q_beta_b[] =
  {
  0, 2};

  int dim = (com.NSsites == 8 || M2a ? 4 : 3), ngrid, igrid, ip[4] =
  {
  0}, j, k, h, it;
  ...
  double fh, fX, *lnfXs, S1, S2, *lnprior, *pclassM, *meanw, *varw,
    *postSite, *postp0p1 = NULL;
  double fh1site, t, v;
  char timestr[32];
  const char *paras[4];
  ...
  if (com.NSsites == 8)
    {
      com.ncatG = n1d + n1d;	/* w from beta & ws */
    }
  else
    {
      com.ncatG = (M2a ? n1d + 1 + n1d : 2 + n1d);	/* w0, w1=1, w2 */
    }

  if ((meanw =
       (double *) malloc (com.npatt * (2 + nclassM) * sizeof (double))) ==
      NULL)
    {
      error2 ("oom meanw");
    }

  varw = meanw + com.npatt;
  postSite = varw + com.npatt;
  ternary = (com.NSsites == 2 && ternary);

  if (ternary
      && (postp0p1 = (double *) malloc (n1d * n1d * sizeof (double))) == NULL)
    {
      error2 ("oom postp0p1");
    }
  ...
  if ((pclassM =
       (double *) malloc (ngrid * nclassM *
			  (sizeof (double) + sizeof (int)))) == NULL)
    {
  ...
  BayesEB = 1;
  get_grid_para_like_M2M8 (para, n1d, dim, M2a, ternary, p0b, p1b, w0b,
			   wsb, p_beta_b, q_beta_b, x, &S1);
  ...
  for (igrid = 0; igrid < ngrid; igrid++)
      ...
      if (com.NSsites == 2 && !ternary && para[0][ip[0]] + para[1][ip[1]] > 1)
	{
      ...
      for (k = 0; k < nclassM; k++)
	{
	  get_pclassM_iw_M2M8 (&iw[igrid * nclassM + k],
			       &pclassM[igrid * nclassM + k], k, ip, para,
			       n1d, M2a, ternary);
	}
    }
  ...
  for (igrid = 0; igrid < ngrid; igrid++)
      ...
      for (h = 0, lnfXs[igrid] = 0; h < com.npatt; h++)
	{
	  for (k = 0, fh = 0; k < nclassM; k++)
	    fh +=
	      pclassM[igrid * nclassM +
		      k] * com.fhK[iw[igrid * nclassM + k] * com.npatt + h];
      ...
      lnfXs[igrid] +=
	(com.NSsites == 8 ? lnprior[ip[0]] : lnprior[ip[0] * n1d + ip[1]]);
      t = lnfXs[igrid] - S2;
  ...
  for (h = 0; h < com.npatt; h++)
    {
      S1 = -1e300;
      FOR (j, nclassM) postSite[j * com.npatt + h] = 1;
      S2 = -1e300;
      meanw[h] = varw[h] = 1;
      ...
      for (iclassM = 0; iclassM < nclassM; iclassM++)
	{
	  for (igrid = 0; igrid < ngrid; igrid++)
	      ...
	      for (j = dim - 1, it = igrid; j >= 0; j--)
		{
	      ...
	      if (com.NSsites == 2 && !ternary
		  && para[0][ip[0]] + para[1][ip[1]] > 1)
		{
		  continue;
		}
	...
	printf ("\r\tdid %3d / %3d patterns  %s", h + 1, com.npatt,
		printtime (timestr));
    }				/* for(h) */

  /* print out posterior probabilities */
  fprintf (frst,
	   "\nBayes Empirical Bayes (BEB) probabilities for %d classes (class)",
	   nclassM);
  ...
  if (ternary)
      ...
      for (k = 0; k < n1d * n1d; k++)
	{
	  ...
	  if (fabs ((float) (square ((int) sqrt (k + 1.)) - (k + 1))) < 1e-5)	//CMV added cast to select meaningful fabs
	    {
	      FPN (fout);
	    }
	}
...
/********************************************************************/
void
get_grid_para_like_AC (double para[][100], int n1d, int dim,
		       double w0b[], double w2b[], double x[], double *S)
{
   ...
   */
  int modelA = (com.model == 2), i, k, h, iw;	//CHHS Variable int site=10 not used, removed
  ...
  *S = 0;
  ...
  for (iw = 0; iw < com.ncatG; iw++)
      ...
      if (modelA)		/* model A:  10 + 1 + 100 + 10 */
	{
	  if (iw < n1d)
	    {
	      wbranches[0] = wbranches[1] = para[2][iw];	/* class 0:  w0 */
	    }
	  else if (iw == n1d)
	    {
	      wbranches[0] = wbranches[1] = 1;	/* class 1:  w1 */
	    }
  ...
  com.NSsites = NSsites0;
  com.model = model0;
}

static void
get_pclassM_iw_AC (int *iw, double *pclassM, int iclassM, int ip[],
		   double para[][100], int n1d)
{
  ...
  *pclassM = p[iclassM <= 2 ? iclassM : 2];
  ...
  if (iclassM == 0)
    ...
    {
      *iw = n1d + 1 + ip[2] * n1d + ip[3];	/* class 2a model A: w0 & w2 */
    }
  else
    {
      *iw = n1d + 1 + n1d * n1d + ip[3];	/* class 2b model A: w1 & w2 */
    }
}


static int
lfunNSsites_AC (FILE * frst, double x[], int np)
{
  ...
  int modelA = (com.model == 2), dim =
    (modelA ? 4 : 3 + com.nbtype), ngrid, igrid, ip[3 + NBTYPE], j, k,
    h, hp, it;
  int refsp = 0, ncatG0 = com.ncatG, lst =
    (com.readpattern ? com.npatt : com.ls);
  /* # of site classes under model and index for site class */
  ...
  double fhk[4], t, cutoff = 0.5; //CHHS fhk[4] was fhk[3] <- this was a bug, fixed now
  char timestr[32], paras[3 + NBTYPE][5] =
    { "p0", "p1", "w0", "w2", "w3" }, aa;
  const char *sig;
  ...
  if (!modelA)
    for (i = 2; i < com.nbtype; i++)
      {
	sprintf (paras[3 + i], "w%d", i + 2);
      }

  for (i = 0, ngrid = 1; i < dim; i++)
    {
      ngrid *= n1d;
    }
  ...
  if (modelA)
    ...
    {
      for (i = 0, com.ncatG = 1; i < com.nbtype; i++)
	{
	  com.ncatG *= n1d;	/* w2 w3 w4 ... */
	}

      com.ncatG += n1d + 1;	/* w0 & w1=1 */
    }

  k = (n1d * n1d + com.npatt * nclassM + ngrid +
       ngrid * nclassM) * sizeof (double) + ngrid * nclassM * sizeof (int);
  ...
  for (igrid = 0; igrid < ngrid; igrid++)
      ...
      for (j = dim - 1, it = igrid; j >= 0; j--)
	{
      ...
      for (k = 0; k < nclassM; k++)
	{
	  get_pclassM_iw_AC (&iw[igrid * nclassM + k],
			     &pclassM[igrid * nclassM + k], k, ip, para, n1d);
	}
    }
  ...
  for (h = 0; h < com.npatt; h++)
      ...
      for (j = 0; j < nclassM; j++)
	{
      ...
      if ((h + 1) % 10 == 0 || h == com.npatt - 1)
	printf ("\r\tdid %3d / %3d site patterns  %s", h + 1, com.npatt,
		printtime (timestr));
    }				/* for(h) */

  if (debug)
    for (k = 0, printf ("\nS%d: ", site); k < nclassM; k++)
      {
	printf ("%7.4f", postSite[k * com.npatt + site]);
      }
...
int
EigenQaa (FILE * fout, double Root[], double U[], double V[], double rate[])
{
  /*  Codon-based AA model must use FromCodon, even if com.aaDist==AAClasses.
   */
  int naa = 20, i, j, k;
  double Q[20 * 20], mr = 0, t = 0;
  double space[NCODE * NCODE * 2 + NCODE], *Qc =
    space + NCODE * NCODE, *space_pisqrt = Qc + NCODE * NCODE;
  ...
  switch (com.model)
    ...
    case (FromCodon):
      EigenQcodon (0, -1, NULL, NULL, NULL, Root, U, V, &mr,
		   (com.hkyREV
		    || com.codonf == FMutSel ? rate : &com.kappa),
		   com.omega, Qc);
  ...
  if (fout && com.model >= REVaa_0)
      ...
      for (i = 0; i < naa; i++)
	{
	  ...
	  for (j = 0; j < i; j++)
	    fprintf (fout, " %4.0f", Q[i * naa + j] / t / com.pi[j] * 100);

	  for (j = 0; j < i; j++)
	    fprintf (fAArate, " %5.1f", Q[i * naa + j] / t / com.pi[j] * 100);
